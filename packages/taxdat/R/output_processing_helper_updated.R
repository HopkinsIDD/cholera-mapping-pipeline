...
get_rate_raster <- function(covar_data_filename,
  ...
  if(params$single_year==FALSE){
    ...
    obs_year_row_number <- nrow(modeled_cases_mean_by_grid_layer_tmp)/c(end_year-start_year+1)*( obs_year[order(obs_year)]-start_year)#need to be changed
...
aggregate_affected_pop_across_cells <- function(rate_raster_cropped,threshold_list){
...
#  for (layer_idx in seq_len(nlayers(rate_raster_cropped))) {
  #sf object
  for (layer_idx in 1:ncol(rate_raster_cropped)) {
    results_by_layer[[layer_idx]] <- within_layer_aggregator(rate_raster_cropped=rate_raster_cropped[,c(layer_idx,ncol(rate_raster_cropped))],
