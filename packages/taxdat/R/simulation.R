...
create_test_raster <- function(
  ...
  nlayers=9,
  seed,
  test_extent=create_test_extent(seed)
) {
  seed <- get_or_set_seed(seed)
...
## @name create_test_2d_polygons
## See create_test_polygons for details (dimension=2)
create_test_2d_polygons <- function(
...
## @name create_test_lines
## See create_test_polygons for details (dimension=1)
create_test_lines <- function(
  test_raster = create_test_raster(),
  number=10,
  snap=FALSE,
  randomize=TRUE,
  ...
  test_voronoi  <- sf::st_sf(
    sf::st_collection_extract(sf::st_voronoi(sample_points))
  )
  test_voronoi$point_idx <-
    rc$idx[unlist(sf::st_intersects(test_voronoi, rc))]
...
## @name create_test_points
## See create_test_polygons for details (dimension=0)
...
#' @param seed integer A seed to use for the randomly constructed portions of this object
create_test_layered_polygons <- function(
  test_raster = create_test_raster(),
  base_number = 4,
  n_layers=3,
  factor=3,
  ...
  smallest_layer  <- sf::st_sf(
    geometry = create_test_polygons(
    test_raster = test_raster,
    number = base_number * factor ^ (n_layers - 1),
    randomize = randomize,
    snap = snap,
    dimension = 2
  ))
  smallest_layer[[paste("name", n_layers, sep = "_")]] <-
    seq_len(nrow(smallest_layer))
  for (layer in rev(seq_len(n_layers - 1))) {
    ...
    start_names  <-
      sample(previous_layer_names, length(previous_layer_names) / factor)
    smallest_layer[[layer_name]]  <- 1 * NA
    counter  <- 1
    for (name in start_names) {
      smallest_layer[
        smallest_layer[[parent_name]] == name, paste(layer_name)
      ] <- counter
      counter  <- counter + 1
    }
...
independent_covariate <- function(
  ...
  if (spatially_variable & temporally_variable) {
    return(matrix(scale(rnorm(n_grid)), nrow = n_spatial, ncol = n_temporal))
  }
  if (spatially_variable) {
    return(matrix(
      scale(rnorm(n_spatial)),
      nrow = n_spatial,
      ncol = n_temporal
    ))
  }
...
smoothed_covariate <- function(
  ...
  if (spatially_variable) {
    return(matrix(
      scale(smoothing_function(1, rep(0, n_spatial), covariance, centers)),
      nrow = n_spatial,
      ncol = n_temporal
    ))
  }
  if (temporally_variable) {
    return(matrix(
      scale(smoothing_function(1, rep(0, n_temporal), covariance, centers)),
      nrow = n_spatial,
      ncol = n_temporal,
      byrow = TRUE
    ))
...
polygonal_covariate <- function(
  ...
  for (name in tmp_names) {
    ...
    for (p_idx in seq_len(nrow(polygons))) {
      polygonal_contribution[tmp[[p_idx]]]  <-
        polygonal_contribution[tmp[[p_idx]]] +
        polygons[[paste(name, "value", sep = "_")]][p_idx]
    }
  }
  return(matrix(
    scale(polygonal_contribution),
    nrow = n_spatial,
    ncol = n_temporal
  ))
}
...
#' @param seed integer A seed to use for the randomly constructed portions of this object
create_test_covariate <- function(
  test_raster = create_test_raster(),
  nonspatial = TRUE,
  nontemporal = TRUE,
  ...
  radiating_polygons = sf::st_sf(
    sf::st_union(create_test_polygons(dimension = 1))
  ),
  ...
  ## Add in covariates
  ## covariate functions return appropriately sized matrices which are 0 if the covariate is not used
  test_raster[['covariate']] <- test_raster[['covariate']] + as.numeric(independent_covariate(test_raster, nonspatial, nontemporal) * weights[1])
  test_raster[['covariate']] <- test_raster[['covariate']] + as.numeric(smoothed_covariate(test_raster, spatially_smooth, temporally_smooth, smoothing_function, rho) * weights[2])
  test_raster[['covariate']] <- test_raster[['covariate']] + as.numeric(polygonal_covariate(test_raster, polygonal, polygons) * weights[3])
  test_raster[['covariate']] <- test_raster[['covariate']] + as.numeric(radiating_covariate(test_raster, radiating, radiating_polygons, radiating_means, radiation_function) * weights[4])
  test_raster[['covariate']] <- test_raster[['covariate']] + as.numeric(constant_covariate(test_raster, constant) * weights[5])
...
#' @param family character Currently unused
#' @param seed integer A seed to use for the randomly constructed portions of this object
create_multiple_test_covariates <- function(
  test_raster = create_test_raster(),
  ncovariates = 5,
  nonspatial=c(TRUE, FALSE, FALSE, FALSE, FALSE),
  nontemporal=c(FALSE, TRUE, FALSE, FALSE, FALSE, TRUE),
  spatially_smooth=c(FALSE, FALSE, TRUE, TRUE, FALSE),
  temporally_smooth=c(FALSE, FALSE, FALSE, TRUE, FALSE, TRUE),
  polygonal=c(TRUE, FALSE, FALSE, FALSE, FALSE),
  radiating = c(FALSE, TRUE, FALSE, FALSE, TRUE),
  constant = c(FALSE, FALSE, FALSE, FALSE, FALSE),
  rho = rep(.999999, times = ncovariates),
  polygons= create_test_layered_polygons(),
  radiating_polygons = list(
    NA,
    create_test_polygons(dimension = 0, number = 2),
    NA,
    NA,
    sf::st_union(create_test_polygons(dimension = 1))
  ),
  ...
  for (idx in seq_len(ncovariates)) {
    tmp  <- create_test_covariate(
      test_raster = test_raster,
      nonspatial = nonspatial[idx],
      nontemporal = nontemporal[idx],
      spatially_smooth = spatially_smooth[idx],
      temporally_smooth = temporally_smooth[idx],
      polygonal = polygonal[idx],
      radiating = radiating[[idx]],
      constant = constant[[idx]],
      rho = rho[idx],
      polygons = polygons,
      smoothing_function = smoothing_function[[idx]],
      radiation_function = radiation_function[[idx]],
      radiating_polygons = radiating_polygons[[idx]],
      radiating_means = radiating_means[[idx]],
      weights = weights[[idx]],
      family = family
    )
...
#' @param seed integer A seed to use for the randomly constructed portions of this object
create_underlying_distribution <- function(
  covariates = create_multiple_test_covariates(),
...
#' @param seed integer A seed to use for the randomly constructed portions of this object
observe_gridcells <- function(
  underlying_distribution = create_underlying_distribution(),
  proportion_observed = 1,
  number_draws = 1,
  spatial_observation_bias = TRUE,
  temporal_observation_bias = TRUE,
  value_observation_bias = TRUE,
  noise = FALSE,
  seed
) {
  seed <- get_or_set_seed(seed)
  ...
  indices  <- sample(
    seq_len(n), n - n_samp,
    prob = seq_len(n) * 0 + exp(spatial_weights + value_weights + temporal_weights)
  )
...
observe_polygons <- function(
  ...
  seed <- get_or_set_seed(seed)
  observed_grid  <- observe_gridcells(
    underlying_distribution = underlying_distribution,
    proportion_observed = grid_proportion_observed,
    number_draws = number_draws,
    spatial_observation_bias = grid_spatial_observation_bias,
    temporal_observation_bias = grid_temporal_observation_bias,
  ...
  if (all(is.na(polygon_observation_idx))) {
    n_observed_observations <-
      rbinom(1, nrow(time_censored_observations), polygon_proportion_observed)
    polygon_observation_idx <- sample(
      seq_len(nrow(time_censored_observations)),
      n_observed_observations,
...
create_standardized_test_data <- function(
  ...
  weights = rep(
    list(c(0.3, 1, 1, 1, 1)), ncovariates
  ),
  ...
  test_extent <- create_test_extent()
  ...
  test_covariates <- create_multiple_test_covariates(
    ...
    radiating_polygons = radiating_polygons,
    smoothing_function = smoothing_function,
    radiation_function = radiation_function,
    radiating_means = radiating_means,
    weights = weights,
    magnitude = magnitude,
    family = family,
    seed = .GlobalEnv$.Random.seed
  ...
  test_observed_polygons <- observe_polygons(
    ...
    grid_proportion_observed = grid_proportion_observed,
    number_draws = number_draws,
    grid_spatial_observation_bias = grid_spatial_observation_bias,
    grid_temporal_observation_bias = grid_temporal_observation_bias,
    grid_value_observation_bias = grid_value_observation_bias,
    noise = noise,
    ...
    nonlinear_covariates = nonlinear_covariates,
    min_time_left = min_time_left,
    max_time_right = max_time_right,
    seed = .GlobalEnv$.Random.seed
  )
  rc <- list(
    observed_polygons = test_observed_polygons,
    covariates = test_covariates,
    underlying_distribution_mean = test_underlying_distribution[['mean']],
    underlying_distribution_rcases = test_underlying_distribution[['rcases']],
    raster = test_raster,
    seed = .GlobalEnv$.Random.seed
  )
  attr(rc, "seed") <- seed
