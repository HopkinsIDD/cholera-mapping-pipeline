...
rename_database_fields <- function(database_df, source = "api") {
    ...
    if (source == "api") {
        new_database_df <- database_df %>%
            dplyr::rename(TL = attributes.time_left, TR = attributes.time_right, 
        ...
        new_database_df <- database_df %>%
            dplyr::rename(TL = time_left, TR = time_right, is_primary = primary, 
...
read_taxonomy_data_api <- function(username, api_key, locations = NULL, time_left = NULL, 
    ...
    ## First, we want to set up the https POST request.  We make a list containing the
    ## arguments for the request: If the API changes, we will just need to change this
    ## list
    api_type = ""
    if (is.null(uids)) {
        ...
        https_post_argument_list = list(email = username, api_key = api_key, locations = gsub("::", 
            " ", locations), time_left = time_left, time_right = time_right)
    ...
    ## Every object in R is a vector, even the primitives.  For example, c(1,5,6) is
    ## of type integer.  Because of this, we need to explicitly tell the JSON parser
    ## to treat vectors of length 1 differently.  The option for this is auto_unbox =
    ## T
    json = jsonlite::toJSON(https_post_argument_list, auto_unbox = T)
    ## Message prints a message to the user.  It's somewhere between a warning and a
    ## normal print.  In this case, this function might take a while to run, so we let
    ## the user know up front.
    message("Fetching results from JSON API")

    ## This is the line that actually fetches the results.  The syntax for adding
    ## headers is a little weird.  The function add_headers takes named arguments and
    ## returns whatever the arguments to POST are supposed to be.  body is the body
    ...
    ## This returns something correct, but the formatting is really odd.  It is a
    ## little messy, but instead of debugging the formatting, for now I'm converting
    ## to json and back, which fixes the problems.
    ...
    ## Now we have the results of the api data as a nested list.  We want to do the
    ## following in no particular order for the observations, we want to turn them
    ## into a data frame with one row per observation for the location_periods, we
    ## want to turn them into a geometry object and link them to the observations

    ## We start with the observations The | operator is logical or The results should
    ## have observations The observations should have data The data should be the only
    ## thing in observations
    ...
    ## individually, so we'll loop over location periods to extract the geojsons We
    ...
    ## reduce_sf_vector turns a list of sf objects into a single sf object containing
    ## the same information
    locations_sf = taxdat::reduce_sf_vector(all_locations)
    ## We are going to take our properly formatted geojson files and replace the badly
    ## formatted ones
    ...
    results_data$observations$attributes.location_period_id = as(results_data$observations$attributes.location_period_id, 
        class(results_data$location_periods$id))

    ## We then join (as in sql) by the location_periods with the observations by
    ## location_period_id
    ...
    if (observation_collections_present && (nrow(all_results) > 0) && (nrow(results_data$observation_collections) > 
        ...
        all_results <- dplyr::left_join(results_data$observations, results_data$observation_collections, 
            by = c(relationships.observation_collection.data.id = "id"  # lhs column name = rhs column name
))
    }
    if ((nrow(all_results) > 0) && (nrow(results_data$location_periods) > 0)) {
        all_results = dplyr::left_join(all_results, results_data$location_periods, 
            by = c(attributes.location_period_id = "id"  # lhs column name = rhs column name
))
    }
    ...
    all_results$geojson[!is.na(all_results$sf_id)] = locations_sf$geometry[all_results[!is.na(all_results$sf_id), 
        ][["sf_id"]]]
...
pull_taxonomy_data <- function(username, password, locations = NULL, time_left = NULL, 
    ...
    if (source == "api") {
        ...
        rc <- read_taxonomy_data_api(username = username, api_key = password, locations = locations, 
    ...
    if (nrow(rc) == 0) {
        ...
        stop("Didn't find any data ", err_mssg, " in time range [", ifelse(is.null(time_left), 
            "-Inf", as.character(time_left)), " - ", ifelse(is.null(time_right), 
