...
check_use_pop_weight <- function(use_pop_weight) {
    ...
    if (!tolower(use_pop_weight) %in% c("yes", "no")) {
        if (!tolower(use_pop_weight) %in% c("true", "false")) {
            stop("The use_pop_weight parameter has to be true/false or yes/no. ")
        }
...
get_all_config_options <- function() {
    config_options <- list(name = "no-check", countries = "no-check", countries_name = "no-check",
        aoi = as.function(check_aoi), res_space = as.function(check_res_space), res_time = as.function(check_time_res),
        grid_rand_effects_N = as.function(check_grid_rand_effects_N), case_definition = as.function(check_case_definition),
        start_time = as.function(check_time), end_time = as.function(check_time),
        data_source = as.function(check_data_source), ovrt_metadata_table = as.function(check_ovrt_metadata_table),
        OCs = "no-check", taxonomy = as.function(check_taxonomy), covariate_choices = as.function(check_covariate_choices),
        obs_model = as.function(check_obs_model), od_param = as.function(check_od_param),
        time_effect = "stan-check", time_effect_autocorr = "stan-check", use_intercept = "stan-check",
        covariate_transformations = "no-check", beta_sigma_scale = "stan-check",
        sigma_eta_scale = "stan-check", exp_prior = "stan-check", do_infer_sd_eta = "stan-check",
        do_zerosum_cnst = "stan-check", use_weights = "stan-check", use_rho_prior = "stan-check",
        covar_warmup = "stan-check", warmup = "stan-check", aggregate = as.function(check_aggregate),
        tfrac_thresh = as.function(check_tfrac_thresh), censoring = as.function(check_censoring),
        censoring_thresh = as.function(check_censoring_thresh), set_tfrac = as.function(check_set_tfrac),
        snap_tol = as.function(check_snap_tol), use_pop_weight = as.function(check_use_pop_weight),
        sfrac_thresh = as.function(check_sfrac_thresh), ingest_covariates = as.function(check_ingest_covariates),
        ingest_new_covariates = as.function(check_ingest_covariates), stan = c("ncores",
            "model", "genquant", "niter", "recompile"), file_names = list(output_directory = "output_directory",
            data = "observations_filename", covar = "covariate_filename", stan_input = "stan_input_filename",
            initial_values = "initial_values_filename", stan_output = "stan_output_filename",
            stan_genquant = "stan_genquant_filename", country_data_report_filename = "country_data_report_filename",
