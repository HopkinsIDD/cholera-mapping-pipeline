...
table_modeled_cases_by_chain_time <- function(config, cache, cholera_directory) {
  ...
  cache[["covar_cube_tbl"]] <- cache[["covar_cube"]] %>%
    dplyr::ungroup() %>%
    dplyr::select(t, id)
  cache[["covar_cube_tbl"]][, paste("cases", "chain", seq_len(ncol(cache[["grid_cases_mean_by_chain"]])), sep = "_")] <- cache[["grid_cases_mean_by_chain"]]
  ...
  if (params$drop_nodata_years & !all(cache[["modeled_years"]] %in% cache[["analysis_years"]])) {
    ...
    cache[["covar_cube_tbl"]] <- dplyr::filter(cache[["covar_cube_tbl"]], !(t %in% drop_year_ix))
  }

  sf_grid_wider <- sf::st_drop_geometry(cache[["covar_cube_tbl"]])
