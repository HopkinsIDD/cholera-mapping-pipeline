...
get_obs_stats <- function(df) {
  ...
  rc <- dplyr::summarize(rc,
    n_obs = dplyr::n(), n_cases = sum(suspected_cases),
    n_lp = length(unique(location_period_id)), u_lps = paste(sort(unique(location_period_id)),
      collapse = ","
    ), n_OCs = length(unique(observation_collection_id)), u_OCs = paste(sort(unique(observation_collection_id)),
      collapse = ","
    ),
  )
...
plot_raster_population <- function(covar_data_filename, render = T, n_wrap_col = 5) {
  ...
  plt <- plt + ggplot2::geom_tile(data = covar_cube, ggplot2::aes(
    x = x, y = y,
    fill = population
...
get_data_fidelity <- function(stan_input_filenames, model_output_filenames) {
  ...
  for (i in 1:length(model_output_filenames)) {
    ...
    actual_cases$oc_year <- rep(paste0(
      format(taxdat::read_file_of_type(stan_input_filenames[i], "stan_input")$sf_cases_resized$TL, "%Y"),
      "_",
...
plot_rhat <- function(model.rand, render = T) {
  ...
  rhats <- tibble::tibble(Rhat = round(fit_summary$summary[which(str_detect(
    row.names(fit_summary$summary),
    "modeled_cases"
  )), "Rhat"], 2)) %>%
...
plot_chain_convergence <- function(model_output_filenames, pars = c(
                                     "rho", "betas",
                                     "log_std_dev_w"
                                   ), render = T) {
...
get_spatial_coverage <- function(config, cholera_directory, area_cuts = c(
                                   0, 100,
                                   1000, 10000, Inf
                                 )) {
  ...
  if (!any(is.na(sf_cases$location_name)) | !is.null(taxonomy_username)) {
    ...
    u_lps <- u_lps %>%
      dplyr::ungroup() %>%
      dplyr::mutate(area = sf::st_area(geom), area_class = stringr::str_count(
        location_name,
        "::"
...
get_gam_values <- function(config,
  ...
  predict_df <- tibble::tibble(
    ...
    tibble::as_tibble() %>%
    mutate(
      logpop = log(stan_input$stan_data$pop),
  ...
  y_pred_df <- stan_input$sf_grid %>%
    dplyr::mutate(
      log_y = log_y_pred_mean + predict_df$logoffset,
      y = exp(y),
