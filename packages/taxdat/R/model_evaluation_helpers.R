...
update_filename_oldruns <- function(filename, old_runs = FALSE) {
    if (old_runs) {
        filename[["stan_input"]] <- stringr::str_remove(filename[["stan_input"]],
            "allOCs_")
        filename[["stan_output"]] <- stringr::str_remove(filename[["stan_output"]],
            "allOCs_")
        filename[["stan_output"]] <- stringr::str_remove(filename[["stan_output"]],
            "-csF-teT-teaF-weT-F")
        filename[["stan_output"]] <- stringr::str_remove(filename[["stan_output"]],
...
plot_raw_observed_cases <- function(disjoint_set_sf_cases, render = F, plot_file = NULL,
    ...
    plt <- plt + ggplot2::geom_sf(data = disjoint_set_sf_cases, ggplot2::aes(fill = cases)) +
        ...
        ggplot2::labs(fill = "Average cases by location period") + ggplot2::theme_bw() +
...
plot_area_adjusted_observed_cases <- function(disjoint_set_sf_cases, render = F,
    ...
    plt <- plt + ggplot2::geom_sf(data = disjoint_set_sf_cases, ggplot2::aes(fill = area_adjusted_cases)) +
        ...
        ggplot2::labs(fill = "Area-adjusted cases") + ggplot2::theme_bw() + ggplot2::theme(legend.position = "bottom") +
...
plot_raster_covariates <- function(covar_data_filename, render = T) {
    # plt <- ggplot2::ggplot()
    ...
    if (nrow(sf_grid) == prod(dim(covar_cube[, , 1, drop = F]))) {
        ...
        pltdata_dummy <- tidyr::gather(dplyr::filter(pltdata, t == 1), one_of(dimnames(covar_cube_output$covar_cube)[[3]]),
            key = "covars", value = "value")

        # plt <- plt + ggplot2::geom_sf( data = pltdata_dummy,
        # ggplot2::aes(fill = value, color = value) ) +
        # ggplot2::scale_fill_viridis_c(aesthetics = c('colour', 'fill'), guide
        # = ggplot2::guide_colorbar(title = 'Covariate at time 1'), option =
        # 'B', na.value = 'white') + ggplot2::theme_bw() + #
        ...
        plt <- pltdata_dummy %>%
            ...
            do(gg = {
                ggplot(., ggplot2::aes(fill = value, color = value)) + ggplot2::geom_sf() +
                  ...
                  option = "B", na.value = "white") + ggplot2::theme_bw() + ggplot2::theme(legend.position = "bottom",
...
plot_modeled_cases <- function(case_raster, render = T, plot_file = NULL, width = NULL,
    ...
    case_raster <- case_raster %>%
        ...
        # tidyr::pivot_longer(contains('iterations: Chain'), names_to =
        # 'chain', values_to = 'value') %>%
    dplyr::mutate(chain = stringr::str_replace(chain, "modeled cases", ""))

    plt <- ggplot2::ggplot()
    plt <- plt + ggplot2::geom_sf(data = case_raster, ggplot2::aes(fill = value  #, color =  value
), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + "#,
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + color
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + =
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + value"),
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + color
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + =
        ...
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + +
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + #
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + ggplot2::scale_fill_vidris_c('modeled
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + cases',
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + limits
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + =
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + uniform_scale_fun())
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + +
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + ggplot2::scale_fill_viridis_c(trans
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + =
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + 'log',
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + breaks
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + =
        ...
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + 100,
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + 1000),
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + aesthetics
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + =
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + c('colour',
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + 'fill'),
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + guide
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + =
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + ggplot2::guide_colorbar(title
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + =
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + 'Incidence\n
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + [cases/year]'))
        "#, color =  value"), color = "black", size = 0.05) + # ggplot2::scale_fill_vidris_c('modeled cases', limits = uniform_scale_fun()) +  ggplot2::scale_fill_viridis_c(trans = 'log',                                breaks = c(1, 10, 100, 1000),                                aesthetics = c('colour', 'fill'),                                guide = ggplot2::guide_colorbar(title = 'Incidence\n [cases/year]'))  + +
    taxdat::color_scale(type = "cases", use_case = "ggplot map", use_log = TRUE) +
        ggplot2::labs(fill = "Incidence\n [cases/year]") + ggplot2::theme_bw() +
        ggplot2::theme(legend.position = "bottom") + ggplot2::facet_wrap(~t, ncol = 5) +
        ggplot2::theme(legend.text = ggplot2::element_text(angle = 45, vjust = 1,
            hjust = 1))
...
#' @param height plot height
#' @return ggplot object with modeled cases map
plot_disaggregated_modeled_cases <- function(case_raster, disaggregated_case_sf,
    render = T, plot_file = NULL, width = NULL, height = NULL) {
    ...
    plt_case_raster <- plt_case_raster %>%
        subset(plt_case_raster$t %in% unique(plt_2020_case_raster$t)) %>%
        mutate(value = ifelse(value < exp(-5), exp(-5), value))

    plt <- ggplot2::ggplot()
    plt <- ggplot2::ggplot() + ggplot2::geom_sf(data = plt_case_raster, ggplot2::aes(fill = value),
        color = NA, size = 1e-05) + taxdat::color_scale(type = "cases", use_case = "ggplot map",
        use_log = TRUE) + ggplot2::geom_sf(data = plt_2020_case_raster %>%
        mutate(value = NA), fill = NA, color = "black", size = 0.05) + ggplot2::labs(fill = "Incidence\n [cases/year]") +
        ggplot2::theme_bw() + ggplot2::theme(legend.position = "bottom") + ggplot2::theme(legend.text = ggplot2::element_text(angle = 45,
        vjust = 1, hjust = 1)) + ggplot2::facet_wrap(~t, ncol = length(unique(plt_case_raster$t)))
...
plot_modeled_rates <- function(case_raster, render = T, plot_file = NULL, width = NULL,
    height = NULL) {
    ...
    plt <- ggplot2::ggplot()
    plt <- plt + ggplot2::geom_sf(data = case_raster, ggplot2::aes(fill = value),
        color = "black", size = 0.05) + taxdat::color_scale(type = "rates", use_case = "ggplot map",
        use_log = TRUE) + ggplot2::labs(fill = "Incidence rate\n [cases/100'000/year]") +
...
get_data_fidelity <- function(stan_input_filenames, model_output_filenames) {
    ...
    for (i in 1:length(model_output_filenames)) {
        ...
        # taxdat::read_file_of_type(stan_input_filenames[i],
        # 'stan_input')$sf_cases_resized$OC_UID
        # taxdat::read_file_of_type(stan_input_filenames[i],
        # 'stan_input')$sf_cases_resized$'attributes.fields.suspected_cases'
        # taxdat::read_file_of_type(stan_input_filenames[i],
        # 'stan_input')$stan_data$y
        ...
        actual_cases$oc_uid <- rep(taxdat::read_file_of_type(stan_input_filenames[i],
            "stan_input")$sf_cases_resized$OC_UID, each = nchain)  #newly added
        actual_cases$oc_year <- rep(paste0(format(taxdat::read_file_of_type(stan_input_filenames[i],
            "stan_input")$sf_cases_resized$TL, "%Y"), "_", format(taxdat::read_file_of_type(stan_input_filenames[i],
            "stan_input")$sf_cases_resized$TR, "%Y")), each = nchain)  #newly added

        obs_tfrac = data.frame(obs = initial_values_data$stan_data$map_obs_loctime_obs,
            tfrac = initial_values_data$stan_data$tfrac) %>%
            dplyr::group_by(obs) %>%
            dplyr::summarize(tfrac = sum(tfrac))
        actual_cases$tfrac <- 1
        actual_cases[stringr::str_detect(actual_cases$parameters, "tfrac"), ]$tfrac <- rep(obs_tfrac$tfrac,
            each = nchain)
...
plot_model_fidelity <- function(data_fidelity, case_raster, render = T, by_censoring = F) {
    comparison <- data_fidelity
    rate_raster <- case_raster
    if (!by_censoring) {
        plt <- ggplot2::ggplot(comparison[[1]] %>%
            dplyr::filter(stringr::str_detect(parameters, "tfrac"))) + ggplot2::geom_point(ggplot2::aes(y = `modeled cases`,
            x = `actual cases`, col = chains)) + ggplot2::geom_abline(intercept = 0,
            slope = 1) + ggplot2::coord_fixed(ratio = 1, xlim = c(1, max(comparison[[1]][,
            3:4])), ylim = c(1, max(comparison[[1]][, 3:4]))) + ggplot2::theme_bw()
    } else {
        plt <- ggplot2::ggplot(comparison[[1]] %>%
            dplyr::filter(!stringr::str_detect(parameters, "tfrac"))) + ggplot2::geom_point(ggplot2::aes(y = `modeled cases`,
            x = `actual cases`, col = chains)) + ggplot2::geom_abline(intercept = 0,
...
#' @param render default is TRUE
#' @return ggplot object with modeled vs actual cases by observation
plot_model_fidelity_tfrac_adjusted <- function(data_fidelity, case_raster, render = T) {
    comparison <- data_fidelity
    rate_raster <- case_raster

    plt <- ggplot2::ggplot(comparison[[1]] %>%
        dplyr::filter(!stringr::str_detect(parameters, "tfrac"))) + ggplot2::geom_point(ggplot2::aes(y = `modeled cases`,
        x = `actual cases`, col = oc_uid)) + ggplot2::labs(x = "Actual cases", y = "Modeled cases") +
        ggplot2::geom_abline(intercept = 0, slope = 1) + ggplot2::coord_fixed(ratio = 1,
        xlim = c(0, max(comparison[[1]][, 3:4])), ylim = c(0, max(comparison[[1]][,
            3:4]))) + ggplot2::theme_bw()
...
#' @param render default is TRUE
#' @return ggplot object with modeled vs actual cases by observation
plot_model_fidelity_tfrac_converted <- function(data_fidelity, case_raster, render = T) {
    comparison <- data_fidelity
    rate_raster <- case_raster

    plt <- ggplot2::ggplot(comparison[[1]] %>%
        dplyr::filter(stringr::str_detect(parameters, "tfrac"))) + ggplot2::geom_point(ggplot2::aes(y = `modeled cases`/tfrac,
        x = `actual cases`/tfrac, col = oc_uid)) + ggplot2::labs(x = "Actual cases/tfrac",
        y = "tfrac_modeled_cases/tfrac") + ggplot2::geom_abline(intercept = 0, slope = 1) +
        ggplot2::coord_fixed(ratio = 1, xlim = c(0, max(comparison[[1]][, 3:4])),
            ylim = c(0, max(comparison[[1]][, 3:4]))) + ggplot2::theme_bw()
...
#' @param render default is TRUE
#' @return ggplot object with modeled vs actual cases by observation
plot_model_fidelity_tfrac_adjusted_by_year <- function(data_fidelity, case_raster,
    render = T) {
    comparison <- data_fidelity
    rate_raster <- case_raster
    ...
    plt <- ggplot2::ggplot(comparison[[1]] %>%
        ...
        ggplot2::geom_abline(intercept = 0, slope = 1) + ggplot2::coord_fixed(ratio = 1,
        xlim = c(0, max(comparison[[1]][, 3:4])), ylim = c(0, max(comparison[[1]][,
            3:4]))) + ggplot2::theme_bw() + ggplot2::facet_wrap(~oc_year, ncol = 2)
...
#' @param render default is TRUE
#' @return ggplot object with modeled vs actual cases by observation
plot_model_fidelity_tfrac_unadjusted <- function(data_fidelity, case_raster, render = T) {
    comparison <- data_fidelity
    rate_raster <- case_raster

    plt <- ggplot2::ggplot(comparison[[1]] %>%
        dplyr::filter(!stringr::str_detect(parameters, "tfrac"))) + ggplot2::geom_point(ggplot2::aes(y = `modeled cases`,
        x = `actual cases`, col = oc_uid)) + ggplot2::labs(x = "Actual cases", y = "modeled_cases") +
        ggplot2::geom_abline(intercept = 0, slope = 1) + ggplot2::coord_fixed(ratio = 1,
        xlim = c(0, max(comparison[[1]][, 3:4])), ylim = c(0, max(comparison[[1]][,
            3:4]))) + ggplot2::theme_bw() + ggplot2::facet_wrap(~censoring, ncol = 2)
...
get_spatial_coverage <- function(config, cholera_directory, area_cuts = c(0, 100,
    ...
    if (!any(is.na(sf_cases$location_name)) | !is.null(taxonomy_username)) {
        ...
        # Classify areas by spatial scale based on the LP name (area_class = 0
        # indicates country-level observations)
        u_lps <- u_lps %>%
            dplyr::ungroup() %>%
...
get_gam_values <- function(config, cholera_directory) {
    ...
    y_pred_df <- stan_input$sf_grid %>%
        dplyr::mutate(log_y = log_y_pred_mean + predict_df$logoffset, y = exp(y),
            ...
            sx = predict_df$sx, sy = predict_df$sy)
