...
plot_raster_with_fill <- function(name, color_scale_type, fill_column, cache) {
  ...
  plot <- ggplot2::ggplot() +
    ggplot2::geom_sf(
      data = raster_object, ggplot2::aes_string(fill = fill_column),
      lwd = 0
    ) +
...
plot_modeled_cases_mean_raster <- function(config, cache, cholera_directory) {
  ...
  plot <- plot_raster_with_fill(
    name = "modeled_cases_mean_raster", color_scale_type = "cases",
...
plot_modeled_rates_raster <- function(config, cache, cholera_directory) {
  ...
  plot <- plot_raster_with_fill(
    name = "modeled_rates_mean_raster", color_scale_type = "rates",
...
# cache the results
get_covar_cube <- cache_fun_results(
  name = "covar_cube", fun = get_covar_cube_no_cache,
...
# cache the results
get_raster_object <- cache_fun_results(
  name = "sf_grid", fun = get_raster_object_no_cache,
  overwrite = T, config = config
)
...
merge_modeled_cases_mean_into_raster <- cache_fun_results("modeled_cases_mean_raster",
  merge_modeled_cases_mean_into_raster_no_cache,
  overwrite = T, config
)
...
merge_modeled_rates_mean_into_raster <- cache_fun_results("modeled_rates_mean_raster",
  merge_modeled_rates_mean_into_raster_no_cache,
  overwrite = T, config
)
...
stitch_rate_raster_stack <- cache_fun_results("modeled_rates_raster",
  stitch_rate_raster_stack_no_cache,
  overwrite = T, config_filename
)
...
stitch_case_raster_stack <- cache_fun_results("modeled_cases_raster",
  stitch_case_raster_stack_no_cache,
  overwrite = T, config_filename
)
...
#' @return  cached output
stitch_caches_no_cache <- function(output_cache,
                                   name,
                                   input_caches,
                                   initial_value = list(type = "first", value = NULL),
                                   combination_function = function(x, y, ...) {
                                     return(x + y)
                                   },
                                   ...) {
...
stack_case_rate_raster_in_sf_no_cache <- function(output_cache,
  ...
  modeled_cases_rates_raster <- taxdat::get_case_raster(
...
stack_case_rate_raster_in_sf <- cache_fun_results("modeled_cases",
  stack_case_rate_raster_in_sf_no_cache,
  overwrite = T, config_filename
...
plot_raster_with_fill_and_continent_shp <- function(name, color_scale_type, fill_column, cache, shapefilename,
  ...
  if (customize_color_scale) {
    ...
    plot <- ggplot2::ggplot() +
      ggplot2::geom_sf(data = raster_object, ggplot2::aes_string(fill = fill_column), lwd = 0, color = NA) +
      self_color_scale +
      taxdat::map_theme() +
      ggplot2::facet_wrap(~t) +
      geom_sf(data = shp, lwd = 0.1) +
      coord_sf()
  } else {
    plot <- ggplot2::ggplot() +
      ggplot2::geom_sf(data = raster_object, ggplot2::aes_string(fill = fill_column), lwd = 0, color = NA) +
