...
refresh_materialized_views <- function(psql_connection) {
  queries <- paste("REFRESH MATERIALIZED VIEW", c("grids.resized_spatial_grid_polygons", "grids.resized_spatial_grid_centroids", "location_period_raster_map"))
...
destroy_testing_database <- function(psql_connection) {
  drop_query <- c(
    ...
    "DROP MATERIALIZED VIEW IF EXISTS grids.resized_spatial_grid_polygons",
  ...
  sapply(drop_query, function(query){DBI::dbClearResult(DBI::dbSendQuery(conn = psql_connection, query))})
}

insert_testing_locations <- function(psql_connection, location_df) {
  assert(
    "qualified_name" %in% names(location_df),
    "insert_testing_locations cannot insert a location without a qualified name"
  )
  insert_query <- paste(
    "INSERT INTO locations(\"qualified_name\") VALUES",
    paste(
      "(",
...
insert_testing_location_periods <- function(psql_connection, location_period_df) {
  assert(
    "qualified_name" %in% names(location_period_df),
    "insert_testing_location_periods cannot insert a location_period without a qualified name"
  )
  assert(
    "start_date" %in% names(location_period_df),
    "insert_testing_location_periods cannot insert a location_period without a start date"
  )
  assert(
    "end_date" %in% names(location_period_df),
    "insert_testing_location_periods cannot insert a location_period without a end date"
  )

  location_period_df$location_id <- DBI::dbGetQuery(
    conn = psql_connection,
    glue::glue_sql(
      .con = psql_connection,
      "SELECT id FROM locations where qualified_name in ({location_period_df[[\"qualified_name\"]]*})"
    )
  )[["id"]]
  ...
  insert_query <- paste(
    ...
    paste(
      "(",
      glue::glue_sql(.con = psql_connection, "{location_period_df[['location_id']]}"),
      ",",
...
insert_testing_shapefiles <- function(psql_connection, shape_df, create_location_periods = FALSE) {
  assert(
    "qualified_name" %in% names(shape_df),
    "insert_testing_shapefiles cannot insert a shape without a qualified name"
  )
  assert(
    "start_date" %in% names(shape_df),
    "insert_testing_shapefiles cannot insert a shape without a start date"
  )
  assert(
    "end_date" %in% names(shape_df),
    "insert_testing_shapefiles cannot insert a shape without an end date"
  )
  assert(
    "geom" %in% names(shape_df),
    "insert_testing_shapefiles cannot insert a shape without a geometry"
  )
  ...
  shape_df$location_id <- DBI::dbGetQuery(
    conn = psql_connection,
    glue::glue_sql(
      .con = psql_connection,
      "SELECT id FROM locations where qualified_name in ({shape_df[[\"qualified_name\"]]*})"
    )
  )[["id"]]

  location_period_query <- paste(
    "SELECT id FROM location_periods WHERE (\"location_id\", \"start_date\", \"end_date\") IN (",
    paste(
      "(",
      glue::glue_sql(.con = psql_connection, "{shape_df[['location_id']]}"),
      ",",
  ...
  insert_query <- paste(
    "INSERT INTO shapes(\"location_period_id\", \"shape\", \"box\") VALUES",
    paste(
      "(",
      glue::glue_sql(.con = psql_connection, "{shape_df[['location_period_id']]}"),
      ",",
...
insert_testing_observations <- function(psql_connection, observation_df) {
  assert(
    "observation_collection_id" %in% names(observation_df),
    "insert_testing_observations cannot insert a shape without a observation_collection_id"
  )
  assert(
    "phantom" %in% names(observation_df),
    "insert_testing_observations cannot insert a shape without a phantom"
  )
  assert(
    "primary" %in% names(observation_df),
    "insert_testing_observations cannot insert a shape without a primary"
  )
  assert(
    "qualified_name" %in% names(observation_df),
    "insert_testing_observations cannot insert a shape without a qualified name"
  )
  assert(
    "time_left" %in% names(observation_df),
    "insert_testing_observations cannot insert a shape without a time left"
  )
  assert(
    "time_right" %in% names(observation_df),
    "insert_testing_observations cannot insert a shape without a time right"
  )
  assert(
  ...
  observation_df$location_id <- DBI::dbGetQuery(
    conn = psql_connection,
    glue::glue_sql(
      .con = psql_connection,
      "SELECT id FROM locations where qualified_name in ({observation_df[[\"qualified_name\"]]*})"
    )
  )[["id"]]
  ...
  observation_df$location_period_id <- DBI::dbGetQuery(conn = psql_connection, location_period_query)[["id"]]
  ...
  insert_query <- paste(
    ...
    paste(
      "(",
      glue::glue_sql(.con = psql_connection, "{observation_df[['observation_collection_id']]}"),
      ",",
