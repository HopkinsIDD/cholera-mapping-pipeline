...
#' @export 
get_stan_parameters <- function(config, sigma_eta_scale = 5, beta_sigma_scale = 10, 
    warmup = T, covar_warmup = T, time_effect = F, time_effect_autocorr = F, censoring = F, 
    use_weights = T, overdispersion = NA) {
    ...
    default_params <- list(sigma_eta_scale = sigma_eta_scale, beta_sigma_scale = beta_sigma_scale, 
        ...
        time_effect_autocorr = time_effect_autocorr, censoring = censoring, use_weights = use_weights, 
