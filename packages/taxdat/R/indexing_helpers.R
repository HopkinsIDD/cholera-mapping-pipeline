    ...
    for (idx in rev(seq_len(length(n_factors)))) {
        if (skip_next) {
            ...
            if (a_ok & b_ok) {
                ...
                stop(paste(n, "cannot be split into a number that divides", a, "and a number that divides", 
                  b))
            }
        }
    }
...
array_to_adjacency <- function(arr, sparse = TRUE) {
    ...
    for (idx in seq_len(length(dim(arr)))) {
        ...
        tmp_elements[[paste("dimension", idx, sep = "_")]] <- ifelse(tmp_elements[[paste("dimension", 
            idx, sep = "_")]] + 1 <= dim(arr)[[idx]], tmp_elements[[paste("dimension", 
            idx, sep = "_")]] + 1, NA)
        all_elements[[paste("neighbor", idx, sep = "+")]] <- convert_index(dim(arr), 
            tmp_elements)

        tmp_elements <- all_elements[, seq_len(length(dim(arr)))]
        tmp_elements[[paste("dimension", idx, sep = "_")]] <- ifelse(tmp_elements[[paste("dimension", 
            idx, sep = "_")]] - 1 > 0, tmp_elements[[paste("dimension", idx, sep = "_")]] - 
            1, NA)
        all_elements[[paste("neighbor", idx, sep = "-")]] <- convert_index(dim(arr), 
            tmp_elements)
    }
