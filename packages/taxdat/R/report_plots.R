...
plot_true_modeled_grid_cases <- function(cache, cholera_directory, config) {
  ...
  if (all(unlist(lapply(cache[["config"]][["test_metadata"]][["observations"]], function(x) {
    return(x[["proportion_observed"]] == 1)
  })))) {
...
#' @name automated_period
#' @description function to specify time facet labels
automated_period <- function(num, units) {
  list_period <- paste(num, units)
  lubridate::period(list_period)
}
...
plot_w_mean<-function(cache,config,cholera_directory){
  ...
  w_data = cache[['cmdstan_fit']]$draws(variables = 'w') %>%
    reshape2::melt()
  w_mean = w_data %>% group_by(variable) %>% summarise(mean = mean(value))#QZ: mean across chain and iterations
