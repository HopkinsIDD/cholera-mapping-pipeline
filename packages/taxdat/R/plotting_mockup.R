...
plot_sf_with_fill <- function(sf_object, color_scale, fill_column) {
    plot <- ggplot2::ggplot() + ggplot2::geom_sf(data = sf_object, ggplot2::aes_(fill = rlang::sym(fill_column))) + 
        taxdat::color_scale(type = color_scale, use_case = "ggplot map") + taxdat::map_theme() + 
        ggplot2::facet_wrap(~set)
    return(plot)
}

plot_observed_cases_polygon_raw <- function(config, cache = new.env()) {
    get_observed_polygon_cases_disjoint_aggregated(config, cache)
    plot_sf_with_fill(sf_object = cache[["observed_polygon_cases_disjoint_aggregated"]], 
        color_scale = "cases", fill_column = rlang::sym("suspected_cases"))
}

plot_modeled_cases_polygon_raw <- function(config, cache = new.env()) {
    get_observed_polygon_cases_disjoint(config, cache)
    plot <- ggplot2::ggplot() + ggplot2::geom_sf(data = cache[["observed_polygon_cases_disjoint"]], 
        ggplot2::aes(fill = suspected_cases)) + taxdat::color_scale(type = "cases", 
...
cache_fun_results <- function(name, fun, ..., overwrite = FALSE) {
    ...
    return(function(config, cache = new.env()) {
...
get_observation_data_no_cache <- function(config, cache) {
    ...
    return(cache[["stan_input"]][["observation_data"]])
}
get_observation_data <- cache_fun_results("observation_data", get_observation_data_no_cache)
...
get_observed_polygon_cases_disjoint_no_cache <- function(config, cache) {
    get_stan_input(config, cache)
    get_observation_data(config, cache)
    separate_by_overlap(cache[["observation_data"]], name_column = "location_period_id") %>%
        return()
}
...
aggregate_to_location_period <- function(sf_object, aggregation_function, grouping_columns = "location_period_id", 
    ...
    sf_object %>%
        ...
        dplyr::group_modify(function(.x, .y) {
            rc <- sf::st_sf(x = aggregation_function(cases = .x[[case_column]], time_left = .x[["time_left"]], 
                time_right = .x[["time_right"]]), geom = sf::st_geometry(.x)[1])
...
get_observed_polygon_cases_disjoint_aggregated_no_cache <- function(config, cache) {
    get_observed_polygon_cases_disjoint(config, cache)
    aggregate_to_location_period(cache[["observed_polygon_cases_disjoint"]], aggregation_function = function(...) {
        mean(normalize_cases_by_time(...))
    }, grouping_columns = c("location_period_id", "set"), case_column = "suspected_cases") %>%
        return()
}
...
## Unused but also examples
get_model.rand_no_cache <- function(config, cache) {
    load(config[["file_names"]][["stan_output"]])
    return(model.rand)
}
get_model.rand <- cache_fun_results("model.rand", get_model.rand_no_cache)

get_grid_cases_no_cache <- function(config, cache) {
    get_model.rand(config, cache)
    return(MCMCvis::MCMCchains(cache[["model.rand"]], params = "grid_cases"))
}
get_grid_cases <- cache_fun_results("grid_cases", get_grid_cases_no_cache)
