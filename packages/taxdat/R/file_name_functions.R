...
make_grid_filename <- function(cholera_directory, map_name, config = NULL) {
  if (!is.null(config)) {
    if ("file_names" %in% names(config)) {
      ...
      if ("observations_filename" %in% names(config[["file_names"]])) {
        return(paste(filename, config[["file_names"]][["observations_filename"]],
          sep = "/"
        ))
      }
    }
  }
  paste(cholera_directory, "/Analysis/", "data/", map_name, ".grid", ".rdata",
    sep = ""
  )
}

make_observations_filename <- function(cholera_directory, map_name, config = NULL) {
  if (!is.null(config)) {
    if ("file_names" %in% names(config)) {
      filename <- paste(cholera_directory, "Analysis", "data", sep = "/")
      if ("output_directory" %in% names(config[["file_names"]])) {
  ...
  paste(cholera_directory, "/Analysis/", "data/", map_name, ".preprocess", ".rdata",
    sep = ""
  )
}
...
make_covar_filename <- function(cholera_directory, map_name, covariate_name_part,
                                config = NULL) {
  if (!is.null(config)) {
    if ("file_names" %in% names(config)) {
      filename <- paste(cholera_directory, "Analysis", "data", sep = "/")
      ...
      if ("covariate_filename" %in% names(config[["file_names"]])) {
        return(paste(filename, config[["file_names"]][["covariate_filename"]],
          sep = "/"
        ))
      }
    }
  }
  paste(cholera_directory, "/Analysis/", "data/", map_name, ".", covariate_name_part,
    ".covar", ".rdata",
    sep = ""
  )
}
...
make_stan_input_filename <- function(cholera_directory, map_name, covariate_name_part,
                                     config, config_dict) {
  if ("file_names" %in% names(config)) {
    filename <- paste(cholera_directory, "Analysis", "data", sep = "/")
    if ("output_directory" %in% names(config[["file_names"]])) {
    ...
    if ("stan_input_filename" %in% names(config[["file_names"]])) {
      return(paste(filename, config[["file_names"]][["stan_input_filename"]],
        sep = "/"
      ))
    }
  }
...
make_initial_values_filename <- function(cholera_directory, map_name, covariate_name_part,
                                         config, config_dict) {
  if ("file_names" %in% names(config)) {
    filename <- paste(cholera_directory, "Analysis", "data", sep = "/")
    if ("output_directory" %in% names(config[["file_names"]])) {
  ...
  if ("file_names" %in% names(config)) {
    ...
    if ("stan_output_filename" %in% names(config[["file_names"]])) {
      return(paste(filename, config[["file_names"]][["stan_output_filename"]],
        sep = "/"
      ))
    }
  }
  ...
  base_filename <- make_stan_input_filename(
    cholera_directory, map_name, covariate_name_part,
    config, config_dict
  )
...
make_stan_output_filename <- function(cholera_directory, map_name, covariate_name_part,
                                      config, config_dict) {
  if ("file_names" %in% names(config)) {
    filename <- paste(cholera_directory, "Analysis", "data", sep = "/")
    if ("output_directory" %in% names(config[["file_names"]])) {
  ...
  # Get stan input filename
  base_filename <- make_initial_values_filename(
    cholera_directory, map_name, covariate_name_part,
    config, config_dict
  )
  # Base part of output filename
  ...
  paste0(base_filename, to_add, ".stan_output.rds")
}
...
make_map_output_filename <- function(cholera_directory, map_name, covariate_name_part,
                                     stan_model, niter) {
  paste(cholera_directory, "/Analysis/", "output/", map_name, ".", covariate_name_part,
    ".", stan_model, ".", niter, ".pdf",
    sep = ""
  )
}
...
make_map_name <- function(config, .f = NULL) {
  ...
  map_name <- paste(paste(config$countries_name, collapse = "-"), stringr::str_replace(
    res_time,
    " ", "_"
  ), paste(start_time, end_time, sep = "-"), paste(res_space, "km",
    sep = ""
...
get_filenames <- function(config, cholera_directory) {
  ...
  if ("file_names" %in% names(config)) {
    file_names <- setNames(
      paste(cholera_directory, config$file_names, sep = "/"),
      names(config$file_names)
    )
    relative_files_exist <- sapply(file_names, file.exists)
    absolute_files_exist <- sapply(config$file_names, file.exists)
    if (all(relative_files_exist)) {
      return(sapply(file_names, normalizePath))
    }
    if (all(absolute_files_exist)) {
      return(sapply(config$file_names, normalizePath))
    }
    warning("Not all file paths exist")
    return(sapply(config$file_names, normalizePath))
  }
  ...
  covariate_choices <- check_covariate_choices(
  ...
  preprocessed_grid_fname <- make_observations_filename(
    cholera_directory = cholera_directory,
    map_name = map_name
  )

  preprocessed_data_fname <- make_observations_filename(
    cholera_directory = cholera_directory,
    map_name = map_name
  )

  preprocessed_covar_fname <- make_covar_filename(
    cholera_directory = cholera_directory,
    map_name = map_name, covariate_name_part = covariate_name_part
  )

  stan_input_fname <- make_stan_input_filename(
    cholera_directory = cholera_directory,
    map_name = map_name, covariate_name_part = covariate_name_part, config = config,
    config_dict = config_dict
  )

  initial_values_fname <- make_initial_values_filename(
    cholera_directory = cholera_directory,
    map_name = map_name, covariate_name_part = covariate_name_part, config = config,
    config_dict = config_dict
  )

  stan_output_fname <- make_stan_output_filename(
    cholera_directory = cholera_directory,
    map_name = map_name, covariate_name_part = covariate_name_part, config = config,
    config_dict = config_dict
  )
  ...
  rc <- list(
    grid = preprocessed_grid_fname, data = preprocessed_data_fname, covar = preprocessed_covar_fname,
    stan_input = stan_input_fname, initial_values = initial_values_fname, stan_output = stan_output_fname
  )
  return(rc)
}
...
add_explicit_file_names_to_config <- function(config_path, cholera_directory) {
  ...
  if (length(default_file_dir) != 1) {
    stop("All output files expected in a single directory")
  }
