...
automate_mapping_config <- function(p, covariate_names, countries_names, countries_ids) {
    ...
    map_name <- paste0(paste(countries_names, collapse = "_"), "_", lubridate::year(as.Date(p$start_time)),
        "_", lubridate::year(as.Date(p$end_time)), "_", p$res_space, "km")

    cat(paste0("name: '", map_name, "'\n", "countries: ['", paste(countries_ids,
        ...
        "end_time: '", as.Date(p$end_time), "'\n", "covariate_choices: ['", paste(covariate_names,
