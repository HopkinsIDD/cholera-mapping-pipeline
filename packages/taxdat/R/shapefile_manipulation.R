...
is_shapefile_valid <- function(shapefiles, location_column_name = "id", quiet = FALSE) {
    ...
    if (!all(shapefiles$valid)) {
        ...
        rc$reason[!shapefiles$valid] <- paste(rc$reason[!shapefiles$valid], "Shapefile has topology error.")
        sf::st_geometry(rc)[!shapefiles$valid] <- sf::st_make_valid(sf::st_geometry(shapefiles)[!shapefiles$valid])
        shapefiles$valid[!shapefiles$valid] <- sf::st_is_valid(rc[!shapefiles$valid])
        rc$reason[!shapefiles$valid] <- paste(rc$reason[!shapefiles$valid], "Automatic fix failed to fix topology error.")
    }
    ...
    if (any(!shapefiles$valid)) {
        if (!quiet) {
            ...
            print(paste("The following location periods are affected:", paste(shapefiles[!shapefiles$valid, 
                ][[location_column_name]], collapse = ", ")))
        }

        rc$valid[!shapefiles$valid] <- FALSE
        rc$reason[!shapefiles$valid] <- paste(rc$reason[!shapefiles$valid], "Shapefile is not of appropriate dimension:", 
            sf::st_dimension(shapefiles), "vs 2")
    }
    shapefiles$valid <- !grepl("GEOMETRYCOLLECTION", sf::st_geometry_type(shapefiles))
    ## This is not a long term solution in any capacity
    if (any(!shapefiles$valid)) {
        if (!quiet) {
            warning("Geometry collections present in locations.  See output for details")
            print(paste("The following location periods are affected:", paste(shapefiles[!shapefiles$valid][[location_column_name]], 
                collapse = ", ")))
        }
        rc$valid[!shapefiles$valid] <- FALSE
        rc$reason[!shapefiles$valid] <- paste(rc$reason[!shapefiles$valid], "Shapefile is a geometry collection")
        if (!quiet) {
        }
        tmp2 <- do.call(sf:::rbind.sf, lapply(sf::st_geometry(shapefiles)[!shapefiles$valid], 
            function(x) {
...
create_table_from_sf <- function(df, user_name, table_name, overwrite = TRUE, dbname) {
    ...
    index_query <- paste("CREATE INDEX {table_index_name} ON {table_name}", "USING GIST({geometry_name})")
    vacuum_query <- "VACUUM ANALYZE {table_name}"
    conn_pg <- taxdat::connect_to_db(user_name, dbname)
    if (overwrite) {
        DBI::dbClearResult(DBI::dbSendQuery(conn_pg, glue::glue_sql(drop_query, .con = conn_pg)))
    }
