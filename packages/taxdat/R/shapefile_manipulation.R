...
is_shapefile_valid <- function(shapefiles, location_column_name = "id", quiet = FALSE) {
  ...
  if (any(!shapefiles$valid)) {
    if (!quiet) {
      ...
      print(paste("The following location periods are affected:", paste(shapefiles[!shapefiles$valid, ][[location_column_name]], collapse = ", ")))
    }

    rc$valid[!shapefiles$valid] <- FALSE
    rc$reason[!shapefiles$valid] <- paste(
      rc$reason[!shapefiles$valid], "Shapefile is not of appropriate dimension:",
  ...
  if (any(!shapefiles$valid)) {
    ...
    tmp2 <- do.call(sf:::rbind.sf, lapply(
