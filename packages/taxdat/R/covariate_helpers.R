...
generate_time_sequence <- function(dates,
  ...
  if (stringr::str_detect(res_time, "year") | (stringr::str_detect(res_time, "month") & res_time_source$dt_units > 11)) {
...
time_aggregate <- function(src_file,
  ...
  if (!is.null(aoi_extent)) {
    ...
    cat("Croping ", src_file, " to ",
        stringr::str_c(c("[xmin:", ", xmax:", ", ymin:", ", ymax:"), as.vector(aoi_extent)),
        "]\n", sep = "")
  ...
  if (covar_type == "temporal") {
    ...
    if (res_time_source$dt_units >= res_time_list$k) {
      ...
      eval(
        parse(text = stringr::str_c("agg_fun <- function(x) {return(", aggregator, "(x))}"))
      )
...
ingest_covariate <- function(conn,
  ...
  doFun(
                     ...
                     .packages = c("dplyr", "stringr", "raster", "gdalUtils")),
    {
      ...
      res_file_space <- stringr::str_replace(
        res_file_time, "\\.nc",
        stringr::str_c("__resampled_", res_x, "x", res_y, "km.nc")) %>%
        { str <- .
      ...
      if (write_to_db) {
        if (j == 1) {
          ...
          r2psql_cmd <- stringr::str_c("raster2pgsql -s 4326:4326 -I -t auto -d ",
                                       res_file_space,
                                       " tmprast | psql -d cholera_covariates",
                                       sep = " ")
          err <- system(r2psql_cmd)
          if (err != 0) {
            stop(paste("System command", r2psql_cmd, "failed"))
  ...
  DBI::dbGetQuery(conn, glue::glue_sql(
    "SELECT AddRasterConstraints({covar_schema}::name, {covar_alias}::name,
      'rast'::name);", .con = conn))
  DBI::dbClearResult(DBI::dbSendStatement(conn, glue::glue_sql("VACUUM ANALYZE {`DBI::SQL(covar_table)`};", .con = conn)))

  t_end_covar <- Sys.time()

  cat("---- Done", action, stringr::str_to_upper(covar_name), "(took",
...
write_pg_raster <- function(dbuser,
                            ...
                            outfile,
                            band = 1) {
  dsn <- glue::glue("PG:dbname='cholera_covariates'",
...
gdalwarp2 <- function(
  ...
  cmd <- gdal_cmd_builder2(executable = executable, parameter_variables = parameter_variables,
                           ...
                           gdal_installation_id = gdalUtils::gdal_chooseInstallation(hasDrivers = of))
  if (verbose)
    message(paste("GDAL command being used:", cmd))
  cmd_output <- system(cmd, intern = TRUE)
  if (output_raster) {
