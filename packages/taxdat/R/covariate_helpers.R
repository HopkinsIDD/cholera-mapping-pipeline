...
is_connection_to_old_pipeline <- function(psql_connection) {
  check_query <- "select * from information_schema.tables where table_name = 'grid_1_1'"
  return(nrow(DBI::dbGetQuery(conn = psql_connection, check_query)) > 0)
}
...
make_covar_alias <- function(alias, type, res_time, res_space) {
  ...
  covar_alias <- stringr::str_c(alias, ifelse(type == "temporal", stringr::str_replace(
    res_time,
    " ", "_"
...
#' @param map_name A string representing a somewhat unique name for this run
#' @export
clean_all_tmp <- function(dbuser, map_name, dbname) {
  print("-- Cleaning temporary tables")
  conn <- connect_to_db(dbuser, dbname)
...
extract_covariate_metadata <- function(covar_alias, covar_file, covar_type) {
  if (covar_type == "temporal") {
    ...
    # by the dataset
    date_vec <- as.Date(time_vec, origin = as.Date(stringr::str_extract(
      time_att$units,
    ...
    covar_metadata <- data.frame(
      covariate = covar_alias, src_res_x = res_x,
...
get_ncdf_metadata <- function(src_file) {
  ...
  return(list(
    time_info = r_time_info, var_name = var_name, var_att = var_att,
...
transform_raster <- function(res_file, transform) {
  ...
  eval(parse(text = stringr::str_c(
    "trans_fun <- function(x) {return(", transform,
    "(x))}"
  )))
  # transform data
  tryCatch(
...
sum_nonNA <- function(conn, r_file, ref_grid_db, dbuser, dbname) {
  ...
  align_rasters2(
    unaligned = tmp_file1, reference = ref_grid_db, dstfile = tmp_file,
...
write_ncdf <- function(data, res_file, mv = -9999, var_name, long_var_name, var_unit,
  ...
  if (!is.null(time_vals)) {
    time <- ncdf4::ncdim_def(
      name = "time", units = time_units, vals = time_vals,
  ...
  var_data <- ncdf4::ncvar_def(
    name = var_name, units = var_unit, dim = dim, longname = long_var_name,
  ...
  # Place the precip and tmax values in the file need to loop through the
  # layers to get them to match to correct time index Make me configurable
  ...
  for (layer_idx in seq_len(raster::nlayers(data))) {
    ...
    for (row_idx in seq_len(nchunk_row)) {
      ...
      for (col_idx in seq_len(nchunk_col)) {
        cat(
          "Processing layer ", layer_idx, " of ", raster::nlayers(data),
          "\n"
        )
        ...
        if (is.null(time_vals)) {
          local_dataslice <- raster::getValues(local_data[chunk_row_start:chunk_row_end,
            chunk_col_start:chunk_col_end, layer_idx,
            drop = FALSE
          ], format = "matrix")
          ncdf4::ncvar_put(
            nc = ncout, varid = var_data, vals = t(local_dataslice)[
              ,
              rev(seq_len(n_row))
          ...
          ncdf4::ncvar_put(nc = ncout, varid = var_data, vals = t(local_dataslice)[
            ,
            rev(seq_len(n_row))
          ], start = c(
            chunk_col_start, mat_row_start,
            layer_idx
          ), count = c(n_col, n_row, 1), verbose = TRUE)
        }
      }
    }
  }
...
get_time_res <- function(dates, covar_res_time, units) {
  if (length(dates) < 2) {
    dt_days <- ifelse(stringr::str_detect(covar_res_time, "day"), 1, ifelse(stringr::str_detect(
      covar_res_time,
      "month"
    ), 30, 365))
  } else {
...
generate_time_sequence <- function(dates, res_time_source, res_time) {
  ...
  # Generate the sequence of dates These should correspond to the left bounds
  # of each time period
...
time_aggregate <- function(src_file, covar_name, covar_unit, covar_type, covar_res_time,
  ...
  if (!is.null(aoi_extent)) {
    cat("Croping ", src_file, " to ", stringr::str_c(c(
      "[xmin:", ", xmax:", ", ymin:",
      ", ymax:"
    ), as.vector(aoi_extent)), "]\n", sep = "")
  ...
  if (covar_type == "temporal") {
    if (length(r_metadata$dates) < 2) {
      # stop('Covariate', covar_name, 'has only one temporal layer, do
      # not know how to process it')

      # Get the source's temporal resolution
      res_time_source <- get_time_res(
        dates = r_metadata$dates, covar_res_time = covar_res_time,
        units = res_time_list$units
      )
    }

    # Check whether the source's temporal resolution is coarser than the
    # required resolution
    if (res_time_source$dt_units > res_time_list$k) {
      ...
      r_covar_dates <- generate_time_sequence(
        dates = r_metadata$dates, res_time_source = res_time_source,
        res_time = res_time
      )
      # Unpack result
      ...
      eval(parse(text = stringr::str_c(
        "agg_fun <- function(x) {return(", aggregator,
        "(x))}"
      )))
    ...
    var_unit <- stringr::str_c(
      r_metadata$var_att$units, " aggregated at [time resolution:",
    ...
    write_ncdf(
      data = r_out, res_file = res_file, mv = -9999, var_name = r_metadata$var_name,
    ...
    res_file <- stringr::str_replace_all(res_file, stringr::str_replace(
      res_time,
      " ", "-"
    ), "")
    if (!file.exists(res_file)) {
      write_ncdf(
        data = r, res_file = res_file, mv = -9999, var_name = covar_name,
...
space_aggregate <- function(src_file, res_file, ref_grid_db, covar_type, aggregator,
  ...
  # Resample the raster to the reference grid
  align_rasters2(
    unaligned = src_file, reference = ref_grid_db, dstfile = tmp_file,
    projres_only = F, r = aggregator, nThreads = 8, overwrite = T, verbose = F
  )

  if (do_sum) {
    # Aggregation with sums is not implemented in gdal: instead compute the
    # average and then multiply by the number of population-lvel cells
    # covered by each modeling grid cell
    r <- raster::stack(tmp_file)
    # Compute the number of non-na cells covered by each modeling cell
    r_nval <- sum_nonNA(conn, tmp_file, ref_grid_db, dbuser, dbname)
  ...
  raster::writeRaster(r, res_file,
    overwrite = TRUE, NAflag = -9999, format = "CDF",
...
ingest_covariate <- function(conn, covar_name, covar_alias, covar_dir, covar_unit,
  ...
  cat("---- ", paste(toupper(substr(action, 1, 1)), substr(action, 2, nchar(action)),
    sep = ""
  ), " ", stringr::str_to_upper(covar_name), " at resolution [", res_time,
  ...
  proc_dir <- stringr::str_c(
    path_to_cholera_covariates, "/processed_covariates/",
    covar_name, "/", aoi_name, "/"
  )
  ...
  if (do_parallel) {
    ...
    parallel::clusterExport(cl = cl, list(
      "connectToDB", "dbuser", "getTimeRes",
      "generateTimeSequence", "writeNCDF"
    ), envir = environment())
  ...
  export_funs <- c(
    "extractCovariateMetadata", "parse_gdal_res", "parseTimeRes",
  ...
  if (do_parallel) {
    doFun(foreach::foreach(
      j = seq_along(raster_files), .combine = rbind, .inorder = T,
      .export = export_funs, .noexport = no_export, .packages = c(
        "dplyr",
      ...
      res_file_time <- stringr::str_c(proc_dir, stringr::str_replace(
        f_name,
        stringr::str_c(".", f_format), stringr::str_c(
          "__", time_aggregator,
      ...
      if (!file.exists(res_file_time)) {
        ...
        res_file_time <- time_aggregate(
          src_file = full_path, covar_name = covar_name,
      ...
      res_file_space <- stringr::str_replace(res_file_time, "\\.nc", stringr::str_c(
        "__resampled_",
        res_x, "x", res_y, "km.nc"
      )) %>%
        {
          ...
          if (!is.null(transform)) {
            stringr::str_replace(str, "\\.nc", stringr::str_c(
              "_", transform,
              "_trans.nc"
            ))
          } else {
      ...
      if (!file.exists(res_file_space)) {
        ...
        space_aggregate(res_file_time,
          res_file = res_file_space, ref_grid_db = ref_grid_db,
        ...
        if (!is.null(transform)) {
          cat(
            "Transforming", f_name, "according to ||", deparse(transform),
            "|| \n"
          )
      ...
      if (write_to_db) {
        if (j == 1) {
          ...
          r2psql_cmd <- stringr::str_c("raster2pgsql -s 4326:4326 -I -t auto -d ",
            res_file_space, covar_table, "| psql -d ", dbname,
            sep = " "
          )
          ...
          # Write to database
          r2psql_cmd <- stringr::str_c("raster2pgsql -s 4326:4326 -I -t auto -d ",
            res_file_space, " tmprast | psql -d", dbname,
            sep = " "
          )
          err <- system(r2psql_cmd)
          if (err != 0) {
    ...
    res_file_time <- stringr::str_c(proc_dir, stringr::str_replace(f_name, stringr::str_c(
      ".",
      f_format
    ), stringr::str_c("__", time_aggregator, "_", stringr::str_replace(
      res_time,
      " ", "-"
    ), ".nc")))
...
write_metadata <- function(conn, covar_dir, covar_type, covar_alias, res_x, res_y,
  ...
  covar_metadata <- foreach::`%do%`(foreach::foreach(
    full_path = raster_files,
  ...
  covar_metadata <- covar_metadata %>%
    ...
    dplyr::summarise(
      src_res_x = src_res_x[1], src_res_y = src_res_y[1], src_res_time = src_res_time[1],
      first_TL = min(first_TL), last_TL = max(last_TL)
    ) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(
      src_dir = covar_dir, res_x = res_x, res_y = res_y, res_time = ifelse(covar_type ==
  ...
  DBI::dbWriteTable(
    conn = conn, name = tmp_name, covar_metadata, row.names = F,
...
#'
#' @export
write_pg_raster <- function(dbuser, schema, table, outfile, dbname, band = 1) {
  dsn <- glue::glue("PG:dbname='{dbname}'", " user='{dbuser}' port=5432", " schema='{schema}' table='{table}' mode=2")
...
gdalinfo2 <- function(datasetname, json, mm, stats, approx_stats, hist, nogcp, nomd,
  ...
  parameter_variables <- list(
    logical = list(varnames <- c(
      "json", "mm", "stats",
      "approx_stats", "hist", "nogcp", "nomd", "nrat", "noct", "checksum", "nofl",
      "proj4", "version", "formats"
    )), vector = list(varnames <- c()), scalar = list(varnames <- c("sd")),
    character = list(varnames <- c(
      "datasetname", "mdd", "format", "optfile",
      "config", "debug", "oo"
    )), repeatable = list(varnames <- NULL)
  )
  parameter_order <- c(
    "json", "mm", "stats", "approx_stats", "hist", "nogcp",
  ...
  parameter_doubledash <- c(
    "version", "formats", "format", "optfile", "config",
    "debug"
  )
  executable <- "gdalinfo"
  cmd <- gdal_cmd_builder2(
    executable = executable, parameter_noquotes = "datasetname",
  ...
  if (raw_output) {
    ...
    orig <- as.numeric(strsplit(
      gsub(strsplit(grep(cmd_output,
        pattern = "Lower Left  \\(",
        value = TRUE
      ), "\\) \\(")[[1]][1], pattern = "Lower Left  \\(", replacement = ""),
      ","
    )[[1]])
    result$ll.x <- orig[1]
    result$ll.y <- orig[2]
    ...
    if (!missing(proj4)) {
      if (proj4) {
        result$proj4 <- sub("\\s+$", "", gsub("'", "", cmd_output[grep(
          pattern = "PROJ.4 string is:",
          cmd_output
        ) + 1]))
      }
    }
    result$driver <- strsplit(gsub(grep(cmd_output, pattern = "Driver: ", value = TRUE),
      pattern = "Driver: ", replacement = ""
    ), "/")[[1]][1]
    ul <- as.numeric(strsplit(strsplit(strsplit(cmd_output[grep(
      pattern = utils::glob2rx("Upper Left*"),
      cmd_output
    )], "\\(")[[1]][2], "\\)")[[1]][1], ",")[[1]])
    ll <- as.numeric(strsplit(strsplit(strsplit(cmd_output[grep(
      pattern = utils::glob2rx("Lower Left*"),
      cmd_output
    )], "\\(")[[1]][2], "\\)")[[1]][1], ",")[[1]])
    ur <- as.numeric(strsplit(strsplit(strsplit(cmd_output[grep(
      pattern = utils::glob2rx("Upper Right*"),
      cmd_output
    )], "\\(")[[1]][2], "\\)")[[1]][1], ",")[[1]])
    lr <- as.numeric(strsplit(strsplit(strsplit(cmd_output[grep(
      pattern = utils::glob2rx("Lower Right*"),
      cmd_output
    )], "\\(")[[1]][2], "\\)")[[1]][1], ",")[[1]])
...
gdal_cmd_builder2 <- function(executable, parameter_variables = c(), parameter_values = c(),
  ...
  defined_variables <- names(parameter_values)[sapply(parameter_values, function(X) {
    class(X)[1] !=
      "name"
  })]
  if (verbose) {
  ...
  parameter_vector <- c(
    parameter_variables_logical_strings, parameter_variables_vector_strings,
...
align_rasters2 <- function(unaligned, reference, dstfile, output_raster = FALSE,
  ...
  te <- c(reference_info$bbox[1, 1], reference_info$bbox[2, 1], reference_info$bbox[
    1,
    2
  ], reference_info$bbox[2, 2])
  ...
  if (projres_only) {
    synced <- gdalwarp2(
      srcfile = unaligned, dstfile = dstfile, t_srs = proj4_string,
      output_raster = output_raster, multi = multi, wo = wo, verbose = verbose,
      ...
    )
  } else {
    synced <- gdalwarp2(
      srcfile = unaligned, dstfile = dstfile, te = te, t_srs = proj4_string,
...
gdalwarp2 <- function(srcfile, dstfile, s_srs, t_srs, to, order, tps, rpc, geoloc,
  ...
  parameter_variables <- list(
    logical = list(varnames <- c(
      ...
      "setci"
    )), vector = list(varnames <- c("te", "tr", "ts")), scalar = list(varnames <- c(
      "order",
      "et", "refine_gcps", "wm", "cblend"
    )), character = list(varnames <- c(
      "s_srs",
  ...
  parameter_order <- c(
    "tps", "rpc", "geoloc", "tap", "dstalpha", "multi", "q",
  ...
  executable <- "gdalwarp"
  cmd <- gdal_cmd_builder2(
    executable = executable, parameter_variables = parameter_variables,
...
#' @description Create a schema in a postgres database
#' @param conn_pg A DBI database connection to the database
