...
#' @export
get_space_time_ind <- function(df, lp_dict, model_time_slices, res_time, do_parallel = T,
    ...
    res <- doFun(foreach::foreach(rs = itertools::ichunk(iterators::iter(df, by = "row"),
        nchunk), is = itertools::ichunk(iterators::icount(nrow(df)), nchunk), .packages = c("lubridate",
        "foreach", "iterators", "magrittr", "dplyr"), .combine = rbind, .inorder = F),
        {
            foreach::`%do%`(foreach::foreach(r = rs, i = is, .packages = c("lubridate",
                "foreach", "iterators"), .combine = rbind, .inorder = F), {

                # Get the modeling time slices covered by the data
                temporal_bands <- seq(dplyr::last(which(model_time_slices$TL <= r$TL[1])),
                  which(model_time_slices$TR >= r$TR[1])[1])

                # Get indices of spatio-temporal modeling grid covered by the
                # observation
                inds <- which(lp_dict$location_period_id == r$locationPeriod_id[1] &
                  lp_dict$t %in% temporal_bands)

                if (length(inds) > 0) {
                  cell_ind <- lp_dict$upd_long_id[inds]
                  time_slices_ind <- lp_dict$t[inds]

                  ## proportion of time captured by the observation, normalized
                  ## by number of modeling time steps that the observation
                  ## spans
                  tf <- unlist(lapply(temporal_bands, function(x) {
                    suppressMessages(lubridate::days(lubridate::days(1) + min(r$TR[1],
                      model_time_slices$TR[x]) - max(r$TL[1], model_time_slices$TL[x]))/lubridate::days(lubridate::days(1) +
                      model_time_slices$TR[x] - model_time_slices$TL[x]))
                  })) %>%
                    setNames(temporal_bands)

                  timefracs <- tf[as.character(time_slices_ind)]

                  tibble::tibble(map_obs = rep(i, length(cell_ind)), map_cell = cell_ind,
                    tfrac = timefracs)
                }
            })
        })
...
#' @export
reorder_single_source <- function(A, coords) {
    ...
    return(list(A = A_bfs_directed, reordering = bfs_order))
}
...
#' @export
get_space_time_ind_speedup <- function(df, lp_dict, model_time_slices, res_time,
    do_parallel = T, n_cpus = parallel::detectCores() - 2) {
        ...
        "foreach", "iterators", "magrittr", "dplyr"), .combine = rbind, .inorder = F),
        {
            foreach::`%do%`(foreach::foreach(r = rs, i = is, .packages = c("lubridate",
                ...
                if (length(inds) > 0) {
                  ...
                  time_slices_ind <- lp_dict$t[inds]
                  # Unique location period ids (accounting for modeling time
                  # slices) and modeling time slices
                  ...
                  # Outputs: K1: length of the mapping from observation to
                  # location periods K2: length of the mapping from location
                  # periods to cells

                  timefracs <- tf[as.character(lp_loctime_t$t)]

                  tibble::tibble(obs = i, map_obs_loctime_obs = list(rep(i, nrow(lp_loctime_t))),
                    map_obs_loctime_loc = list(lp_loctime_t$loctime_id), map_loc_grid_loc = list(lp_inds),
                    map_loc_grid_grid = list(cell_ind), tfrac = list(timefracs))
                }
            })
        })
    ...
    return(list(obs = obs, map_obs_loctime_obs = map_obs_loctime_obs, map_obs_loctime_loc = map_obs_loctime_loc,
        map_loc_grid_loc = map_loc_grid_loc, map_loc_grid_grid = map_loc_grid_grid,
        u_loctimes = u_loctimes, u_loctimes_ind = u_loctimes_ind, tfrac = tfrac))
}
