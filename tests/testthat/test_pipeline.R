...
observations_df <- data.frame(observation_collection_id = c(1, 1, 2, 2), time_left = lubridate::ymd(c("2000-01-01", 
    ...
    "testlocation", "testlocation", "testlocation"), primary = rep(TRUE, times = 4), 
...
test_that("pull_observation_data runs successfully", {
    conn_pg <- taxdat::connect_to_db(dbuser, dbname)
    expect_error({
        taxdat::pull_observation_data(conn_pg, "testlocation", lubridate::ymd("2000-01-01"), 
            lubridate::ymd("2001-12-31"))
    }, NA)
})
...
test_that("pull_observation_data has the right size", {
    ...
    expect_error({
        nrow(taxdat::pull_observation_data(conn_pg, "testlocation", lubridate::ymd("2000-01-01"), 
    ...
    expect_error({
        nrow(taxdat::pull_observation_data(conn_pg, "fakelocation", lubridate::ymd("2000-01-01"), 
            lubridate::ymd("2000-12-31")))
    }, NA)
    expect_equal({
        nrow(taxdat::pull_observation_data(conn_pg, "fakelocation", lubridate::ymd("2000-01-01"), 
    ...
    expect_error({
        nrow(taxdat::pull_observation_data(conn_pg, "testlocation", lubridate::ymd("2010-01-01"), 
    ...
    expect_error({
        nrow(taxdat::pull_observation_data(conn_pg, "testlocation", lubridate::ymd("1000-01-01"), 
