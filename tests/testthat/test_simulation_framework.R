...
#### testing.1 ####
data_fun <- function(){taxdat:::create_standardized_test_data(
  ## Extent parameters:,
  ### NONE,
  ## Raster parameters:,
  nrows = 20,
...
covariate_raster_funs <- lapply(
  seq_len(length(full_data$covariates)),
  function(covariate_idx){
    ...
    lapply(
      ...
      function(time_index) {
        return(list(
          name = ifelse(covariate_idx == 1, "population", paste("covariate",covariate_idx, sep = "")),
...
test_that("execute_pipeline.R runs successfully", {
    Sys.setenv(CHOLERA_CONFIG = rprojroot::find_root_file(criterion = ".choldir", "tests", "testthat", "config_simulation.yml"))
    expect_error({
