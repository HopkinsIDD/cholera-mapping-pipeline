...
data_fun <- function() {
  taxdat:::create_standardized_test_data(
    ...
    polygonal = c(FALSE, FALSE), radiating = c(FALSE, TRUE), constant = c(
      TRUE,
      FALSE
    ), rho = rep(0.999999, times = nc), radiating_polygons = list(
      NA,
      sf::st_union(sf::st_sfc(sf::st_point(c(0.25, 0.25)), sf::st_point(c(
        0.75,
        0.75
      )), crs = sf::st_crs(taxdat::create_test_polygons())))
    ), radiation_function = rep(list(function(x,
                                              mu) {
    ...
    noise = FALSE, polygon_proportion_observed = 1, polygon_observation_rates = exp(rnorm(
      nrow(test_polygons),
      -1
    )), polygon_size_bias = FALSE, nonlinear_covariates = FALSE, min_time_left = min_time_left,
