...
#' @return ggplot object
plot_grid_cases_mean_raster <-function(config,cache) {
  merge_grid_cases_mean_into_raster(config,cache)
  plot <- plot_raster_with_fill(name="grid_cases_mean_raster",
                                color_scale_type = "cases",
                                fill_column = "grid_cases_mean")
  return(plot)  
}
...
#' @return ggplot object
plot_modeled_rates_raster <-function(config,cache) {
  merge_modeled_rates_mean_into_raster(config,cache)
  plot <- plot_raster_with_fill(name="modeled_rates_mean_raster",
                                color_scale_type = "rates",
                                fill_column = "modeled_rates_mean")
  return(plot)  
...
###########################################################
# pull raster object from covar file
...
#' @return covar cube
get_covar_cube_no_cache <- function(config, cache) {
...
#cache the results
get_covar_cube <- cache_fun_results(name="covar_cube", fun=get_covar_cube_no_cache,overwrite = T,config=config)
...
get_raster_object_no_cache <- function(config, cache) {
  get_covar_cube(config, cache)
  sf_grid <-cache[["covar_cube"]]$sf_grid
  return(sf_grid)
}
#cache the results
get_raster_object <- cache_fun_results(name="sf_grid", fun=get_raster_object_no_cache,overwrite=T,config=config)
...
#' @param cache the cached environment that contains all the parameter information
#' @return  grid_cases
get_grid_cases_no_cache <- function(config, cache) {
  get_model_rand(config, cache)
  grid_cases<-MCMCvis::MCMCchains(cache[["model.rand"]], params = "grid_cases")
  return(grid_cases)
}
#cache the results
...
#' @descriptione extrac modeled rates (from log lambda) from model.rand
#' @param config config file that contains the parameter information
#' @param cache the cached environment that contains all the parameter information
#' @return  modeled_rates
get_modeled_rates_no_cache <- function(config, cache) {
  get_model_rand(config, cache)
  modeled_rates <- exp(MCMCvis::MCMCchains(cache[["model.rand"]], params = "log_lambda"))
  return(modeled_rates)
}
#cache the results
get_modeled_rates <- cache_fun_results("modeled_rates", get_modeled_rates_no_cache,overwrite = T,config=config)
#get_modeled_rates(config=config,cache)


#get new aggregated data: get_grid_cases_mean
get_grid_cases_mean_no_cache <- function(config,cache){
  get_grid_cases(config,cache)
  grid_cases_mean <-aggregate_to_grid_cases_mean(cache[["grid_cases"]],funs="mean")
  return(grid_cases_mean)
}
get_grid_cases_mean <- cache_fun_results("grid_cases_mean",get_grid_cases_mean_no_cache,overwrite = T,config=config)
#get_grid_cases_mean(config,cache)

#' @name aggregate_to_grid_cases_mean
#' @title aggregate_to_grid_cases_mean
#' @descriptione get the mean of the modeled cases for each grid cell
#' @param config config file that contains the parameter information
#' @param cache the cached environment that contains all the parameter information
#' @return  modeled cases mean for each grid cell by times
aggregate_to_grid_cases_mean <- function(grid_cases,funs="mean"){
  grid_cases_mean <-apply(grid_cases, 2, mean)
  return(grid_cases_mean)
}

#get new aggregated data: get_modeled_rates_mean
get_modeled_rates_mean_no_cache <- function(config,cache){
  get_modeled_rates(config,cache)
  modeled_rates_mean <-aggregate_to_modeled_rates_mean(cache[["modeled_rates"]],funs="mean")
  return(modeled_rates_mean)
}
get_modeled_rates_mean <- cache_fun_results("modeled_rates_mean",get_modeled_rates_mean_no_cache,overwrite = T,config=config)
#get_modeled_rates_mean(config=config,cache)

#' @name aggregate_to_modeled_rates_mean
#' @title aggregate_to_modeled_rates_mean
#' @descriptione get the mean of the modeled rates each grid cell
#' @param config config file that contains the parameter information
#' @param cache the cached environment that contains all the parameter information
...
#' @name merge_grid_cases_mean_into_raster
#' @title merge_grid_cases_mean_into_raster
#' @descriptione merge the modeled cases mean into raster
#' @param config config file that contains the parameter information
#' @param cache the cached environment that contains all the parameter information
#' @return  raster filled with modeled cases mean for each grid cell by times
merge_grid_cases_mean_into_raster_no_cache <- function(config, cache) {
  get_raster_object(config,cache)
  grid_cases_mean_raster <- cache[["sf_grid"]]
  get_grid_cases_mean(config,cache)
  grid_cases_mean <- cache[["grid_cases_mean"]]
  grid_cases_mean_raster$grid_cases_mean <- grid_cases_mean
  return(grid_cases_mean_raster)
}
merge_grid_cases_mean_into_raster <- cache_fun_results("grid_cases_mean_raster",merge_grid_cases_mean_into_raster_no_cache,overwrite = T,config)
#merge_grid_cases_mean_into_raster(config,cache)
...
#' @descriptione merge the modeled rates into raster
#' @param config config file that contains the parameter information
#' @param cache the cached environment that contains all the parameter information
#' @return  raster filled with modeled rates mean for each grid cell by times
merge_modeled_rates_mean_into_raster_no_cache <- function(config, cache) {
  get_raster_object(config,cache)
  grid_rates_mean_raster <- cache[["sf_grid"]]
  get_modeled_rates_mean(config,cache)
