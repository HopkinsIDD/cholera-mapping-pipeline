...
params:
  ...
  config_directory: "Analysis/configs/2015_2019_country_negbinom"
  max_countries: Inf
---
...
rate_rescaling <-  1e5  # rescale to have incidence per 10'000 people
  ...
  plt <- plt +
    ggplot2::geom_sf(
      data = case_rast %>%
        ...
        inner_join(avail_times) %>%     # filter out years with no observations
        group_by(country, rid, x, y) %>%
        summarise(`modeled cases` = max(`modeled cases`, na.rm = T)),
...
if (str_detect(params$config_directory, "negbinom")) {
  ...
  ggplot(phis, aes(x = mean, y = country)) +
    geom_point() +
    geom_errorbarh(aes(xmin = `2.5%`, xmax = `97.5%`), height = 0) +
    geom_vline(aes(xintercept = 0), lty = 2, size = .3, col = "red") +
    theme_bw()
