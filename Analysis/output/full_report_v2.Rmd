...
annual_stats <- data %>%
  ...
  summarise(
    range = max(annual_cases) - min(annual_cases),
    sd = sd(annual_cases),
...
all_comp <- map(unique(rhats$country), function(cntry) {
  dat <- rhats %>%
    mutate(
      tfrac_class = cut(tfrac, tfrac_cuts),
  ...
  dat %>%
    ...
    geom_abline(aes(intercept = 0, slope = 1), lty = 2) +
    geom_errorbar(
      data = filter(dat, rhat_above == "FALSE"),
    ...
    geom_point(
      data = filter(dat, rhat_above == "TRUE"),
...
who_comp <- dir(out_dir,
  ...
  map2_df(.x = ., .y = str_extract(., "[A-Z]{3}"), .f = function(x, y) {
    res <- read_csv(x)
    if (nrow(res) > 0) {
      return(res %>% mutate(country = y))
    }
  }) %>%
