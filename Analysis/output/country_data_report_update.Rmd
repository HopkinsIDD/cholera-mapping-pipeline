...
```{r gam input of cases,fig.cap=capFig("Observations input for the GAM model (cases)")}
  plot_gam_fit_input_cases (name="initial_values_data",cache=cache)
```

```{r gam input of rates,fig.cap=capFig("Observations output of the GAM model (rates)")}
  plot_gam_fit_input_rates (name="initial_values_data",cache=cache)
```

# GAM Output
**Observations output of the GAM model:**
```{r gam output of cases,fig.cap=capFig("Observations input for the GAM model (cases)")}
  plot_gam_fit_output_cases(name="gam_output_df",cache=cache)
```

```{r gam output of rates,fig.cap=capFig("Observations input for the GAM model (rates)")}
