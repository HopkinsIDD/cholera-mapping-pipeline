...
names(empty_raster)[which(names(empty_raster) == "modeled_cases_mean")] <- paste(
  "modeled cases\n",
  "\niterations: Chain"
)
...
names(empty_raster)[which(names(empty_raster) == "modeled_rates_mean")] <- paste(
  "modeled rates\n",
  "\niterations: Chain"
)
...
for (i in 1:length(model_output_filenames)) {
  ...
  actual_cases$oc_year <- rep(
    format(taxdat::read_file_of_type(
      stan_input_filenames[i],
      "stan_input"
