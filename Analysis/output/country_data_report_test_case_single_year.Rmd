...
``` {r echo=FALSE}
  ...
  covar[1:3,1]=c(FALSE, FALSE,FALSE)
  covar[1:3,2]=c(TRUE, TRUE,TRUE)
  covar[1:3,3]=c(TRUE, TRUE,TRUE)
  covar[1:3,4]=c(FALSE, FALSE,FALSE)
  covar[1:3,5]= c(TRUE, TRUE,TRUE)
  covar[1:3,6]= c(FALSE,  FALSE,TRUE)
  covar[1:3,7]=c(TRUE,TRUE,TRUE)
  covar[1:3,8]=c(TRUE,TRUE,FALSE)

  #rownames(covar)=paste("Covariate",1:ncol(params_table))
    rownames(covar)=paste("Covariate",1:3)
...
# file_names<-list()
# file_names[["stan_output"]]<-"C:/IDD/Cholera/cholera-mapping-pipeline/Analysis/output/test5.stan_output.rdata"
# file_names[["stan_input"]]<-"C:/IDD/Cholera/cholera-mapping-pipeline/Analysis/output/test5.stan_input.rdata"
...
```{r importcase}
    ...
    empty_raster<-merge(covar_grid,loc_sf,by.x="qualified_name",by.y="location_name")%>%select("updated_id","x","y","qualified_name","shape","t","spacetime_grid_id")
    empty_raster<-empty_raster[with(empty_raster,order(empty_raster$spacetime_grid_id)),]
    empty_raster<-sf::st_as_sf(empty_raster)
    empty_raster$t<-as.factor(empty_raster$t)
    levels(empty_raster$t)<-1:nlevels(empty_raster$t)
...
empty_raster <- dplyr::mutate(empty_raster,
                             modeled_cases_mean = NA,
                             modeled_rates_mean = NA,
                             )
empty_raster$modeled_cases_mean <- modeled_cases_mean

names(empty_raster)[which(names(empty_raster)=="modeled_cases_mean")] <- paste("modeled cases\n",
                                                                              "\niterations: Chain")


empty_raster$modeled_rates_mean <- modeled_rates_mean
names(empty_raster)[which(names(empty_raster)=="modeled_rates_mean")] <- paste("modeled rates\n",
                                                                             "\niterations: Chain")
case_raster<-empty_raster
case_raster_case <- case_raster %>% dplyr::select(dplyr::contains("modeled cases"),
                                                  t) %>% tidyr::gather(dplyr::contains("iterations: Chain"),
