...
package_list <- c("stringr", "dplyr", "magrittr", "purrr", "readr", "sf", "raster",
    "fasterize", "remotes", "rgeoboundaries", "taxdat", "optparse", "yaml")

### Run options

option_list <- list(optparse::make_option(c("-c", "--config"), action = "store",
    type = "character", help = "Model run configuration file"), optparse::make_option(c("-d",
    "--cholera_directory"), action = "store", default = NULL, type = "character",
    help = "Cholera directory"), optparse::make_option(c("-o", "--old_runs"), action = "store",
    default = NULL, type = "character", help = "old runs or not"), optparse::make_option(c("-s",
    "--single_year"), action = "store", default = NULL, type = "character", help = "single year runs or not"))
...
params <- optparse::OptionParser(option_list = option_list) %>%
    optparse::parse_args()

install.packages(paste0(params$cholera_directory, "/packages/taxdat"), type = "source",
    repos = NULL)



## Method1: create summary values for each layer across cells, then summarize
## across layers to get CIs. for each layer, group the cells by incidence rates
## (mild, moderate, and high incidence areas)
...
#' @return rate_raster
get_disaggregated_rate_raster <- function(covar_data_filename, model_output_filenames,
    stan_input_filenames, if_single_year_run) {
    ...
    modeled_rates <- exp(as.array(model.rand)[, , grepl("log_lambda", names(model.rand)),
        drop = FALSE])
    modeled_rates_mean_by_grid_layer_tmp <- as.data.frame(t(apply(modeled_rates,
        c(1, 3), mean))) %>%
    ...
    if (params$single_year == FALSE) {
        obs_years <- (nrow(modeled_rates_mean_by_grid_layer_tmp)/5) * ((min(lubridate::year(stan_input$sf_cases_resized$TL)):max(lubridate::year(stan_input$sf_cases_resized$TR))) -
            2015)
        modeled_rates_mean_by_grid_layer <- data.frame()
        for (row_idx in unique(obs_years)) {
            tmp <- modeled_rates_mean_by_grid_layer_tmp[(row_idx + 1):(row_idx +
                nrow(modeled_rates_mean_by_grid_layer_tmp)/5), ]
            modeled_rates_mean_by_grid_layer <- rbind(modeled_rates_mean_by_grid_layer,
                tmp)
        }
    } else {
    ...
    rate_raster <- merge(rate_raster[rate_raster$long_id %in% unique(modeled_rates_mean_by_grid_layer$id),
        ], modeled_rates_mean_by_grid_layer, by.x = "long_id", by.y = "id")

    colnames(rate_raster)[str_detect(colnames(rate_raster), ".*[0-9].*")] <- paste0("layer",
        seq_len(dim(modeled_rates)[1]))
    rate_raster <- rate_raster[, str_detect(colnames(rate_raster), ".*[0-9].*")]

    disaggregated_rate_raster <- raster::raster(rate_raster, res = max(0.00833333,
        0.00833333))
    for (layer_idx in seq_len(ncol(rate_raster) - 1)) {
        ...
        single_disaggregated_rate_raster <- fasterize::fasterize(layer_value, single_layer,
            field = paste0("layer", layer_idx))
    ...
    if (if_single_year_run) {
        # import all single-year data and stack the raster data replace config
        # file name with year 2016-2019
        ...
        for (year_idx in additional_years) {
            if (params$old_runs) {
                new_config_name = str_replace_all(params$config, year_with_config,
                  year_idx)
                ...
                file_names_tmp <- update_filename_oldruns(filename = file_names_tmp,
                  old_runs = params$old_runs)
                file_names_tmp[["stan_output"]] <- stringr::str_remove(file_names_tmp[["stan_output"]],
                  "iv-wN-cwN-csF-teT-teaF-weF.")
                file_names_tmp[["stan_output"]] <- stringr::str_remove(file_names_tmp[["stan_output"]],
                  "-F")
            } else {
                new_config_name = paste(paste0(c(strsplit(params$config, "/")[[1]][1:(length(strsplit(params$config,
                  "/")[[1]]) - 1)]), collapse = "/"), c(str_replace_all(strsplit(params$config,
                  "/")[[1]][length(strsplit(params$config, "/")[[1]])], year_with_config,
                  year_idx)), sep = "/")
                config_tmp <- yaml::read_yaml(paste0(params$cholera_directory, new_config_name))
                file_names_tmp <- taxdat::get_filenames(config_tmp, params$cholera_directory)
                file_names_tmp <- update_filename_oldruns(filename = file_names_tmp,
            ...
            modeled_rates_mean_by_grid_layer_tmp <- as.data.frame(t(apply(modeled_rates_tmp,
                c(1, 3), mean))) %>%
                mutate(id = seq_len(dim(modeled_rates_tmp)[3]))

            rate_raster_tmp <- merge(rate_raster_tmp[rate_raster_tmp$long_id %in%
                unique(modeled_rates_mean_by_grid_layer_tmp$id), ], modeled_rates_mean_by_grid_layer_tmp,
                by.x = "long_id", by.y = "id")

            colnames(rate_raster_tmp)[str_detect(colnames(rate_raster_tmp), ".*[0-9].*")] <- paste0("layer",
                seq_len(dim(modeled_rates_tmp)[1]))
            rate_raster_tmp <- rate_raster_tmp[, str_detect(colnames(rate_raster_tmp),
                ".*[0-9].*")]
            disaggregated_rate_raster_tmp <- raster::raster(rate_raster_tmp, res = max(0.00833333,
                0.00833333))

            for (layer_idx in seq_len(ncol(rate_raster_tmp) - 1)) {
                layer_value <- rate_raster_tmp[, layer_idx]
                single_layer <- raster::raster(rate_raster_tmp, res = max(0.00833333,
                  0.00833333))
                single_disaggregated_rate_raster_tmp <- fasterize::fasterize(layer_value,
                  single_layer, field = paste0("layer", layer_idx))
                if (!extent(single_disaggregated_rate_raster_tmp) == extent(disaggregated_rate_raster_tmp)) {
                  single_disaggregated_rate_raster_tmp <- extend(single_disaggregated_rate_raster_tmp,
                    disaggregated_rate_raster_tmp, value = NA)
                  extent(single_disaggregated_rate_raster_tmp) = extent(disaggregated_rate_raster_tmp)
                }
                disaggregated_rate_raster_tmp <- stack(disaggregated_rate_raster_tmp,
                  single_disaggregated_rate_raster_tmp)
            }

            names(disaggregated_rate_raster_tmp) <- colnames(rate_raster_tmp)[-ncol(rate_raster_tmp)]
            disaggregated_rate_raster_tmp <- extend(disaggregated_rate_raster_tmp,
                disaggregated_rate_raster, value = NA)
...
aggregate_affected_pop_across_cells <- function(pop_raster_cropped, rate_raster_cropped,
    shapefile, threshold_list) {
    ...
    for (threshold_idx in seq_len(length(threshold_list))) {
        pop_prop[, threshold_idx] <- 100 * sum(values(pop_raster_cropped)[values(rate_raster_cropped) >=
            threshold_list[threshold_idx]], na.rm = T)/sum(values(pop_raster_cropped),
            na.rm = T)
    }
...
#' @return s
aggregate_affected_pop_across_layers <- function(pop_prop, probability_cutoffs, include_mean = TRUE,
...
#' @return 
get_affected_pop <- function(pop_raster_cropped, rate_raster_cropped, shapefile,
    threshold_list = c(0.001, 1e-04, 1e-05), include_mean = TRUE, calculate_disjoint_values = TRUE,
    within_layer_aggregator = aggregate_affected_pop_across_cells, across_layer_aggregator = aggregate_affected_pop_across_layers,
    ...
    for (layer_idx in seq_len(nlayers(rate_raster_cropped))) {
        results_by_layer[[layer_idx]] <- within_layer_aggregator(pop_raster_cropped = pop_raster_cropped,
            rate_raster_cropped = rate_raster_cropped[[layer_idx]], shapefile = shapefile,
            threshold_list = threshold_list)
    }
    pop_prop = do.call("rbind", results_by_layer)
    return(across_layer_aggregator(pop_prop = pop_prop, probability_cutoffs = probability_cutoffs,
        include_mean = include_mean, calculate_disjoint_values = calculate_disjoint_values))
}
...
if (params$old_runs) {
    file_names[["stan_output"]] <- stringr::str_remove(file_names[["stan_output"]],
        "iv-wN-cwN-csF-teT-teaF-weF.")
    file_names[["stan_output"]] <- stringr::str_remove(file_names[["stan_output"]],
        "-F")
}
...
pop_raster_cropped <- crop_to_shapefile(raster = pop_raster, shapefile = shapefile,
    snap = "out")

disaggregated_rate_raster <- get_disaggregated_rate_raster(covar_data_filename = file_names[["covar"]],
    model_output_filenames = file_names[["stan_output"]], stan_input_filenames = file_names[["stan_input"]],
    if_single_year_run = params$single_year)
# rate_raster_cropped <- crop_to_shapefile(raster =
# disaggregated_rate_raster,shapefile = shapefile,snap='out')
rate_raster_cropped <- crop_to_shapefile(raster = disaggregated_rate_raster[[1]],
    shapefile = shapefile, snap = "out")

for (i in seq_len(nlayers(disaggregated_rate_raster))[-1]) {
    tmp <- crop_to_shapefile(raster = disaggregated_rate_raster[[i]], shapefile = shapefile,
        snap = "out")
...
table <- get_affected_pop(pop_raster_cropped = pop_raster_cropped, rate_raster_cropped = rate_raster_cropped,
    shapefile = shapefile, threshold_list = c(0.001, 1e-04, 1e-05), include_mean = TRUE,
    calculate_disjoint_values = TRUE, within_layer_aggregator = aggregate_affected_pop_across_cells,
    across_layer_aggregator = aggregate_affected_pop_across_layers, probability_cutoffs = c(0.025,
        0.5, 0.975))
