...
params:
  ...
  config: "Analysis/configs/2015_2019_country/config_SSD_2015_2019.yml"
---
...
# Mapping results summary for `r stringr::str_extract(params$config, "(?<=g_)[A-Z]{3}(?=_)")` and period `r stringr::str_extract(params$config, "(?<=_)[0-9]{4}_[0-9]{4}(?=\\.)")`
...
taxdat::plot_raster_population(covar_data_filename = file_names["covar"], 
                               render = T)
```
...
taxdat::plot_raster_covariates(covar_data_filename = file_names["covar"], 
                               render = T)
```
...
taxdat::plot_chain_convergence(stan_output_filename,
...
who_annual_cases <- taxdat::pull_output_by_source(who_annual_cases, "%WHO Annual Cholera Reports%",
                                                  database_api_key_rfile = str_c(params$cholera_directory, "Analysis/R/database_api_key.R"))
who_annual_cases %>%
  as.data.frame() %>%
