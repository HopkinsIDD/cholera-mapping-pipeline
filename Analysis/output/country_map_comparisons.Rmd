...
for (i in 1:length(all_configs)) {
  ...
  if (country_count >= params$max_countries) {
    break
  }
  ...
  try({
    ...
    calc_country_summ <- sf_grid_wider %>%
      dplyr::group_by(t) %>%
      dplyr::summarise(
        dplyr::across(dplyr::contains("cases_"), sum),
...
for (cntry in all_locations) {
  ...
  calc_country_summ_old <- data.frame(
    cases_old = sum(raster::values(country_case), na.rm = TRUE),
...
plot_df <- dplyr::bind_rows(dplyr::select(country_summ, -pop), country_summ_old) %>%
  ...
  dplyr::mutate(
    t = factor(t, levels = levels_yrs, labels = labels_yrs),
