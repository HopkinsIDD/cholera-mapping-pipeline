...
disjoint_set_sf_cases=disjoint_set_sf_cases%>%rename(cases=suspected_cases)%>%mutate(area=as.numeric(sf::st_area(disjoint_set_sf_cases))/1000/1000,area_adjusted_cases=cases/area)


disjoint_set_sf_cases <- dplyr::arrange(disjoint_set_sf_cases, 
                                        -area)
overlaps <- sf::st_relate(disjoint_set_sf_cases, disjoint_set_sf_cases, 
                          "2********")
...
if(all(params$Loc_with_inconsistent_data=="no")){
  ...
  # 
  disjoint_set_sf_cases%>%group_by(set)%>%
    do(gg={
      ggplot(.,ggplot2::aes(fill = cases))+
        ...
        ggplot2::theme(legend.position = "bottom")+
        ggplot2::facet_wrap(~set)})%>%
    .$gg%>%gridExtra::grid.arrange(grobs=.,nrow=1)
...
if (!is.null(case_raster)) {
  ...
  case_raster_case <- case_raster %>% dplyr::select(dplyr::contains("modeled cases"),
                                                    id, t) %>% tidyr::gather(dplyr::contains("iterations: Chain"),
                                                                             key = "chain", value = "value") %>% dplyr::mutate(chain = stringr::str_replace(chain,
                                                                                                                                                            "modeled cases", ""))
...
if(!is.null(case_raster)){
  ...
  case_raster_rate <- case_raster %>% dplyr::select(dplyr::contains("modeled rates"), 
                                                    id, t) %>% tidyr::gather(dplyr::contains("iterations: Chain"), 
                                                                             key = "chain", value = "value") %>% dplyr::mutate(chain = stringr::str_replace(chain, 
                                                                                                                                                            "modeled rates", ""))
...
for (i in 1:length(model_output_filenames)) {
  ...
  nchain <- dim(MCMCvis::MCMCchains(model.rand, params = "lp__"))[1]/niter_per_chain
  stan_data <- read_file_of_type(stan_input_filenames[i],
                                 "stan_input")$stan_data
  modeled_cases <- as.array(model.rand)[, , grepl("modeled_cases",
                                                  names(model.rand)), drop = FALSE]
  modeled_cases_chain_mean <- apply(modeled_cases, c(2,
                                                     3), mean)
  actual_cases <- matrix(stan_data$y, nrow(modeled_cases_chain_mean),
                         ncol(modeled_cases_chain_mean), byrow = TRUE)
  ...
  actual_cases$oc_uid <- rep(taxdat::read_file_of_type(stan_input_filenames[i],
                                                       "stan_input")$sf_cases_resized$OC_UID, each = nchain)
  actual_cases$oc_year <- rep(format(taxdat::read_file_of_type(stan_input_filenames[i],
                                                               "stan_input")$sf_cases_resized$TR, "%Y"),
                              each = nchain)
  comparison <- dplyr::left_join(modeled_cases_chain_mean,
                                 actual_cases, by = c(chains = "chains", parameters = "parameters"))
