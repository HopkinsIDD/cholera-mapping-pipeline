...
for(idx in 1:nrow(new_file_names)){
  ...
  start_year= as.numeric(substr(new_file_names$file_name[idx],5,8))
...
```{r tfrac max plot,fig.height=8,fig.width=8,echo=FALSE}
# Plotting
for(run_id_idx in unique(tfrac_table$run_id)){
  for(iso_idx in unique(tfrac_table[tfrac_table$run_id==run_id_idx,]$iso)){
  
  grid.newpage()

  p<-tfrac_table %>%
  dplyr::filter(run_id==run_id_idx & iso == iso_idx) %>%
