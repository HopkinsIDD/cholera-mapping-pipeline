...
# remove shapefiles that are not in the fake_output_obs
cache[["output_shapefiles"]] <- cache[["output_shapefiles"]] %>%
  dplyr::filter(location_period_id %in% cache[["stan_input"]]$fake_output_obs$locationPeriod_id) %>%
...
**Shapefile outlines for all observed location periods:**
```{r shpoutlines, fig.height=30, fig.width=30, out.width="150%",fig.cap=capFig("Shapefile outlines for all observed location periods. ")}
plot_unique_db_shps(config=params$config,cache=cache,cholera_directory=params$cholera_directory)
```

**Shapefile outlines for all observed location periods by time slices:**
```{r shpoutlines_by_timeslices, fig.height=30, fig.width=30, out.width="150%",fig.cap=capFig("Shapefile outlines for all observed location periods by time slices. ")}
plot_unique_db_shps_by_time(config=params$config,cache=cache,cholera_directory=params$cholera_directory)
```
...
```
**Modeled cases raster by time slices and chains:**
```{r caserast_bygridtimechain, fig.cap=capFig("Modeled cases by time and chain"), fig.height=8, fig.width=8}
plot_modeled_cases_by_gridtime_chain(config=params$config,cholera_directory=params$cholera_directory,cache=cache,render = T)
```
**Mean modeled cases raster across time slices and chains:**
```{r meancaserast, fig.cap=capFig("Mean modeled cases"), fig.height=5, fig.width=10}
plot_modeled_cases_mean_raster(config=params$config,cholera_directory=params$cholera_directory,cache=cache)
```
...
```{r raterast_bygridtimechain, fig.cap=capFig("Modeled rates by time and chain"), fig.height=8, fig.width=8}
...
```
**Mean modeled rates raster across time slices and chains:**
```{r meanraterast, fig.cap=capFig("Mean modeled rates"), fig.height=5, fig.width=10}
plot_modeled_rates_raster(config=params$config,cholera_directory=params$cholera_directory,cache=cache)
```
...
```
### Median Proportion per risk category (per 100'000)
```{r risk_prop,fig.height=5,fig.width=10,fig.cap=capFig("Median proportion per risk category (per 100'000)")}
risk_cat_dict <- c("<1", "1-10", "10-20", "20-50", "50-100", ">100")

risk_cat_pop <- cache[["genquant"]]$summary(variables = "location_risk_cat_prop", .cores = 1) %>% 
  mutate(id = str_extract(variable, "(?<=\\[)[0-9]+") %>% as.numeric(),
...
### Median number per risk category  (per 100'000)
```{r risk_num,fig.height=5,fig.width=10,fig.cap=capFig("Median number per risk category  (per 100'000)")}
risk_cat_dict <- c("<1", "1-10", "10-20", "20-50", "50-100", ">100")

risk_cat_num <- cache[["genquant"]]$summary(variables = "location_risk_cat_num", .cores = 1) %>% 
  mutate(id = str_extract(variable, "(?<=\\[)[0-9]+") %>% as.numeric(),
         risk_cat = str_extract(variable, "(?<=,)[0-9]+") %>% as.numeric(),
         risk_cat = factor(risk_cat_dict[risk_cat], levels = risk_cat_dict),
         location_period_id = cache[["output_shapefiles"]]$location_period_id[id],
...
### Posterior probability of districts in a given risk category
```{r risk_cat_prob,fig.height=5,fig.width=10,fig.cap=capFig("Posterior probability of districts in a given risk category.")}
risk_cat_dict <- taxdat::get_risk_cat_dict()

risk_cat_prob<-cache[["genquant"]]$draws(variables = "location_risk_cat")%>%
  apply(.,c(1,2),rbind) %>%
  apply(.,c(1),rbind) %>%
  as.data.frame(.) %>%
  gather(.,"location","risk_cat") %>%
  mutate(
    id = str_extract(location,"[0-9]+") %>% as.numeric(),
    #location = str_replace(location,"V","Location"),
    risk_cat = factor(risk_cat_dict[risk_cat], levels = risk_cat_dict)
  ) %>% 
  group_by(id,risk_cat) %>%
  summarize(risk_cat_prob = n()/4000) %>%
  spread(risk_cat,risk_cat_prob,fill = 0) %>%
  mutate(
    location_period_id = cache[["output_shapefiles"]]$location_period_id[id],
    admin_level = str_extract(location_period_id, "ADM[0-2]")
  ) %>% 
  gather(.,"risk_cat","risk_cat_prob",-c("location_period_id","admin_level","id")) %>% 
  inner_join(cache[["output_shapefiles"]], ., by = c("location_period_id", "admin_level")) #%>% 

risk_cat_prob %>% 
  mutate(risk_cat = factor(risk_cat, levels = risk_cat_dict)) %>%
  ggplot(aes(fill = risk_cat_prob %>% as.numeric())) +
  geom_sf(col = "white", size = .05) +
  facet_grid(admin_level ~ risk_cat) +
  scale_fill_viridis_c() +
  ggthemes::theme_map() +
...
### Maximum posterior probability of districts in a given risk category
```{r max_risk_cat_prob,fig.height=5,fig.width=10,fig.cap=capFig("Maximum posterior probability of districts in a given risk category.")}

risk_cat_prob %>% 
  mutate(risk_cat = factor(risk_cat, levels = risk_cat_dict)) %>%
  group_by(shapeName,location_period_id,admin_level,geom)%>%
  filter(risk_cat_prob==max(risk_cat_prob)) %>% 
  ...
  ggthemes::theme_map() +
  theme(legend.position = "right") +
  guides(fill=guide_legend(title="Risk cat with max probability"))

```

### Population

```{r pop-raster,fig.height=5,fig.width=10,fig.cap=capFig("Population per year")}
...
```{r pop-comparison,fig.height=5,fig.width=10,fig.cap=capFig("Comparison of population data between genquant and wpp")}
plot_pop_comparison(config=params$config, cache=cache, cholera_directory=params$cholera_directory)
```

## Validation
**Fraction of full observations covered by the prediction interval, by admin level:**
```{r coverage,fig.height=5,fig.width=10,fig.cap=capFig("Fraction of full observations covered by the prediction interval, by admin level")}
plot_posterior_coverage_cdr(cache=cache,config=params$config,cholera_directory = params$cholera_directory)
```
...
**Gelman-Rubin Rhat:**
```{r rhat, fig.height=5, fig.width=10, fig.cap=capFig("Gelman-Rubin Rhat at the observation level")}
plot_Rhat(name="model.rand",cache)
```

**Gelman-Rubin Rhat by admin units:**
```{r rhat by admin units, fig.height=8, fig.width=8, fig.cap=capFig("Gelman-Rubin Rhat for generated quantities admin-level outputs")}
plot_Rhat_by_admin(cache=cache,config=params$config,cholera_directory = params$cholera_directory)
```

**W distribution:**
```{r w_distribution, fig.height=10, fig.width=10, fig.cap=capFig("W distribution")}
if(config_file$spatial_effect){
plot_w_mean(cache = cache, config = params$config, cholera_directory = params$cholera_directory)
} else{
  print("No spatial effect for this model run.")
}
```
...
if(config_file$warmup){
  if(config_file$spatial_effect){
    plot_gam_fit_input (name="initial_values_data", cache=cache, mean=FALSE, type="cases", use_log = TRUE)
  } else {
    plot_gam_fit_input (name="initial_values_data", cache=cache, mean=FALSE, type="cases", use_log = FALSE)
  }
}
```
...
if(config_file$warmup){
    if(config_file$spatial_effect){
    plot_gam_fit_input (name="initial_values_data", cache=cache, mean=FALSE, type="rates", use_log = TRUE)
  } else {
    plot_gam_fit_input (name="initial_values_data", cache=cache, mean=FALSE, type="rates", use_log = FALSE)
  }
}
```
...
if(config_file$warmup){
  if(config_file$spatial_effect){
    plot_gam_fit_input (name="initial_values_data", cache=cache, mean=FALSE, type="case variance", use_log = TRUE)
  } else {
    plot_gam_fit_input (name="initial_values_data", cache=cache, mean=FALSE, type="case variance", use_log = FALSE)
  }
}
```
...
if(config_file$warmup){
  if(config_file$spatial_effect){
    plot_gam_fit_input (name="initial_values_data", cache=cache, mean=FALSE, type="rate variance", use_log = TRUE)
  } else {
    plot_gam_fit_input (name="initial_values_data", cache=cache, mean=FALSE, type="rate variance", use_log = FALSE)
  }
}
```
