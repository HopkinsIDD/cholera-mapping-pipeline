...
option_list <- list(
  optparse::make_option(c("-c", "--config"),
    action = "store", default = Sys.getenv("CHOLERA_CONFIG", "config.yml"),
...
if (!file.exists(spatial_coverage_filename) | opt$redo) {
  spatial_coverage <- taxdat::get_spatial_coverage(
    config = config,
...
if (!file.exists(who_filename) | opt$redo) {
  ...
  who_annual_cases <- taxdat::pull_output_by_source(
    sf_cases = who_annual_cases,
    source_match = "%WHO Annual Cholera Reports%",
    database_api_key_rfile = str_c(
