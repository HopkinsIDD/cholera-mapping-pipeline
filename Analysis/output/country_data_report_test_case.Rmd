...
params:
  ...
  test_true_grid_case_filename: "/home/app/cmp/Analysis/output/test_case_1_true_grid_cases.rdata"
...
disjoint_set_sf_cases=disjoint_set_sf_cases%>%rename(cases=suspected_cases)%>%mutate(area=as.numeric(sf::st_area(disjoint_set_sf_cases)),area_adjusted_cases=cases/area)
...
disjoint_set_sf_cases$cases=ifelse(disjoint_set_sf_cases$cases==0,exp(-5),disjoint_set_sf_cases$cases)
  ...
  disjoint_set_sf_cases%>%group_by(set)%>%
      do(gg={
        ggplot(.,ggplot2::aes(fill = cases))+
          ggplot2::geom_sf() +
          taxdat::color_scale(type = "case", use_case = "ggplot map", use_log = TRUE)+
...
# cache<-new.env()
# cache[["area_adjusted_observed_polygon_cases_disjoint_aggregated"]]<-disjoint_set_sf_cases
# plot_sf_with_fill(cache = cache, name = "area_adjusted_observed_polygon_cases_disjoint_aggregated",
#         color_scale_type = "cases", fill_column = "area_adjusted_suspected_cases")
disjoint_set_sf_cases$area_adjusted_cases=ifelse(disjoint_set_sf_cases$area_adjusted_cases==0,exp(-5),disjoint_set_sf_cases$area_adjusted_cases)
taxdat::plot_area_adjusted_observed_cases(
  disjoint_set_sf_cases,
  render = T
)
...
```{r covars, fig.height=5, fig.width=12, fig.cap="Figure 3.1 Covariate rasters in modeling process"}
#taxdat::plot_raster_covariates(covar_data_filename = file_names["covar"], render = T)
        ...
        pltdata[which(pltdata$t==1),]$t="2000"
        pltdata[which(pltdata$t==2),]$t="2001"
        }else{
         pltdata[which(pltdata$t==1),]$t="2000"
        }
        pltdata$t<-as.factor(pltdata$t)

        pltdata_dummy <- tidyr::gather(dplyr::filter(pltdata, !qualified_name == "1" ), one_of(colnames(pltdata)[stringr::str_detect(colnames(pltdata),"covariate|pop")]),key = "covars", value = "value")%>%subset(!covars=="covariate_contribution")
      ...
      do(gg={
        ggplot(.,ggplot2::aes(fill = value, color = value)) + ggplot2::geom_sf()+
          ...
          ggplot2::scale_fill_viridis_c(aesthetics = c("colour", "fill"),
                                        guide = ggplot2::guide_colorbar(title = "Covariates in\nmodeling process"),
...
``` {r covars_data_simulation, fig.height=5, fig.width=12, fig.cap="Figure 3.2. Non-population covariate rasters in data simulation process"}
#plot the unused covariates
data_simulation_covs <- readRDS(params$Cov_data_simulation_filename)

covdata<-as.data.frame(do.call(rbind, data_simulation_covs[2:(length(data_simulation_covs))])) %>%
...
covdata<-sf::st_as_sf(covdata)
        ...
        if(length(unique(covdata$t))>1){
        covdata[which(covdata$t==1),]$t="2000"
        covdata[which(covdata$t==2),]$t="2001"
        }else{
         covdata[which(covdata$t==1),]$t="2000"
   ...
   covdata%>%group_by(covariate)%>%
       do(gg={
        ggplot(.,ggplot2::aes(fill = value, color = value)) + ggplot2::geom_sf()+
          ...
          ggplot2::scale_fill_viridis_c(aesthetics = c("colour", "fill"),
                                        guide = ggplot2::guide_colorbar(title = "Non-population \ncovariates in\ndata simulation"),
                                        option = "B", na.value = "white") +
          ggplot2::theme_bw() +
          ggplot2::facet_wrap(~t)+
          ggplot2::theme(legend.position = "bottom",
...
pre_case_raster <- taxdat::get_case_raster(
  ...
  model_output_filenames = file_names[["stan_output"]]
    ...
    empty_raster$t<-as.numeric(empty_raster$t)
...
if (!is.null(case_raster)) {
 case_raster_case <- case_raster %>% dplyr::select(dplyr::contains("modeled cases"),t,shape) %>%
   ...
   dplyr::mutate(chain = stringr::str_replace(chain,"modeled cases", ""))
     ...
     if(length(unique(case_raster_case$t))>1){
        case_raster_case[which(case_raster_case$t==1),]$t="2000"
        case_raster_case[which(case_raster_case$t==2),]$t="2001"
        }else{
        case_raster_case[which(case_raster_case$t==1),]$t="2000"

        }
case_raster_case$value=ifelse(case_raster_case$value==0,exp(-5),case_raster_case$value)
plt <- ggplot2::ggplot()
plt <- plt +
  ggplot2::geom_sf(
...
if(!is.null(case_raster)){
  ...
  case_raster_rate <- case_raster %>% dplyr::select(dplyr::contains("modeled rates"),
        shape, t) %>% tidyr::gather(dplyr::contains("iterations: Chain"),
        key = "chain", value = "value") %>% dplyr::mutate(chain = stringr::str_replace(chain,
        "modeled rates", ""))

  case_raster_rate$t<-as.numeric(case_raster_rate$t)

  if(length(unique(case_raster_rate$t))>1){
        case_raster_rate[which(case_raster_rate$t==1),]$t="2000"
        case_raster_rate[which(case_raster_rate$t==2),]$t="2001"
        }else{
        case_raster_rate[which(case_raster_rate$t==1),]$t="2000"
...
```{r true and modeled gridded cases, fig.cap="Figure 7. Observations vs. modeled gridded cases", fig.height=6, fig.width=6}
 ...
 true_grid_case<-readRDS(config$test_true_grid_case_filename)#%>%select(row,col,cases,t)#%>%rename(x=row,y=col)
