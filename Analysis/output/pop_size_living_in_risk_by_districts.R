...
find_output_dir <- function(cholera_directory,
  ...
  if (country_status[country_status$approved_country == country_code, ]$report_old_new_output == "old") {
    if (country_status[country_status$approved_country == country_code, ]$single_year_multiple_year == "single") {
      old_output_dir <- old_output_dir_single
    } else {
...
get_rate_sf <- function(covar_data_filename,
  ...
  if (!if_single_year_run) {
    ...
    for (row_idx in unique(obs_years)) {
      ...
      if (!is.null(dropped_year_index) & match(row_idx, unique(obs_years)) %in% dropped_year_index) {
        next
      }

      # To assume 0 for a year
      tmp <- modeled_cases_mean_by_grid_layer_tmp[
        (row_idx + 1):(row_idx + (nrow(modeled_cases_mean_by_grid_layer_tmp) / 5)),
        1:niter_per_chain
      ] # 5 here as well -- changed on March 23th
  ...
  tryCatch(
    {
      ...
      if (!params$single_year) {
        ...
        for (config_fns in params$config) {
          ...
          if (!is.null(dropped_year_index) & (match(config_fns, params$config) %in% dropped_year_index)) {
            next
          }
      ...
      intermediate_output <- aggregate_affected_pop_across_cells_by_districts(
        sf_rate_pop = sf_rate_pop,
...
if (all(paste0(Dec_2021_full_list, "_sum_table.csv") %in% list.files(final_output_dir))) {
  ...
  ### make the table
  agg_sum_table <- tibble::tibble(
    country_name = as.character(),
  ...
  for (country_code in Dec_2021_full_list) {
    ...
    agg_sum_table <- agg_sum_table %>%
      add_row(
        country_name = country_name_full_list[match(country_code, Dec_2021_full_list)],
  ...
  names(agg_sum_table) <- c(
    "Country Name", ">= 1e-05 Mean", ">= 1e-05 Median", ">= 1e-05 IQR", ">= 1e-05 Q0.025", ">= 1e-05 Q0.05", ">= 1e-05 Q0.25", ">= 1e-05 Q0.75", ">= 1e-05 Q0.95", ">= 1e-05 Q0.975",
...
get_rate_raster <- function(covar_data_filename,
  ...
  if (!if_single_year_run) {
    ...
    for (row_idx in unique(obs_years)) {
      ...
      # To assume 0 for a year
      tmp <- modeled_cases_mean_by_grid_layer_tmp[
        (row_idx + 1):(row_idx + (nrow(modeled_cases_mean_by_grid_layer_tmp) /
