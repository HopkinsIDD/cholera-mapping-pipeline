...
get_disaggregated_rate_raster <- function(covar_data_filename,
  ...
  #names(disaggregated_rate_raster)<-colnames(rate_raster)[1:20]
...
if(iso_code=="AGO"){
  ...
  #year_list=c("2015","2016","2017")#2019 shapefiles are different
  year_list=c("2015","2016","2017","2018","2019")#2019 shapefiles are different
}else if(iso_code=="NER"){
  year_list=c("2018","2019")#try in small rasters
...
rate_raster_tmp <- rate_raster_tmp[,str_detect(colnames(rate_raster_tmp),".*[0-9].*")]
 disaggregated_rate_raster_tmp <- raster::raster(rate_raster_tmp, res = max(0.00833333, 0.00833333))
  ...
  #for (layer_idx in 1:200){
######
   error_checking <- data.frame(layer_id=layer_idx,year=year_idx)
   write.csv(error_checking,"/home/qulu/cmp_report/Analysis/sdn_error_checking.csv")
########
...
#########
   writeRaster( disaggregated_rate_raster_tmp ,'sdn_disaggregated_rate_raster_tmp.tif',overwrite=TRUE)
   writeRaster( single_disaggregated_rate_raster_tmp ,'sdn_single_disaggregated_rate_raster_tmp.tif',overwrite=TRUE)
#########
...
#names(disaggregated_rate_raster_tmp)<-colnames(rate_raster_tmp)[1:200]
...
#########
writeRaster(disaggregated_rate_raster ,'sdn_disaggregated_rate_raster.tif',overwrite=TRUE)
#########
}
  }
...
aggregate_affected_pop_across_layers <- function(pop_prop,probability_cutoffs, include_mean = TRUE, calculate_disjoint_values = TRUE) {
  ...
  if (include_mean) {
    ...
    #if (calculate_disjoint_values) {
      #mean_values <- c(0, mean_values)
      #mean_values <- c(na.exclude(mean_values - dplyr::lag(mean_values)))
      #probability_distribution <- t(t(cbind(0,probability_distribution))-dplyr::lag(t(cbind(0,probability_distribution))))[,-1]
    #}
  }
...
#writeRaster(rate_raster_cropped ,paste0(iso_code,'_test_2015_2016_2017.tif'))
...
filename<-paste0("final_table",iso_code,"_2016_2019.csv")
