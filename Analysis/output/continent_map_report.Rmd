...
output:
  html_document:
    ...
    toc_float: true
params:
  cholera_directory: "/home/kaiyuezou/mapping_pipeline/2_23_continent_mapping/cholera-mapping-pipeline"
  config_directory: "Analysis/configs/2015_2019_full_base/"
  output_dir: "Analysis/output/2015_2019_full_base/"
  max_countries: Inf
  dropped_years: list("ZWE" = c(2017), "ZNZ" = c(2018, 2019), "SDN" = c(2015, 2017))
---
```
...
#tmp
params <- new.env()
params$cholera_directory <- "/home/kaiyuezou/mapping_pipeline/9_23_dev/cholera-mapping-pipeline"
params$config_directory <- "Analysis/configs/Gavi_reports/Dec_2021_runs"
params$output_dir <- "Analysis/data/true_2021_Dec_runs_output_for_push/cholera-mapping-output"
params$map_output_dir <- "continent_map"
params$max_countries <- Inf
params$dropped_years <- list("ZWE" = c(2017), "ZNZ" = c(2018, 2019), "SDN" = c(2015, 2017)) 
...
for(country in all_countries) {
  ...
  # ### Undecided 
  # config_caches <- lapply(all_configs[[country]], yaml::read_yaml(single_config))
  # country_cache <- new.env()
  # for(config_filename in all_configs[[country]]) {
  #   # do_stuff_with_config
  # } 
  # merge_grid_cases_aggregate(
  #   output_cache = country_cache,
  #   input_caches = config_caches,
  # )

  # country_caches[[country]] <- country_cache
}

### Plotting 
afr_shp <- sf::st_boundary(sf::st_read("/home/kaiyuezou/mapping_pipeline/9_23_dev/cholera-mapping-pipeline/Layers/africa_self_shp/afr_g2014_2013_0.shp"))
shapefilename <- "/home/kaiyuezou/mapping_pipeline/9_23_dev/cholera-mapping-pipeline/Layers/africa_self_shp/afr_g2014_2013_0.shp"

plot_raster_with_fill_with_shp <- function(name, color_scale_type, fill_column, cache, shapefilename) {
    ...
    plot <- ggplot2::ggplot() + ggplot2::geom_sf(data = raster_object, ggplot2::aes_string(fill = fill_column),
        ...
        taxdat::map_theme() + ggplot2::facet_wrap(~t) + geom_sf(data = shp) + coord_sf()
    return(plot)
}

plot_case <- plot_raster_with_fill_with_shp("case_results", "cases", "modeled_cases_mean", final_cache, shapefilename)
pdf("/home/kaiyuezou/mapping_pipeline/2_23_continent_mapping/cholera-mapping-pipeline/Analysis/output/continent_case_testing.pdf")
plot_case
# ggplot() + geom_sf(data = afr_shp) + coord_sf()
# plot_case
# plot(afr_shp)
dev.off()

plot_rate <- plot_raster_with_fill_with_shp("rate_results", "rates", "modeled_rates_mean", final_cache, shapefilename)
pdf("/home/kaiyuezou/mapping_pipeline/2_23_continent_mapping/cholera-mapping-pipeline/Analysis/output/continent_rate_testing.pdf")
plot_rate
dev.off()
