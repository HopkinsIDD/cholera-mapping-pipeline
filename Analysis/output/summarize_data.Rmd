...
for (country in config[["Country.Name"]]) {
  ...
  tryCatch({
    ...
    aggregated_layers <- apply(values(cropped_cases * raster_shp), 2, sum, na.rm = TRUE)
    ...
    raster_shp <- fasterize::fasterize(shapefile, cropped_rates)
    aggregated_layers <- apply(values(cropped_rates * raster_shp), 2, sum, na.rm = TRUE)
...
for (country in config[["Country.Name"]]) {
  if (local_config[["Include"]] == "MultiYearModel") {
    ...
    lancet_cases <- raster::raster(paste(
      params$cholera_mapping_output_directory,
      "lancet_results",
      "lancet_cases_raster.tif",
      sep = "/"
    ...
    cropped_cases <- raster::crop(lancet_cases, shapefile)
    raster_shp <- fasterize::fasterize(shapefile, cropped_cases)
    all_data[["Current"]][["Mean.Cases"]][[country]] <- sum(values(cropped_cases * raster_shp), na.rm = TRUE)
    all_data[["Current"]][["Mean.Cases"]][[country]] <- sum(values(cropped_cases * raster_shp), na.rm = TRUE)
