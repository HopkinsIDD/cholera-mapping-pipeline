...
for (country in config[["Country.Name"]]) {
  if (local_config[["Include"]] == "MultiYearModel") {
    ...
    high_output <-  MCMCvis::MCMCsummary(full_output, params = "grid_cases", func = quantile, probs = .975)
    low_output <-  MCMCvis::MCMCsummary(full_output, params = "grid_cases", func = quantile, probs = .025)
    if (local_config[["Data"]] == "full") {
      ...
      all_data[["Current"]][["025.Quantile"]][[country]] <- sum(low_output[["quantile"]][unique(stan_input$stan_data$map_loc_grid_grid)]) / (length(used_years))
      all_data[["Current"]][["975.Quantile"]][[country]] <- sum(high_output[["quantile"]][unique(stan_input$stan_data$map_loc_grid_grid)]) / (length(used_years))
    }
