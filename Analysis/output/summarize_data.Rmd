...
for (country in config[["Country.Name"]]) {
  ...
  tryCatch({
    ...
    all_data[["Lancet"]][["975.Cases"]][[country]] <- all_data[["Lancet"]][["025.Rates"]][[country]] * pop_by_country[[country]]
...
for (country in config[["Country.Name"]]) {
  ...
  if (local_config[["Include"]] == "MultiYearModel") {
    ...
    if (local_config[["Data"]] == "full") {
      ...
      all_data[["Current"]][["025.Cases"]][[country]] <- sum(low_output[["quantile"]]) / (2019 - 2015 + 1)
      all_data[["Current"]][["975.Cases"]][[country]] <- sum(high_output[["quantile"]]) / (2019 - 2015 + 1)
      ...
      all_data[["Current"]][["025.Cases"]][[country]] <- sum(low_output[["quantile"]][unique(stan_input$stan_data$map_loc_grid_grid)]) / (length(used_years))
      all_data[["Current"]][["975.Cases"]][[country]] <- sum(high_output[["quantile"]][unique(stan_input$stan_data$map_loc_grid_grid)]) / (length(used_years))
    }
    ...
    tryCatch({
      ...
      for (year in years) {
        ...
        all_data[["Current"]][["025.Cases"]][[country]][as.character(year)] <- sum(low_output[["quantile"]], na.rm = TRUE)
        all_data[["Current"]][["975.Cases"]][[country]][as.character(year)] <- sum(high_output[["quantile"]], na.rm = TRUE)
      }
