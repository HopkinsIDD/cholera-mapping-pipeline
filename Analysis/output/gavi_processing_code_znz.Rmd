---
title: "gavi_processing_code_znz"
output: html_document
params: 
  cholera_directory: "C:/IDD/Cholera/gavi_report/"
  config: "SLE.yml"
  old_runs: FALSE
  single_year: FALSE
---
...
get_disaggregated_rate_raster <- function(covar_data_filename,
  ...
  if(params$single_year==FALSE){
    ...
    for (row_idx in unique(obs_years)) {
      ...
      modeled_cases_mean_by_grid_layer_tmp1<-rbind(modeled_cases_mean_by_grid_layer_tmp1,tmp)}
  }else{
    modeled_cases_mean_by_grid_layer_tmp1 <- modeled_cases_mean_by_grid_layer_tmp
  }

#comvert 1000 layers to 5000 layers (time)
  data1=data.frame(modeled_cases_mean_by_grid_layer_tmp1[1:length(unique(rate_raster$geom)),1:1000])
  data2=data.frame(modeled_cases_mean_by_grid_layer_tmp1[(1+length(unique(rate_raster$geom))):(2*length(unique(rate_raster$geom))),1:1000])
  data3=data.frame(modeled_cases_mean_by_grid_layer_tmp1[(1+(2*length(unique(rate_raster$geom)))):(3*length(unique(rate_raster$geom))),1:1000])
  data4=data.frame(modeled_cases_mean_by_grid_layer_tmp1[(1+(3*length(unique(rate_raster$geom)))):(4*length(unique(rate_raster$geom))),1:1000])
  data5=data.frame(modeled_cases_mean_by_grid_layer_tmp1[(1+(4*length(unique(rate_raster$geom)))):(5*length(unique(rate_raster$geom))),1:1000])
  
  modeled_cases_mean_by_grid_layer<-cbind(data1,data2,data3,data4,data5)

  if(!ncol(modeled_cases_mean_by_grid_layer)==5000){
    stop("wrong rate raster processing")
  }
...
# 
# file_names<-list()
# file_names[["stan_output"]] <-"C:/IDD/Cholera/gavi_report/SEL_stan_output.rdata"
# file_names[["covar"]] <- "C:/IDD/Cholera/gavi_report/SLE_1_years_2015-01-01-2019-12-31_20km_suspected.dw-aw-as-ao-cs-cw-ac.covar.rdata"
# file_names[['stan_input']]<-"C:/IDD/Cholera/gavi_report/SLE_1_years_2015-01-01-2019-12-31_20km_suspected.dw-aw-as-ao-cs-cw-ac.pc-sp1-agT-tftN-tfsN.stan_input.rdata"
# file_names[["data"]]<-"C:/IDD/Cholera/gavi_report/SLE_1_years_2015-01-01-2019-12-31_20km_suspected.preprocess.rdata"

#shapefile imported
iso_code <- as.character(stringr::str_extract(params$config, "[A-Z]{3}"))
#shapefile <- rgeoboundaries::gb_adm0(iso_code)
shapefile<- sf::st_read("/home/qulu/cmp_report/Analysis/znz1.shp") 
...
```{r processing}
table<-get_affected_pop(pop_raster_cropped=pop_raster_2017_cropped,
