...
```{r echo = FALSE}
plot_config_comparison_table(cache = comparison_cache)
```
...
```{r caserast, fig.cap=capFig("Modeled cases grid comparison"), fig.height=10, fig.width=10}
plot_modeled_cases_scatter_plot_by_grids_stitched(cache = comparison_cache)
```

**Modeled cases comparison by admin1:**
```{r caserast, fig.cap=capFig("Modeled cases admin comparison"), fig.height=10, fig.width=10}
...
```{r caserast, fig.cap=capFig("Modeled cases observation comparison"), fig.height=25, fig.width=10}
...
```{r echo = FALSE}
plot_cases_by_admin_stitched(cache = comparison_cache, type = "intersperse")
```
**Table 5. Modeled cases difference by admin level and time:**
```{r echo = FALSE}
plot_cases_by_admin_stitched(cache = comparison_cache, type = "difference")
```
