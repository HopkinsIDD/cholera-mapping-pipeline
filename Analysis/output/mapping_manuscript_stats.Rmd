...
print(paste0("Overall, cholera cases were clustered in a relatively small percentage of the continent, affecting in 2016-2020 ", round(sum(mai_grid_pop_1620[mai_grid_pop_1620$mean>1,]$pop)/10^6,0), " million people (",round(sum(mai_grid_pop_1115[mai_grid_pop_1115$mean>1,]$pop)/10^6,0)," in 2011-2015) living in 20 km x 20 km grid cells having more than 1 case/year"))
...
load(here::here(str_c(params$postprocessed_output_directory, "/outbreak_analysis_data.rdata")))
load(here::here(str_c(params$postprocessed_output_directory, "/recent_cholera_outbreaks_res.rdata")))
endemicity <- readRDS(here::here(str_c(params$postprocessed_output_directory, "endemicity_validated.rds")))


# Numbers
n_tot <- nrow(final_joins)
n_adm2 <- nrow(final_joins %>% filter(admin_level > 1))
pct_adm2 <- n_adm2/n_adm2*100

format_pct <- function(x) {formatC(x, digits = 1, format = 'f')}

sustained_low_outbreaks <- final_joins %>% 
  st_drop_geometry() %>% 
...
sustained_low_outbreaks_regions <- sustained_low_outbreaks %>% 
  count(AFRO_region) %>% 
  mutate(frac = n/sum(n))

sustained_low_outbreak_countries <- sustained_low_outbreaks  %>% 
  distinct(country) %>% 
  pull(country)

cat(
  str_glue("
  - Total number of outbreak locations: {n_tot}
    - At ADM2 or lower level:  {n_adm2} ({format_pct(pct_adm2)}%)
    - At ADM1 level:  {n_tot - n_adm2} \n
  - Occurrence:
    - sustained high risk: {with(filter(obs_outbreaks, AFRO_region == 'overall'), n[endemicity == 'sustained high risk'])} ({with(filter(obs_outbreaks, AFRO_region == 'overall'), format_pct(100*frac[endemicity == 'sustained high risk']))}% vs. {with(non_obs_outbreaks, format_pct(100*frac[endemicity == 'sustained high risk']))}% in non-outreabk locations)
    - history of high risk: {with(filter(obs_outbreaks, AFRO_region == 'overall'), n[endemicity == 'history of high risk'])} ({with(filter(obs_outbreaks, AFRO_region == 'overall'), format_pct(100*frac[endemicity == 'history of high risk']))}% vs. {with(filter(non_obs_outbreaks, AFRO_region == 'overall'), format_pct(100*frac[endemicity == 'history of high risk']))}% in non-outreabk locations)
     - sustained low risk: {with(filter(obs_outbreaks, AFRO_region == 'overall'), n[endemicity == 'sustained low risk'])} ({with(filter(obs_outbreaks, AFRO_region == 'overall'), format_pct(100*frac[endemicity == 'sustained low risk']))}% vs. {with(filter(non_obs_outbreaks, AFRO_region == 'overall'), format_pct(100*frac[endemicity == 'sustained low risk']))}% in non-outreabk locations)
     - countries with ADM2 outbreaks in sustained low risk: {length(sustained_low_outbreak_countries)} countries: {str_c(sustained_low_outbreak_countries, collapse = ',')}
     - sustained low risk by region: Southern Africa {with(sustained_low_outbreaks_regions, n[AFRO_region == 'Southern Africa'])} of {sum(sustained_low_outbreaks_regions$n)} ({format_pct(100*with(sustained_low_outbreaks_regions, frac[AFRO_region == 'Southern Africa']))})
             ")
)
...
cat(
  str_glue(
    "
    - Sustained high risk OR:
      - Central Africa: {with(OR_txt, txt[param == 'sustained high risk' & AFRO_region == 'Central Africa'])}
      - Eastern Africa: {with(OR_txt, txt[param == 'sustained high risk' & AFRO_region == 'Eastern Africa'])}
    - ICC sustained high risk ranges: {str_c(formatC(digits = 2, format = 'f', range(ICC_stats$mean[ICC_stats$param == 'sustained high risk'])), collapse = '-')}
    ")
)
```
