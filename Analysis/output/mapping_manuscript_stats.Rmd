...
# sf::st_make_valid(readRDS(paste0(params$postprocessed_output_directory,'/2011_2015_shapefiles.rds')) %>% filter(admin_level >=2))
# shp_adm2_1620 <- sf::st_make_valid(readRDS(paste0(params$postprocessed_output_directory,'/2011_2015_shapefiles.rds')) %>% filter(admin_level >=2))
...
summary_both_high_risk_adm2_1120 <- output_endemicity %>% 
  ungroup() %>% 
  summarise(bothhighrisk_adm2_units = 100 * length(endemicity[endemicity=="high-both"])/n(),
