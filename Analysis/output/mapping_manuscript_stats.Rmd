...
# print out average suspected cases summary excluding ETH and SDN
...
print(
  paste0(
  "Annual average cholera cases across Africa (excluding ETH and SDN) in 2011-2015: ", round(output_mai_cases_all_1115$mean-sum(output_mai_cases_adm2_1115[output_mai_cases_adm2_1115$country %in% c("SDN","ETH") & output_mai_cases_adm2_1115$admin_level == "ADM0",]$mean),0)
  )
)
print(
  paste0(
  "Annual average cholera cases across Africa (excluding ETH and SDN) in 2016-2020: ", round(output_mai_cases_all_1620$mean-sum(output_mai_cases_adm2_1620[output_mai_cases_adm2_1620$country %in% c("SDN","ETH") & output_mai_cases_adm2_1620$admin_level == "ADM0",]$mean),0)
  )
)
...
data.frame(readRDS(paste0(params$postprocessed_output_directory,'/mai_ratio_stats.rds'))) %>%
  filter(admin_level =="ADM0" & !country %in% c("BWA", "ERI", "GAB", "GNQ", "LSO")) %>% #exclude countries with zero case i both periods.
  select(country,mean,q2.5,q97.5) %>% 
  mutate(
    `Mean IRR` = round(mean,2),
    IRR_status = ifelse(q2.5>1,"significant_increase",ifelse(q97.5 < 1,"significant_decrease","non_significant"))
  ) %>%
  group_by(IRR_status) %>% 
  summarize(
    Num_of_countries = n()
  ...
  rbind(
    data.frame(
      IRR_status = "both_zero",
      Num_of_countries = length(c("BWA", "ERI", "GAB", "GNQ", "LSO")))
  ) %>% 
  mutate(
    `Prop (%)` = round(Num_of_countries/43*100,2)
  ) %>% 
    kableExtra::kbl(caption = "Table 12-1. Summary for incidence rate ratio") %>% 
  kableExtra::kable_styling(full_width = FALSE) %>% 
  kableExtra::scroll_box(width = "100%")

# adm2 with significant increase/decrease/total number of adm2
data.frame(readRDS(paste0(params$postprocessed_output_directory,'/mai_ratio_stats.rds'))) %>% 
  dplyr::select(!geom)%>%
  dplyr::filter(admin_level =="ADM2") %>% 
  dplyr::mutate(
    signf_increase = ifelse(
      mean>1 & q2.5>1,
      TRUE, FALSE
    ),
    signf_decrease = ifelse(
      mean<1 & q97.5<1,
      TRUE, FALSE
    )
  ) %>% 
  dplyr::group_by(country) %>% 
  dplyr::summarise(
    "Total number of adm2" = n(),
    "Num of signf increase adm2" = length(signf_increase[signf_increase]),
    "Num of signf decrease adm2" = length(signf_decrease[signf_decrease]),
    "Prop of signf increase adm2 (%)" = round(length(signf_increase[signf_increase])/n()*100,2),
    "Prop of signf decrease adm2 (%)" = round(length(signf_decrease[signf_decrease])/n()*100,2)
  ) %>% 
  dplyr::arrange(desc(`Prop of signf increase adm2 (%)`)) %>% 
  kableExtra::kbl(caption = "Table 13. Adm2 with significant changes in incidence tate tatio between the two periods by country") %>% 
  kableExtra::kable_styling(full_width = FALSE) %>% 
  kableExtra::scroll_box(width = "100%")
...
data.frame(readRDS(paste0(params$postprocessed_output_directory,'/mai_region_ratio_stats.rds'))) %>%
  ...
  kableExtra::kbl(caption = "Table 14. Incidence Rate Ratio between the two periods by region") %>% 
  kableExtra::kable_styling(full_width = FALSE) %>% 
  kableExtra::scroll_box(width = "100%")
...
by=c("country",'admin_level','location_period_id',"shp_id")) %>% 
  ...
  kableExtra::kbl(caption = "Table 15. Population living in ADM2 units with significantly increasing/decreasing incidence rate between the two periods") %>% 
  kableExtra::kable_styling(full_width = FALSE) %>% 
  kableExtra::scroll_box(width = "100%")
...
**Table 18. Population at risk by region. 2011-2015**
...
**Table 19. Population at risk by region. 2016-2020**
...
**Table 21. Population at high risk by region by time period**
...
**Table 22. % of adm2 at high risk by time periods**
```{r Results_paragraph_4_high_risk_by_country, echo=FALSE}
# threshold is 50% ----
## The same object/stats as Figure 4B
output_endemicity_50<-readRDS(paste0(params$postprocessed_output_directory,'/endemicity_50.rds')) 

# % adm2 at high risk by country
output_risk_cat_1115_50 <- readRDS(paste0(params$postprocessed_output_directory,'/2011_2015_risk_categories_50.rds')) %>% 
      st_drop_geometry() %>% 
  dplyr::filter(admin_level == "ADM2") 

output_risk_cat_1620_50 <- readRDS(paste0(params$postprocessed_output_directory,'/2016_2020_risk_categories_50.rds')) %>% 
      st_drop_geometry() %>% 
  dplyr::filter(admin_level == "ADM2") 

table22_A_50<- output_risk_cat_1115_50 %>% 
  dplyr::mutate(high_risk = ifelse(risk_cat %in% c('1-10','<1'),FALSE, TRUE)) %>% 
  dplyr::group_by(country) %>% 
  dplyr::summarise(highrisk_adm2_units_50 = 100 * length(high_risk[high_risk==TRUE])/n(),
                   highrisk_adm2_units_num_50 = length(high_risk[high_risk==TRUE]),
                   total_adm2 = n()) %>% 
  dplyr::mutate(time_period = '2011-2015')
table22_B_50 <-   output_risk_cat_1620_50 %>% 
  dplyr::mutate(high_risk = ifelse(risk_cat %in% c('1-10','<1'),FALSE, TRUE)) %>% 
  dplyr::group_by(country) %>% 
  dplyr::summarise(highrisk_adm2_units_50 = 100 * length(high_risk[high_risk==TRUE])/n(),
                   highrisk_adm2_units_num_50 = length(high_risk[high_risk==TRUE]),
...
# threshold is 95% ----
## The same object/stats as Figure 4B
output_endemicity_95<-readRDS(paste0(params$postprocessed_output_directory,'/endemicity_95.rds')) 

# % adm2 at high risk by country
output_risk_cat_1115_95 <- readRDS(paste0(params$postprocessed_output_directory,'/2011_2015_risk_categories_95.rds')) %>% 
      st_drop_geometry() %>% 
  dplyr::filter(admin_level == "ADM2") 

output_risk_cat_1620_95 <- readRDS(paste0(params$postprocessed_output_directory,'/2016_2020_risk_categories_95.rds')) %>% 
      st_drop_geometry() %>% 
  dplyr::filter(admin_level == "ADM2") 

table22_A_95<- output_risk_cat_1115_95 %>% 
  dplyr::mutate(high_risk = ifelse(risk_cat %in% c('1-10','<1'),FALSE, TRUE)) %>% 
  dplyr::group_by(country) %>% 
  dplyr::summarise(highrisk_adm2_units_95 = 100 * length(high_risk[high_risk==TRUE])/n(),
                   highrisk_adm2_units_num_95 = length(high_risk[high_risk==TRUE]),
                   total_adm2 = n()) %>% 
  dplyr::mutate(time_period = '2011-2015')
table22_B_95 <-   output_risk_cat_1620_95 %>% 
  dplyr::mutate(high_risk = ifelse(risk_cat %in% c('1-10','<1'),FALSE, TRUE)) %>% 
  dplyr::group_by(country) %>% 
  dplyr::summarise(highrisk_adm2_units_95 = 100 * length(high_risk[high_risk==TRUE])/n(),
                   highrisk_adm2_units_num_95 = length(high_risk[high_risk==TRUE]),
...
#LSO is not in this table because we ran models on adm1 of LSO
table22_A<-table22_A_50 %>% 
  dplyr::inner_join(
    table22_A_95,by=c("country","time_period","total_adm2")
  )

table22_B<-table22_B_50 %>% 
  dplyr::inner_join(
    table22_B_95,by=c("country","time_period","total_adm2")
  )

#summary table 22
table22_A %>% 
  dplyr::select(!total_adm2) %>% 
  dplyr::bind_rows(
    data.frame(
    country = "All countries",
    highrisk_adm2_units_50 = round(100 * sum(table22_A$highrisk_adm2_units_num_50)/sum(table22_A$total_adm2),2),
    highrisk_adm2_units_num_50 = sum(table22_A$highrisk_adm2_units_num_50),
    highrisk_adm2_units_95 = round(100 * sum(table22_A$highrisk_adm2_units_num_95)/sum(table22_A$total_adm2),2),
    highrisk_adm2_units_num_95 = sum(table22_A$highrisk_adm2_units_num_95),
    time_period = "2011-2015"
  ))  %>% 
  dplyr::bind_rows(table22_B) %>% 
    dplyr::bind_rows(
    data.frame(
    country = "All countries",
    highrisk_adm2_units_50 = round(100 * sum(table22_B$highrisk_adm2_units_num_50)/sum(table22_B$total_adm2),2),
    highrisk_adm2_units_num_50 = sum(table22_B$highrisk_adm2_units_num_50),
    highrisk_adm2_units_95 = round(100 * sum(table22_B$highrisk_adm2_units_num_95)/sum(table22_B$total_adm2),2),
    highrisk_adm2_units_num_ = sum(table22_B$highrisk_adm2_units_num_50),
    time_period = "2016-2020"
  ))  %>% 
  dplyr::rename(`% of high-risk adm2 units (50%)` = highrisk_adm2_units_50,
                `Number of high-risk adm2 units (50%)` = highrisk_adm2_units_num_50,
                `% of high-risk adm2 units (95%)` = highrisk_adm2_units_95,
                `Number of high-risk adm2 units (95%)` = highrisk_adm2_units_num_95)  %>% 
  dplyr::select(
    time_period, country, `% of high-risk adm2 units (50%)`,`Number of high-risk adm2 units (50%)`,`% of high-risk adm2 units (95%)`,`Number of high-risk adm2 units (95%)`
  ) %>% 
  knitr::kable() %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped")) %>%
    kableExtra::kable_paper(full_width = F)
...
# threshold is 50% ----
summary_pop_risk_adm2_1115_50 <- readRDS(paste0(params$postprocessed_output_directory,'/2011_2015_risk_categories_50.rds')) %>% 
      st_drop_geometry() %>% 
  dplyr::filter(admin_level == "ADM2") %>% 
  dplyr::mutate(high_risk = ifelse(risk_cat %in% c('1-10','<1'),FALSE, TRUE)) %>% 
...
print(paste("Using 50% as the threshold, between 2011-2015, there are", round(summary_pop_risk_adm2_1115_50$highrisk_adm2_units,4),"% of adm2 units in the high risk category (>10 per 100000) among ", round(summary_pop_risk_adm2_1115_50$country_prop,4),"% countries(",summary_pop_risk_adm2_1115_50$total_country,"/43)."))

summary_pop_risk_adm2_1620_50 <- readRDS(paste0(params$postprocessed_output_directory,'/2016_2020_risk_categories_50.rds')) %>% 
      st_drop_geometry() %>% 
  dplyr::filter(admin_level == "ADM2") %>% 
  dplyr::mutate(high_risk = ifelse(risk_cat %in% c('1-10','<1'),FALSE, TRUE)) %>% 
  dplyr::summarise(highrisk_adm2_units = 100 * length(high_risk[high_risk==TRUE])/n(),
...
print(paste("Using 50% as the threshold, between 2016-2020, there are", round(summary_pop_risk_adm2_1620_50$highrisk_adm2_units,4),"% of adm2 units in the high risk category (>10 per 100000) among ", round(summary_pop_risk_adm2_1620_50$country_prop,4),"% countries(",summary_pop_risk_adm2_1620_50$total_country,"/43)."))

# threshold is 95% ----
summary_pop_risk_adm2_1115_95 <- readRDS(paste0(params$postprocessed_output_directory,'/2011_2015_risk_categories_95.rds')) %>% 
      st_drop_geometry() %>% 
  dplyr::filter(admin_level == "ADM2") %>% 
  dplyr::mutate(high_risk = ifelse(risk_cat %in% c('1-10','<1'),FALSE, TRUE)) %>% 
  dplyr::summarise(highrisk_adm2_units = 100 * length(high_risk[high_risk==TRUE])/n(),
...
print(paste("Using 95% as the threshold, between 2011-2015, there are", round(summary_pop_risk_adm2_1115_95$highrisk_adm2_units,4),"% of adm2 units in the high risk category (>10 per 100000) among ", round(summary_pop_risk_adm2_1115_95$country_prop,4),"% countries(",summary_pop_risk_adm2_1115_95$total_country,"/43)."))

summary_pop_risk_adm2_1620_95 <- readRDS(paste0(params$postprocessed_output_directory,'/2016_2020_risk_categories_95.rds')) %>% 
      st_drop_geometry() %>% 
  dplyr::filter(admin_level == "ADM2") %>% 
  dplyr::mutate(high_risk = ifelse(risk_cat %in% c('1-10','<1'),FALSE, TRUE)) %>% 
  dplyr::summarise(highrisk_adm2_units = 100 * length(high_risk[high_risk==TRUE])/n(),
...
print(paste("Using 95% as the threshold, between 2016-2020, there are", round(summary_pop_risk_adm2_1620_95$highrisk_adm2_units,4),"% of adm2 units in the high risk category (>10 per 100000) among ", round(summary_pop_risk_adm2_1620_95$country_prop,4),"% countries(",summary_pop_risk_adm2_1620_95$total_country,"/43)."))
...
## Paragraph 5
**Table 23. Population living and % of adm2 by 10-year risk category**
```{r Results_paragraph_5_pop_living_prop_adm2_by_10-year risk_category, echo=FALSE}
output_endemicity_50 %>% 
  ungroup() %>% 
  group_by(endemicity) %>% 
  summarise(pop_50 = round(sum(pop)/10^6,1),
            adm2_50 = length(unique(location_period_id)),
            adm2_prop_50 = round(100*length(unique(location_period_id))/nrow(output_endemicity_50),2),
            total_adm2_50 = nrow(output_endemicity_50),
            total_country_50 = length(unique(country))) %>% 
  ungroup() %>%
  mutate(pop_prop_50 = round(pop_50/sum(pop_50)*100,1)) %>% 
  dplyr::inner_join(
    output_endemicity_95 %>% 
  ungroup() %>% 
  group_by(endemicity) %>% 
  summarise(pop_95 = round(sum(pop)/10^6,1),
            adm2_95 = length(unique(location_period_id)),
            adm2_prop_95 = round(100*length(unique(location_period_id))/nrow(output_endemicity_95),2),
            total_adm2_95 = nrow(output_endemicity_95),
            total_country_95 = length(unique(country))),
  by=c("endemicity")
  ) %>% 
  ungroup() %>%
  mutate(pop_prop_95 = round(pop_95/sum(pop_95)*100,1)) %>%
  dplyr::rename(`10-year risk cat` = endemicity,
                `Population(million) (50%)` = pop_50,
                `Number of adm2 units (50%)` = adm2_50,
                `Total number of adm2 units (50%)` = total_adm2_50,
                `% of adm2 units (%) (50%)` = adm2_prop_50,
                `Total number of countries (50%)` = total_country_50,
                `% of population (50%)`= pop_prop_50,
                `Population(million) (95%)` = pop_95,
                `Number of adm2 units (95%)` = adm2_95,
                `Total number of adm2 units (95%)` = total_adm2_95,
                `% of adm2 units (%) (95%)` = adm2_prop_95,
                `Total number of countries (95%)` = total_country_95,
                `% of population (95%)`= pop_prop_95)  %>% 
  
  dplyr::select(
    `10-year risk cat`,`Population(million) (50%)`,`% of population (50%)`, `Number of adm2 units (50%)`, `Total number of adm2 units (50%)`,`% of adm2 units (%) (50%)`,`Total number of countries (50%)`,`Population(million) (95%)`,`% of population (95%)`, `Number of adm2 units (95%)`, `Total number of adm2 units (95%)`,`% of adm2 units (%) (95%)`,`Total number of countries (95%)`
  ) %>% 
  knitr::kable() %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped")) %>%
    kableExtra::kable_paper(full_width = F)
...
**Table 24. Population living and % of adm2 by 10-year risk category**
```{r Results_paragraph_5_pop_living_prop_adm2_by_10-year risk_category_by_country, echo=FALSE}
output_endemicity_50 %>% 
  ungroup() %>% 
  dplyr::group_by(country) %>% 
  dplyr::mutate(total_pop = sum(pop)) %>% 
  dplyr::filter(endemicity %in% c("history of high risk","sustained high risk")) %>% 
  dplyr::group_by(country) %>% 
  dplyr::summarise(
    `prop of pop living in history of high risk (%)` = round(sum(pop[endemicity == "history of high risk"])/unique(total_pop)*100,0),
    `prop of pop living in sustained high risk (%)` = round(sum(pop[endemicity == "sustained high risk"])/unique(total_pop)*100,0),
    `prop of pop living in sustained high risk or history of high risk (%)` = round(sum(pop)/unique(total_pop)*100,0)
  ) %>% 
  knitr::kable() %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped")) %>%
    kableExtra::kable_paper(full_width = F)
...
**Table 25. % of adm2 at high risk in both time periods (sustained high cholera risk)**
```{r Results_paragraph_5_high_risk_both_time_periods_by_country, echo=FALSE}
# population living in regions with sustained high cholera risk by country
table25 <- output_endemicity_50 %>% # output endemicity is from endemicity object in 4B
  dplyr::group_by(country) %>% 
  dplyr::summarise(highrisk_adm2_units_50 = 100 * length(endemicity[endemicity=="sustained high risk"])/n(),
                   highrisk_adm2_units_number_50 = length(endemicity[endemicity=="sustained high risk"]),
                   total_adm_units_50 = n()) %>% 
  dplyr::inner_join(
     output_endemicity_95 %>% # output endemicity is from endemicity object in 4B
  dplyr::group_by(country) %>% 
  dplyr::summarise(highrisk_adm2_units_95 = 100 * length(endemicity[endemicity=="sustained high risk"])/n(),
                   highrisk_adm2_units_number_95 = length(endemicity[endemicity=="sustained high risk"]),
                   total_adm_units_95 = n()),
  by=("country")
  ) %>% 
  select(country,highrisk_adm2_units_50,highrisk_adm2_units_number_50,total_adm_units_50,highrisk_adm2_units_95,highrisk_adm2_units_number_95,total_adm_units_95)

table25 %>% 
  bind_rows(
    data.frame(
    country = "All country",
    highrisk_adm2_units_number_50 = sum(table25$highrisk_adm2_units_number_50),
    total_adm_units_50 = sum(table25$total_adm_units_50),
    highrisk_adm2_units_number_95 = sum(table25$highrisk_adm2_units_number_95),
    total_adm_units_95 = sum(table25$total_adm_units_95)
    
    ) %>% 
      mutate(
        highrisk_adm2_units_50 = round(100 * highrisk_adm2_units_number_50/total_adm_units_50,2),
        highrisk_adm2_units_95 = round(100 * highrisk_adm2_units_number_95/total_adm_units_95,2)
      )
  ) %>% 
  dplyr::rename(`% of adm2 units at high-risk in both periods (50%)` = highrisk_adm2_units_50,
                `Number of adm2 units at high-risk in both periods (50%)` = highrisk_adm2_units_number_50,
                `Total number of adm2 units (50%)` = total_adm_units_50,
                `% of adm2 units at high-risk in both periods (95%)` = highrisk_adm2_units_95,
                `Number of adm2 units at high-risk in both periods (95%)` = highrisk_adm2_units_number_95,
                `Total number of adm2 units (95%)` = total_adm_units_95) %>% 
  knitr::kable() %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped")) %>%
    kableExtra::kable_paper(full_width = F)

summary_both_high_risk_adm2_1120_50 <- output_endemicity_50 %>% 
  dplyr::ungroup() %>% 
...
print(paste("Using 50% threshold, there are", round(summary_both_high_risk_adm2_1120_50$bothhighrisk_adm2_units,4),"% of the adm2 units in high risk category (>10 per 100000) for both time periods and they are concentrated in ",round(summary_both_high_risk_adm2_1120_50$country_prop,4),"% (",summary_both_high_risk_adm2_1120_50$total_country,"/43)of the countries."))

summary_both_high_risk_adm2_1120_95 <- output_endemicity_95 %>% 
  dplyr::ungroup() %>% 
  dplyr::summarise(bothhighrisk_adm2_units = 100 * length(endemicity[endemicity=="sustained high risk"])/n(),
                   country_prop = 100 * length(unique(country[endemicity=="sustained high risk"]))/43,
                   total_country = length(unique(country[endemicity=="sustained high risk"])))

print(paste("Using 95% threshold, there are", round(summary_both_high_risk_adm2_1120_95$bothhighrisk_adm2_units,4),"% of the adm2 units in high risk category (>10 per 100000) for both time periods and they are concentrated in ",round(summary_both_high_risk_adm2_1120_95$country_prop,4),"% (",summary_both_high_risk_adm2_1120_95$total_country,"/43)of the countries."))

invisible(gc())

```

```{r Results_paragraph_5_total_high_risk_pop_both_time_periods, echo=FALSE}
load(paste0(params$output_directory,"/data_bundle_for_figures_test.rdata"))
...
risk_pop_50_adm2 <- combine_period_output(prefix_list = prefix_list,
                                          ...
                                          output_dir = params$postprocessed_output_directory) %>% 
...
high_risk_1115<-risk_pop_50_adm2 %>% 
    filter(location_period_id %in% output_endemicity_50[output_endemicity_50$endemicity== "history of high risk",]$location_period_id) %>% 
    filter(period == "2011-2015" & !risk_cat %in% c("<1","1-10"))

  print(paste0("Number of people (millions) living in history of high category due to a high incidence classification in 2011-2015: ",round(sum(output_endemicity_50[output_endemicity_50$location_period_id%in%high_risk_1115$location_period_id,]$pop)/10^6,1)))
  
  high_risk_1620<-risk_pop_50_adm2 %>% 
    filter(location_period_id %in% output_endemicity_50[output_endemicity_50$endemicity== "history of high risk",]$location_period_id) %>% 
    filter(period == "2016-2020" & risk_cat %in% get_risk_cat_dict()[3:6]) 
  
  print(paste0("Number of people (millions) living in history of high category due to a high incidence classification in 2016-2020: ",round(sum(output_endemicity_50[output_endemicity_50$location_period_id%in%high_risk_1620$location_period_id,]$pop)/10^6,2)))
...
```{r cholrea-occurrence-analysis, echo=FALSE,message=FALSE}
load("/home/cholerapipelinerestmp/postprocess/processed_outputs/outbreak_analysis_data.rdata")
...
occurrence_stats %>% 
  kableExtra::kbl(caption = "Table 26. Distribution of 10-year classes in ADM2 or lower locations by observed outbreaks:\n") %>% 
...
obs_outbreaks %>% 
  ...
  mutate(text = str_glue("{n}/{tot} ({formatC(frac * 100, format = 'f', digits = 1)}%)")) %>%
  select(-n, -frac, -tot) %>% 
  kableExtra::kbl(caption = "Table 27. ADM units with outbreaks in sustained low regions\n") %>% 
  kableExtra::kable_styling(full_width = F)
...
final_joins %>% 
  ...
  inner_join(output_endemicity_50 %>% st_drop_geometry()) %>% 
  filter(endemicity == "sustained low risk") %>% 
  distinct(country) %>% 
  get_AFRO_region(ctry_col = "country") %>% 
  kableExtra::kbl(caption = "Table 28. Countries with outbreaks in sustained low regions\n") %>% 
  kableExtra::kable_styling(full_width = F)
```


```{r occurrence-model, echo=FALSE,message=FALSE}
load("/home/cholerapipelinerestmp/postprocess/processed_outputs/recent_cholera_outbreaks_res.rdata")
...
OR_text %>% 
  kableExtra::kbl(caption = "Table 29. Odd ratios (mean) of cholera occurrence by region:\n") %>% 
  kableExtra::kable_styling(full_width = F) 
```

```{r probability of outbreak occurrency-reference group, echo=FALSE,message=FALSE}
baseline_prob_stats %>% 
  ...
  select(AFRO_region,mean,q2.5,q97.5) %>% 
  rename(
    `African region` = AFRO_region,
    `Mean probability` = mean,
    `q2.5 of probability` = q2.5,
    `q97.5 of probability` = q97.5
  ) %>% 
  kableExtra::kbl(caption = "Table 30. Probability (mean) of cholera occurrence for sustained low group by region") %>% 
  kableExtra::kable_paper(full_width = F)
```

```{r ICC, echo=FALSE,message=FALSE}
print(paste0("mean ICC range of sustained high risk category: ",round(min(ICC_stats[ICC_stats$param == 'sustained high risk',]$mean),2),'-',round(max(ICC_stats[ICC_stats$param == 'sustained high risk',]$mean),2)))
```

```{r fig6 related stats, echo=FALSE,message=FALSE}
...
case_frac_sel %>% 
  mutate(`Population targeted (M)` = target_pop/10^6,
         `Proportion reached (mean, 95% Crls)` = paste0(round(mean_diff*100,2)," (",round(100*q025_diff,2),"-",round(q975_diff*100,2),")")) %>%
  select(ranking,`Population targeted (M)`,`Proportion reached (mean, 95% Crls)`) %>% 
  spread(key= ranking,value = `Proportion reached (mean, 95% Crls)`) %>% 
  arrange(`Population targeted (M)`) %>% 
  rename("Realized coverage gap" = `2011-2015` ,
         "Best case coverage gap" = optimal) %>% 
  kableExtra::kbl(caption = "Table 31. Proportion of 2016-2020 cases") %>% 
  kableExtra::kable_paper(full_width = F)
...
pop_frac_sel %>% 
  ...
  spread(key= ranking,value = `Proportion reached (mean, 95% Crls)`) %>% 
  arrange(`Population targeted (M)`) %>% 
  select(`Population targeted (M)`,`2011-2015`,`2016-2020`,`2011-2020`,"optimal") %>%
  kableExtra::kbl(caption = "Table 32. Population living in ADM2 units with cholera occurrence in 2022-2023 reached by different strategies") %>% 
  kableExtra::kable_paper(full_width = F)

pop_frac_sel %>% 
  select(ranking,mean_diff,target_pop) %>% 
  spread(key= ranking,value = mean_diff) %>% 
  mutate(
    `ratio_1115_to_optimal` = round(`2011-2015`/optimal,2),
    `ratio_1120_to_optimal` = round(`2011-2020`/optimal,2),
    `ratio_1620_to_optimal` = round(`2016-2020`/optimal,2)
  ) %>% 
  mutate(`Population targeted (M)` = target_pop/10^6) %>% 
  select(`Population targeted (M)`,`ratio_1115_to_optimal`,`ratio_1620_to_optimal`,`ratio_1120_to_optimal`) %>% 
  kableExtra::kbl(caption = "Table 33. Ratio of population living in ADM2 units with cholera occurrence in 2022-2023 reached by different strategies compared to optimal strategy") %>% 
  kableExtra::kable_paper(full_width = F)
...
case_frac_sel %>% 
  ...
  mutate(`Population targeted (M)` = target_pop/10^6) %>% 
  select(`Population targeted (M)`,`ratio_1115_to_optimal`) %>% 
  kableExtra::kbl(caption = "Table 34. Ratio of 2016-2020 cases reached by different strategies compared to optimal strategy") %>% 
  kableExtra::kable_paper(full_width = F)
```