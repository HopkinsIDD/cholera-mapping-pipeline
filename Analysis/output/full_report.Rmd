...
knitr::opts_chunk$set(include = FALSE, error=TRUE, dev="CairoPNG")
library(stringr)
library(dplyr)
library(magrittr)
library(purrr)
...
all_locations <- stringr::str_extract(all_configs, "(?<=g_)[A-Z]{3}(?=_)")
all_times <- unique(stringr::str_extract(all_configs, "(?<=_)[0-9]{4}_[0-9]{4}(?=\\.)"))
...
for(config_filename in all_configs){
  ...
  if(all(file.exists(file_names))){
    ...
    print(head(sf_grid))
    print(dim(sf_grid))
    print(head(sf_cases_resized))
    print(dim(sf_cases_resized))
...
single_sf_grid <- sf_grid %>% dplyr::group_by(filename, id,type) %>% dplyr::summarize(modeled = mean(modeled), geometry = unique(geom)) %>% dplyr::ungroup()
```
...
rm(single_sf_grid)
chain_sf_grid <- sf_grid %>% dplyr::group_by(filename, id,chain,type) %>% dplyr::summarize(modeled = mean(modeled), geometry = unique(geom)) %>% dplyr::ungroup()
```
...
rm(chain_sf_grid)
time_sf_grid <- sf_grid %>% dplyr::group_by(filename, id,t,type) %>% dplyr::summarize(modeled = mean(modeled), geometry = unique(geom)) %>% dplyr::ungroup()
```
