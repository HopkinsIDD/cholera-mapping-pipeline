...
```{r makenosplit}
single_sf_grid <- sf_grid %>%
  dplyr::group_by(id, type) %>%
  dplyr::summarize(modeled = mean(modeled), geometry = unique(geom)) %>%
  dplyr::ungroup()
```
...
```{r makechainsplit}
rm(single_sf_grid)
chain_sf_grid <- sf_grid %>%
  dplyr::group_by(id, chain, type) %>%
  dplyr::summarize(modeled = mean(modeled), geometry = unique(geom)) %>%
  dplyr::ungroup()
```
...
```{r maketimesplit}
rm(chain_sf_grid)
time_sf_grid <- sf_grid %>%
  dplyr::group_by(id, t, type) %>%
  dplyr::summarize(modeled = mean(modeled), geometry = unique(geom)) %>%
  dplyr::ungroup()
```
