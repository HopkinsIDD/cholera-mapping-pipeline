...
params:
  config: "/Analysis/configs/Dec_2021_runs/TZA_mainland/config_TZA_mainland_2015.yml"
  ...
  daily_period_obs_disagree_threshold: '50'
  report_testing: 'FALSE'
  fake_OCs: '3'
  fake_OCs_daily: '1'
  
---
### This is a short report to plot data of different observation lengths for TZA mainland.
...
for (locations in unique(raw_data$location_name)){
  for (years in unique(raw_data$year)){
    if (sum(raw_data[raw_data$location_name == locations 
...
```{r echo=FALSE, fig.height=as.numeric(plot2_height_factor), fig.width=10}
...
temp_df <- raw_data_filtered %>% 
  ...
  summarise(n = n())
if (sum(temp_df$n) > nrow(temp_df)){print('Warning: some daily observations from the same OC do not agree with with each other, but did not get plotted in the following figure. ')}
...
raw_data_filtered_dis_period_stack %>% 
  ...
  facet_wrap( ~ location_name + dis_period + year, ncol = 2, scales = "free") +
  theme_bw()
