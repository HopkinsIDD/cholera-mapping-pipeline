...
params:
  ...
  local_output_dir: "/home/.../cholera-mapping-output"
...
if (!is.null(params$local_output_dir)) {
...
taxonomy_data <- taxdat::pull_taxonomy_data(
  username = taxonomy_username, password = taxonomy_password,
...
if (!is.null(params$OC_filter) & (nchar(params$OC_filter) > 0 & !identical(params$OC_filter, " "))) {
  OC_vector <- as.character(as.numeric(strsplit(params$OC_filter, ",")[[1]]))
...
if (exists("OC_vector")) {
  OC_vector_for_table <- OC_vector
} else {
  OC_vector_for_table <- "ALL"
}

params_setting <- tibble::tibble(
  params_name = c(
    "Whether filtered out zeros: ",
  ...
  params_value = c(
    filter_zeros, cut_across_year_observations,
...
## Table 3. Summary table for the difference between pre-processed dataset and the dataset directly drawn from taxonomy
```{r echo=FALSE, warning=FALSE}
...
sum_obs %>%
  kableExtra::kable(col.names = c(
    "Year", "#dropped observations", "#dropped cases", "#dropped location-periods",
...
if (exists("OC_vector")) {
  raw_data <- subset(raw_data, as.character(OC_UID) %in% OC_vector)
}
  ...
  for (locations in unique(raw_data$location_name)) {
    for (years in unique(raw_data$year)) {
      if (length(raw_data[raw_data$location_name == locations &
        raw_data$year == years, ]$suspected_cases) == 0) {} else if (sum(raw_data[raw_data$location_name == locations &
        raw_data$year == years, ]$suspected_cases) == 0) {
        raw_data <- raw_data[-which(raw_data$location_name == locations &
          raw_data$year == years), ]
        if (!exists("plot1_ncol_factor")) {
          plot1_ncol_factor <- 1
        }
      }
    }
...
if (any(raw_across_year_data_temp$TL_year != raw_across_year_data_temp$TR_year)) {
  ...
  if (cut_across_year_observations) {
    raw_across_year_data_cut <- raw_data %>%
      ...
      mutate(
        first_year = as.numeric(year == TL_year),
      ...
      mutate(time_fraction = case_when(
        first_year == 1 ~ (year_length - abs(as.numeric(difftime(TL, paste0(year, "-01-01"), units = "days")))) / period_length,
  ...
  for (i in seq(fake_OCs)) {
    ...
    for (locations in unique(raw_data_filtered$location_name)) {
      for (years in unique(raw_data_filtered$year)) {
        ...
        raw_data_filtered <- raw_data_filtered %>%
          mutate(
            TL = case_when(
              OC_UID == OC_UID_temp & TL > TR ~ TR,
              TRUE ~ TL
            ),
            TR = case_when(
              OC_UID == OC_UID_temp & TL >= TR ~ sample(
                seq(as.Date(raw_data_filtered[raw_data_filtered$OC_UID == OC_UID_temp & raw_data_filtered$location_name == locations & raw_data_filtered$year == years, ]$TL),
    ...
    if (i == random_number) {
      ...
      raw_data_filtered <- raw_data_filtered %>%
        ungroup() %>%
        add_row(
          OC_UID = rep(OC_UID_temp, length(sampled_indices)),
...
```{r echo=FALSE}
plot1pre_ncol_factor <- 1
plot1pre_height_factor <- 5 * length(unique(raw_data_filtered$location_name)) / plot1pre_ncol_factor

if (period_OC_divided_by_timelength) {
  print("The rectangles represent daily-averaged cases (total cases/length of period). ")
} else {
  print("The rectangles represent total cases over periods. ")
}
```

```{r echo=FALSE, fig.height=as.numeric(plot1pre_height_factor), fig.width=10}
if ((nrow(raw_data[raw_data$TL != raw_data$TR, ]) == 0 |
  nrow(raw_data[raw_data$TL == raw_data$TR, ]) == 0)) {
  if (nrow(raw_data[raw_data$TL != raw_data$TR, ]) == 0) {
    raw_data_filtered %>%
      filter(TR == TL) %>%
      ggplot(colour = OC_UID) +
      ...
      facet_wrap(~location_name, ncol = as.integer(plot1pre_ncol_factor), scales = "free") +
      theme_bw()
    ...
    if (period_OC_divided_by_timelength) {
      raw_data_filtered %>%
        filter(TL != TR) %>%
        ggplot(aes(
          x = ymd(TL), y = suspected_cases / (time_length(interval(TL, TR), "day") + 1),
          color = OC_UID
        )) +
        geom_rect(aes(
          xmin = TL, xmax = TR, ymin = 0,
          ymax = suspected_cases / (time_length(interval(TL, TR), "day") + 1),
          colour = OC_UID
        ),
        fill = NA, size = 0.35 * plot1pre_ncol_factor, alpha = 0.3,
        stat = "identity"
        ) +
        scale_x_date(date_labels = "%m-%Y") +
  ...
  if (period_OC_divided_by_timelength) {
    raw_data_filtered %>%
      ...
      geom_segment(
        data = (raw_data_filtered %>% filter(TR == TL)),
...
## Fig 2. Overall figure -- all observations by location AND year
**This comprehensive figure includes all (or specified) pre-processed observations. **
**The daily-based observations are represented in vertical lines **
**The period-based observations are represented in rectangles **
```{r echo=FALSE}
plot1_ncol_factor <- 2 # for now

plot1_height_factor <- 3 * nrow(unique(raw_data_filtered[, c("location_name", "year")])) / plot1_ncol_factor # fixed on 2/17
if (across_year_indicator) {
  plot1_height_factor_2 <- 3 * length(unique(raw_across_year_data_filtered[c("location_name", "year"), ])) / plot1_ncol_factor
} else {
  plot1_height_factor_2 <- 10
} # 10 won't be used
...
## Fig 3. Overall figure -- all pre-processed observations that span across multiple years by location
**The daily-based observations are represented in vertical lines **
**The period-based observations are represented in rectangles **
...
## Fig 4. Overall figure -- cumulative cases by location AND year
**This comprehensive figure includes all (or specified) pre-processed observations. **
...
## Fig 5. Disagreement figure -- daily incidence v.s daily incidence from different OC
      ...
      if (is.null(raw_data_filtered_dis_daily_temp1) | is.null(raw_data_filtered_dis_daily_temp2)) {
        ...
        for (TLs in unique(TLsTotal)) {
          if (TLs %in% TLs1 & TLs %in% TLs2) {
            raw_data_filtered_dis_daily_stack <- rbind(
            ...
            raw_data_filtered_dis_daily_stack[
              raw_data_filtered_dis_daily_stack$location_name == locations &
                ...
                raw_data_filtered_dis_daily_stack$TL == TLs,
            ]$oc2 <-
...
raw_data_filtered_dis_daily_stack <- raw_data_filtered_dis_daily_stack %>%
  ...
  rowwise() %>%
  mutate(
    lower_oc = min(oc1, oc2),
    higher_oc = max(oc1, oc2)
  )

raw_data_filtered_dis_daily_stack_temp <- raw_data_filtered_dis_daily_stack
plot3_height_factor <- (3 * nrow(unique(raw_data_filtered_dis_daily_stack_temp[c(
  "location_name",
  "year",
  "groups_compared"
)]))
...
raw_data_filtered_dis_daily_stack %>%
  ...
  geom_point(aes(x = ymd(TL), y = oc1),
    color = rgb(0.2, 0.7, 0.1, 0.5),
    size = 0.7, alpha = 1, shape = 23
  ) +
  geom_point(aes(x = ymd(TL), y = oc2),
...
## Fig 6. Disagreement figure -- daily incidence v.s daily incidence from the same OC
...
raw_data_filtered_dis_daily_same <- raw_data_filtered_dis_daily %>%
  ...
  summarise(
    duplicate = n(), lower_oc = min(suspected_cases, na.rm = TRUE),
...
raw_data_filtered_dis_daily_same %>%
  ...
  geom_point(aes(x = ymd(TL), y = lower_oc),
    color = rgb(0.2, 0.7, 0.1, 0.5),
    size = 0.7, alpha = 1, shape = 23
  ) +
  geom_point(aes(x = ymd(TL), y = higher_oc),
    color = rgb(0.7, 0.2, 0.1, 0.5),
    size = 0.7, alpha = 1, shape = 23
  ) +
...
## Fig 7. Disagreement figure -- number of cases over periods v.s daily incidence
...
raw_data_filtered_dis_period <- raw_data_filtered %>%
  ...
  mutate(
    period_token = case_when(
      TL != TR ~ 1,
      TRUE ~ 0
    ),
    daily_token = case_when(
      TL == TR ~ 1,
      TRUE ~ 0
    ),
    dis_period = 0,
    ...
    for (ocids in unique(raw_data_filtered_dis_period$OC_UID)) {
      if (any(raw_data_filtered_dis_period[
        ...
        for (h in 1:length(TL)) {
          ...
          if (min(extract_cases, na.rm = TRUE) != max(extract_cases, na.rm = TRUE)) {
            warning("The extract_cases has more than one different elements, only the first one is used. ")
          }
            ...
            if (any(raw_data_filtered_dis_period[
              ...
              if (sum_cases != extract_cases & abs(sum_cases - extract_cases) >= daily_period_obs_disagree_threshold) {
                ...
                new_df2 <- raw_data_filtered_dis_period[
                  ...
                  mutate(dis_period = paste0("period_from_", ocids, "_daily_from_", ocids_2)) %>%
                  mutate(
                    mean_daily = mean(suspected_cases),
...
raw_data_filtered_dis_period_stack %>%
  ...
  geom_segment(
    data = (raw_data_filtered_dis_period_stack %>% filter(TR == TL)),
    aes(x = ymd(TL), xend = ymd(TL), y = 0, yend = suspected_cases),
    size = 0.7, alpha = 0.7
  ) +
  geom_point(
...
## Fig 8. Stan input data -- number of cases by OC_UID over periods and locations
