...
if(!is.null(cache[["config"]][["test_metadata"]])){
 params_table=data.frame(do.call(rbind,cache[["config"]][["test_metadata"]]))
 ...
 data.frame(parameter=rownames(params_table)[c(1:10,20:23)],params_table[c(1:10,20:23),1])%>%
...
``` {r echo=FALSE}
if(!is.null(cache[["config"]][["test_metadata"]])){
   covar=data.frame(t(params_table[c(11:19),]))
...
``` {r covars_data_simulation, fig.height=5, fig.width=10, fig.cap=capFig("Non-population covariate rasters in data simulation process")}
