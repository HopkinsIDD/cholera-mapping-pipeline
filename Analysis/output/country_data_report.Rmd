...
params: 
  cholera_directory: "/home/kaiyuezou/mapping_pipeline/country_data_report_updates/cholera-mapping-pipeline"
  config: "/Analysis/configs/production_tests/country_data_report_comparison_test/BEN/config_BEN_2015_2019_nocovar.yml"
...
params <- new.env()
params$cholera_directory <- "/home/kaiyuezou/mapping_pipeline/country_data_report_updates/cholera-mapping-pipeline"
params$config <- "/Analysis/configs/production_tests/country_data_report_comparison_test/BEN/config_BEN_2015_2019_nocovar.yml"
...
# The updates to make
##scatterplot of grid-cell GAM model output and Stan model output
##scatterplot of observation GAM model output and Stan model output
##also GAM input v.s GAM output
##remember to use the stitch functions 
...
**GAM Output v.s GAM Input Rates Scatterplot:**
```{r gam output input rates comparison,fig.height=5, fig.width=10, fig.cap=capFig("GAM Output v.s GAM Input Rates")}
stitch_gam_input_with_gam_output_rate(output_cache = cache, input_caches = list(cache$initial_values_data, cache$gam_output_df))
plot_gam_input_vs_output_rate_scatter_plot(cache = cache) #this will output plots 
```
...
#need to use the fidelity plotting functions because the scales are settled there 
...
plot_stan_output_vs_gam_output_rate_scatter_plot(cache = cache) #this will output plots 
```

**Stan Output v.s GAM Output Cases by Time Slices:**
```{r stangamcates, fig.cap=capFig("Stan Output v.s GAM Output Cates"), fig.height=5, fig.width=10}
#not done yet 
```
