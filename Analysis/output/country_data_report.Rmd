...
params:
  ...
  config: "/Analysis/configs/Dec_2021_runs/ZWE_2015_2019/config_ZWE_2015_2019.yml"
...
if(nrow(sf_grid) == prod(dim(pop_layer))){
  ...
  for (ts in unique(pltdata$t)){
    ...
    for (locs in admin_pop_table$adminlevel){
      cropped <- raster::crop(raster_data, boundary_sf[boundary_sf$shapeName == locs, ], snap = "near")#change from out to near (qz)
