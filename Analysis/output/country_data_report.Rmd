...
if (!is.null(cache[["config"]][["test_metadata"]])) {
  ...
  return(NULL)
  warning("Production runs don't have the true distribution of cases at the grid level.")
}
```
...
### Error Tables
```{r echo=FALSE }
plot_error_by_grid(cache=cache,cholera_directory = params$cholera_directory,config=params$config)
```

```{r echo=FALSE}
plot_error_by_grid_spatial_scale(cache=cache,cholera_directory = params$cholera_directory,config=params$config)
```
...
**w distribution**
```{r w_distribution,fig.height=8,fig.width=10,fig.cap=capFig("Distribution of mean w across chains and iterations")}
...
```{r gam output rates versus gam input rates,fig.height=5, fig.width=10, fig.cap = capFig("GAM input rates versus GAM output rates")}
taxdat::plot_gam_input_output_scatter(config = params$config, cholera_directory = params$cholera_directory, cache = cache)
``` 

```{r, results="asis", echo=FALSE}
if (!is.null(cache[["config"]][["test_metadata"]])) {
  cat("### True underlying distribution maps", "\n", "#### True grid cases raster by time slices:")
...
```{r, results="asis", echo=FALSE}
if (!is.null(cache[["config"]][["test_metadata"]])) {
  cat("#### True grid rate raster by time slices:")
