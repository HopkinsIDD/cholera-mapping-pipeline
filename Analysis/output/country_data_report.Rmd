...
pop_raster_data <- raster(paste0(params$cholera_directory,"/Layers/pop/2000_1km_Aggregated.tif"))##update to temporal-varying population (1*1KM)
...
```{r importcase}
case_raster <- NULL
case_raster <- taxdat::get_case_raster(covar_data_filename = file_names["covar"],
                                       genquant_filenames = file_names["stan_genquant"])
...
if(!is.null(case_raster)){
  plot_disaggregated_modeled_cases(case_raster=case_raster,disaggregated_case_sf=disaggregated_case_sf)
} else{
...
if(!is.null(sf_cases_resized) & !is.null(model.rand)){
  ...
  chains <- data_fidelity[[1]] %>% group_by(variable) %>% summarise(modeled = mean(`modeled cases`))
  who_annual_cases$modeled <- chains$`modeled cases`
