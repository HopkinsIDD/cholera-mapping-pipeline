...
library(package = "taxdat", character.only = T, lib = "/home/kzou7/rlibs/cmp/4.0.2/gcc/9.3.0/") #tmp
library(package = "cmdstanr", character.only = T, lib = "/home/kzou7/rlibs/cmp/4.0.2/gcc/9.3.0/") #tmp
library(package = "Cairo", character.only = T) #tmp
sf::sf_use_s2(FALSE)
...
plot_disaggregated_modeled_cases_time_varying(config_file=config_file, disaggregated_case_sf=cache[["disaggregated_case_sf"]], add_shp_from_stan_input = T)
```
...
plot_modeled_rates_time_varying(config_file=config_file, disaggregated_rate_sf=cache[["disaggregated_rate_sf"]], add_shp_from_stan_input = T, render = T)
```
...
**Table 5.1 Observed and estimates of WHO annual cholera reports based on the observation-level modeled cases:**
```{r WHO output1, fig.height=510, fig.width=10, fig.cap = "comparison with WHO Output 1"}
plot_WHOcomparison_table(config=params$config, cache=cache, cholera_directory=params$cholera_directory, 
  allow_data_pull = !run_on_marcc)
```
**Table 5.2 Observed and estimates of WHO annual cholera reports based on the sum of grid-level modeled cases:**
```{r WHO output2, fig.height=510, fig.width=10, fig.cap = "comparison with WHO Output 2"}
plot_WHOcomparison_table(config=params$config, cache=cache, cholera_directory=params$cholera_directory, 
  observation_level_modeled_cases = FALSE, allow_data_pull = !run_on_marcc)
```
**Table 5.3 Observed and estimates of WHO annual cholera reports based on the observation-level modeled cases with other sources of annual data added:**
```{r WHO output3, fig.height=510, fig.width=10, fig.cap = "comparison with WHO Output 3"}
plot_WHOcomparison_table(config=params$config, cache=cache, cholera_directory=params$cholera_directory, 
  allow_data_pull = !run_on_marcc, add_other_source = TRUE)
```
**Table 5.4 Observed and estimates of WHO annual cholera reports based on the sum of grid-level modeled cases with other sources of annual data added:**
```{r WHO output4, fig.height=510, fig.width=10, fig.cap = "comparison with WHO Output 4"}
plot_WHOcomparison_table(config=params$config, cache=cache, cholera_directory=params$cholera_directory, observation_level_modeled_cases = FALSE, 
  allow_data_pull = !run_on_marcc, add_other_source = TRUE)
```
