---
title: "gavi_processing_code_single_year"
...
get_disaggregated_rate_raster <- function(covar_data_filename,
  ...
  covar_cube_output_2019 <- read_file_of_type(covar_data_filename, "covar_cube_output")
  rate_raster_2019 <- covar_cube_output_2019$sf_grid
  non_na_gridcells_2019 <- taxdat::get_non_na_gridcells(covar_data_filename)
  rate_raster_2019 <- rate_raster_2019[non_na_gridcells_2019,]
  model.rand_2019 <- read_file_of_type(model_output_filenames, "model.rand")
  
  # average modeled cases across chains
  modeled_cases_2019  <- as.array(model.rand_2019 )[, , grepl("grid_case", names(model.rand_2019 )),drop=FALSE]
  modeled_cases_mean_by_grid_layer_tmp_2019 <- as.data.frame(t(apply(modeled_cases_2019, c(1,3), mean)))%>%
  mutate(id=seq_len(dim(modeled_cases_2019)[3]))
  stan_input_2019 <- read_file_of_type(stan_input_filenames, "stan_input")

  modeled_cases_mean_by_grid_layer_2019<-modeled_cases_mean_by_grid_layer_tmp_2019
  pop_sf_grid_2019<-stan_input_2019$sf_grid



############################import znz##########################
covar_cube_output_znz <- read_file_of_type("/home/qulu/cmp_report/Analysis/data/Zanzibar_pxY_1_years_2015-01-01-2019-12-31_20km_suspected.dw.covar.rdata", "covar_cube_output")
  rate_raster_znz <- covar_cube_output_znz$sf_grid
  non_na_gridcells_znz <- taxdat::get_non_na_gridcells("/home/qulu/cmp_report/Analysis/data/Zanzibar_pxY_1_years_2015-01-01-2019-12-31_20km_suspected.dw.covar.rdata")
  rate_raster_znz <- rate_raster_znz[non_na_gridcells,]
  model.rand_znz <- read_file_of_type("/home/qulu/cmp_report/Analysis/data/ZNZ_stan_output.rdata", "model.rand")
  
  # average modeled cases across chains
  modeled_cases_znz <- as.array(model.rand_znz)[, , grepl("grid_case", names(model.rand_znz)),drop=FALSE]
  modeled_cases_mean_by_grid_layer_znz <- as.data.frame(t(apply(modeled_cases_znz, c(1,3), mean)))%>%
  mutate(id=seq_len(dim(modeled_cases_znz)[3]))
  stan_input_znz<- read_file_of_type("/home/qulu/cmp_report/Analysis/data/Zanzibar_pxY_1_years_2015-01-01-2019-12-31_20km_suspected.dw.pc-sp1-agT-tftN-tfsN.stan_input.rdata", "stan_input")


  #head(modeled_cases_mean_by_grid_layer_znz[19:24,])
  #modeled_cases_mean_by_grid_layer_znz[25:30,]<-0

  #2015
  #modeled_cases_mean_by_grid_layer<-rbind(  modeled_cases_mean_by_grid_layer,  modeled_cases_mean_by_grid_layer_znz[1:6,])
  #2016
  #modeled_cases_mean_by_grid_layer<-rbind(  modeled_cases_mean_by_grid_layer,  modeled_cases_mean_by_grid_layer_znz[7:12,])

  #2017
  #modeled_cases_mean_by_grid_layer<-rbind(  modeled_cases_mean_by_grid_layer,  modeled_cases_mean_by_grid_layer_znz[13:18,])
  
  #2018
  #modeled_cases_mean_by_grid_layer_znz[19:24,]<-0
  #modeled_cases_mean_by_grid_layer<-rbind(  modeled_cases_mean_by_grid_layer,  modeled_cases_mean_by_grid_layer_znz[19:24,])
  
  #2019
  modeled_cases_mean_by_grid_layer_znz[25:30,]<-0
  modeled_cases_mean_by_grid_layer<-rbind(  modeled_cases_mean_by_grid_layer,  modeled_cases_mean_by_grid_layer_znz[25:30,])
  modeled_cases_mean_by_grid_layer$id<-1:nrow( modeled_cases_mean_by_grid_layer)


  modeled_cases_mean_by_grid_layer_2019<-rbind(  modeled_cases_mean_by_grid_layer_2019,  modeled_cases_mean_by_grid_layer_znz[25:30,])
  modeled_cases_mean_by_grid_layer_2019$id<-1:nrow(  modeled_cases_mean_by_grid_layer_2019)
 
  pop_sf_grid_znz<-stan_input_znz$sf_grid[1:(nrow(stan_input_znz$sf_grid)/5),]
   pop_sf_grid<-rbind(pop_sf_grid,pop_sf_grid_znz)
   pop_sf_grid$id<-1:nrow( pop_sf_grid)

   pop_sf_grid_2019<-rbind(pop_sf_grid_2019,pop_sf_grid_znz)
   pop_sf_grid_2019$id<-1:nrow( pop_sf_grid_2019)
#################################################################
...
##2019######################################
 pop_sf_grid_2017_2019<-get_pop2017(pop_sf_grid_2019)
  
  #combine the population estimates into case raster
  modeled_cases_mean_by_grid_layer_2019$pop2017<-pop_sf_grid_2017_2019[1:nrow(modeled_cases_mean_by_grid_layer_2019),]$pop2017

  modeled_rates_mean_by_grid_layer_2019<-  modeled_cases_mean_by_grid_layer_2019
  modeled_rates_mean_by_grid_layer_2019[str_detect(colnames(modeled_rates_mean_by_grid_layer_2019),".*[0-9].*")] <- modeled_cases_mean_by_grid_layer_2019[str_detect(colnames(modeled_cases_mean_by_grid_layer_2019),".*[0-9].*")]/modeled_cases_mean_by_grid_layer_2019$pop2017
  modeled_rates_mean_by_grid_layer_2019$id=1:nrow(modeled_rates_mean_by_grid_layer_2019)
  rate_raster_2019 <- merge(rate_raster_2019[1:length(unique(rate_raster_2019$geom)),],modeled_rates_mean_by_grid_layer_2019,by="id")

  colnames(rate_raster_2019)[str_detect(colnames(rate_raster_2019),".*[0-9].*")&!colnames(rate_raster_2019)%in%"pop2017"] <- paste0("layer",seq_len(5*dim(modeled_cases_2019)[1]))

  rate_raster_2019 <- rate_raster_2019[,str_detect(colnames(rate_raster_2019),".*[0-9].*")&!colnames(rate_raster_2019)%in%"pop2017"]

for(row_idx in 1:nrow(rate_raster)){
print(row_idx)
#row_idx=109
  if(rate_raster[row_idx,]$geometry %in% unique(rate_raster_2019$geometry)){
  rate_raster[row_idx,]= rate_raster_2019[which(rate_raster_2019$geometry==rate_raster[row_idx,]$geometry),]
print("yes")
} else{
   rate_raster[row_idx,1:1000]=0
print("no")
}
}

######################################
  raster_2020<-raster::raster(rate_raster, res =c(0.1666667,0.1666667)) 
  
 for (layer_idx in seq_len(ncol(rate_raster)-1)){
 #for (layer_idx in 1:20){

print(paste0("2019_",layer_idx))
    layer_value <- rate_raster[,layer_idx]
     
    #empty raster
    single_layer <- raster::raster(rate_raster, res =c(0.1666667,0.1666667))
  ...
  #names(raster_2020)<-colnames(rate_raster)[1:20]
...
#shapefile <-sf::st_read("/home/qulu/cmp_report/Analysis/output/tza_non_znz.shp")
...
#rate raster
rate_raster_2020 <- get_disaggregated_rate_raster(covar_data_filename=file_names[["covar"]],
                                                           model_output_filenames=file_names[["stan_output"]],
                                                           stan_input_filenames=file_names[['stan_input']],
                                                           if_single_year_run=params$single_year
                                                           )
...
```{r processing}
results_by_layer <- list()
for (layer_idx in seq_len(nlayers(rate_raster_2020_cropped))) {
    results_by_layer[[layer_idx]] <- aggregate_affected_pop_across_cells(pop_raster_cropped=pop_raster_2017_cropped,
                                                             rate_raster_cropped=rate_raster_2020_cropped[[layer_idx]],
                                                             shapefile=shapefile,
                                                             threshold_list=c(0.001,0.0001,0.00001))
...
filename<-paste0("/home/qulu/cmp_report/Analysis/output/pop_single_year_",iso_code,year_list,"_with_znz.csv")
#filename<-paste0("/home/qulu/cmp_report/Analysis/output/pop_single_year_",iso_code,year_list,".csv")
filename
