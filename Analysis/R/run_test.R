...
## Pull data frames needed to create testing database from the api
## This doesn't pull covariates, but does pull everything else
...
# Create 3 layers of testing polygons
# starting with a single country, and splitting each polygon into 4 sub-polygons
...
raster_df <- lapply(
  ...
  function(x) {
    tibble::tibble(name = x$name, start_date = x$start_date, end_date = x$end_date, covar = list(x$fun()))
  }
) %>%
  do.call(what = bind_rows) %>%
  dplyr::group_by(name) %>%
  summarize(
    covar = list(do.call(what = bind_rows, mapply(
      SIMPLIFY = FALSE,
      covar,
      start_date,
      FUN = function(covar, time_left) {
