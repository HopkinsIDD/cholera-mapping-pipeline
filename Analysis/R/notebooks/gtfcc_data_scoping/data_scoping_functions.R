...
get_geo_name <- function(lvl) {
  ...
  if (lvl == 99) return('Level ?')
  if (lvl == 0) return('Country-level')
  if (lvl == 1) return('Level 1')
  if (lvl >= 2 & lvl < 99) return('Level 2+')
}
...
prettier_table <- function(data) {
  data %>% 
    ...
    kable_styling(bootstrap_options = c('bordered', 'condensed'), fixed_thead = TRUE,
...
pretty_graph <- function(data, colors, color_names) {
  ...
  ggplot(data=data, aes(x=region, y=proportion, fill=data_type)) +
    ...
    xlab('Region') + ylab('Proportion of observations')
}
...
data_map <- function(dt_plot, ad, indicator, indicator_name) {
  ...
  myplot <- 
    ...
    if (grepl('prop', indicator)) {
      ...
      scale_fill_viridis_c(indicator_name, na.value = 'grey80', 
