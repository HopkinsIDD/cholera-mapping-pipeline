...
for (i in 1:nrow(cov_meta)) {
  dsn <- glue::glue("PG:dbname='cholera_covariates' host=localhost user='{thisuser}' port=5432 schema='covariates' table='{cov_meta$covariate[i]}' mode=2")
  ras <- try(readGDAL(dsn)) # Get your file as SpatialGridDataFrame
  if (inherits(ras, "try-error")) {
    cat("Raster layer not found for '", cov_meta$covariate[i], "' in database")
    cov_meta <- cov_meta[-i, ]
  } else {
    outrast <- brick(ras)
    cov_rasters[[i]] <- outrast[[1]]
  }
}
...
for (cov in unique(cov_meta$cov)) {
  ...
  for (res_x in unique(cov_res_x)) {
    ...
    if (is.na(unique(cov_res_t[cov_res_x == res_x]))) {
      p1 <- function() {
        ...
        plot(rast, main = "Static")
      }
      rast <- cov_plots[[cov_res_x == res_x]]
      p_res_time[[k]] <- cowplot::ggdraw(p1) 
      rm("p1")
    } else {
      for (res_time in unique(cov_res_t[cov_res_x == res_x])) {
        # cat(res_time, "\n")
        p1 <- function() {
          par(
            mar = c(3, 3, 1, 1),
          ...
          plot(rast, main = res_time)}
        if (sum(cov_res_x == res_x & cov_res_t == res_time) > 0) {
          rast <- cov_plots[[cov_res_x == res_x & cov_res_t == res_time]]
        } else {
          break()
        }
        p_res_time[[k]] <- cowplot::ggdraw(p1) 
    ...
    cov_plot_res_x[[j]] <- cowplot::plot_grid(
      plotlist = list(cov_title_res_x, 
                      cowplot::plot_grid(plotlist = p_res_time, 
  ...
  print(cowplot::plot_grid(
    plotlist = list(cov_title,
                    cowplot::plot_grid(plotlist = cov_plot_res_x,
                                       ncol = 1)),
    ncol = 1,
    rel_heights = c(.1, 1)))
  l <- l + 1
