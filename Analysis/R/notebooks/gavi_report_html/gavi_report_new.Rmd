...
```{r readdata}
csv_dir <- "/Users/kaiyuezou/Desktop/gavi_report/"

bad <- read_csv(paste0(csv_dir, "gavi_summary_bad.csv")) %>% ## lancet numbers are wrong
...
new_who <- read_csv(paste0(csv_dir, "who_2010_2019_cases_rates.csv"))
bad$WHO_estimate <- unlist(lapply(unique(bad$country), function(country_code){
  ...
  #bad[bad$country==country_code, ]$WHO_estimate <- new_estiamte
  return(new_estimate)
}))
...
#tza_m_index <- match("TZA", gavi$country)
#gavi[tza_m_index, c("country")] <- "TZA_M"
#gavi <- gavi %>% 
#  add_row(country = "TZA") %>% 
#  arrange(country)
#gavi[gavi$country == "TZA", ]$current_cases_mean <- gavi[gavi$country == "TZA_M", ]$current_cases_mean + 
#                                                    gavi[gavi$country == "ZNZ", ]$current_cases_mean
#TZA_2017_pop <- as.numeric(tidyr::world_bank_pop[tidyr::world_bank_pop$country == "TZA" & 
#                                                 tidyr::world_bank_pop$indicator == "SP.POP.TOTL", c("2017")])
#gavi[gavi$country == "TZA", ]$current_rates_mean <- gavi[gavi$country == "TZA", ]$current_cases_mean / TZA_2017_pop

#gavi[gavi$country == "TZA", ]$lancet_rates_mean <- gavi[gavi$country == "TZA_M", ]$lancet_rates_mean
#gavi[gavi$country == "TZA", ]$lancet_cases_mean <- gavi[gavi$country == "TZA_M", ]$lancet_cases_mean
#gavi[gavi$country == "TZA", ]$lancet_rates_quantile_low <- gavi[gavi$country == "TZA_M", ]$lancet_rates_quantile_low
#gavi[gavi$country == "TZA", ]$lancet_cases_quantile_low <- gavi[gavi$country == "TZA_M", ]$lancet_cases_quantile_low
#gavi[gavi$country == "TZA", ]$lancet_rates_quantile_high <- gavi[gavi$country == "TZA_M", ]$lancet_rates_quantile_high
#gavi[gavi$country == "TZA", ]$lancet_cases_quantile_high <- gavi[gavi$country == "TZA_M", ]$lancet_cases_quantile_high

#gavi[gavi$country == "TZA_M", ]$lancet_rates_mean <- NA
#gavi[gavi$country == "TZA_M", ]$lancet_cases_mean <- NA
#gavi[gavi$country == "TZA_M", ]$lancet_rates_quantile_low <- NA
#gavi[gavi$country == "TZA_M", ]$lancet_cases_quantile_low <- NA
#gavi[gavi$country == "TZA_M", ]$lancet_rates_quantile_high <- NA
#gavi[gavi$country == "TZA_M", ]$lancet_cases_quantile_high <- NA
...
#csv_output <- read_csv(paste0(csv_dir, "completed_report/csv_output_template.csv"))
...
rateRatio <- dplyr::filter(datlong, type == "rates" & source %in% c("current", "lancet")) %>%
    ...
    #mutate(current_unlist = unlist(current),
    #       lancet_unlist = unlist(lancet)) %>% 
    #select(-current, -lancet) %>% 
    #rename(current = current_unlist, lancet = lancet_unlist) %>% 
...
pop_prop <- read_csv(paste0(csv_dir, "gavi_pop.csv"))
#pop_prop[match("TZA", pop_prop$country_iso), c("country_iso")] <- "TZA_M"
csv_output <- read_csv(paste0(csv_dir, "completed_report/csv_output_template.csv"))
...
readr::write_csv(dplyr::filter(csv_output, `country code` != "ZNZ"), "csv_output_done.csv")
#readr::write_csv(csv_output, "csv_output_done.csv")
...
#pop_prop <- pop_prop[-(match("ZNZ", pop_prop$country_iso)), ]
...
africa <- sf::st_read('/Users/kaiyuezou/Desktop/gavi_report/new_africa_shapefiles/afr_g2014_2013_0/afr_g2014_2013_0.shp', quiet = TRUE)

pop_prop <- readr::read_csv('/Users/kaiyuezou/Desktop/gavi_report/gavi_pop.csv') 
pop_prop <- pop_prop %>% 
  mutate(high = `>=0.001_mean_value`,
         medium = `>=0.0001_mean_value`,
...
readr::write_csv(dplyr::filter(table1, Source == "JHU" & Years == "2015-2019"), "table1.csv")
readr::write_csv(dplyr::filter(table1, Country != "Zanzibar, Tanzania"), "table1_full.csv")
...
dplyr::filter(datlong, type == "rates") %>%
  ...
  dplyr::select(country, source, years, mean, low, high) %>%
  dplyr::arrange(country) %>%
  dplyr::mutate(mean = mean*10000, low = low*10000, high = high*10000) %>%
...
readr::write_csv(dplyr::filter(table2, Source == "JHU" & Years == "2015-2019"), "table2.csv")
readr::write_csv(dplyr::filter(table2, Source != "WHO" & Country != "Zanzibar, Tanzania"), "table2_full.csv")
...
readr::write_csv(dplyr::filter(table3, Country != "Zanzibar, Tanzania"), "table3.csv")
