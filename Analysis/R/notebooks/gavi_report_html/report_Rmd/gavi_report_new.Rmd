...
csv_output <- csv_output %>%
  ...
  group_by(indicator) %>%
  mutate(
    `country code` = unique(datlong$countrycode),
...
pop_prop <- pop_prop %>%
  mutate(
    High = `>=0.001_mean_value` * pop_2017 / 1e8,
    Medium = `>=0.0001_mean_value` * pop_2017 / 1e8
  ) %>% # 6 for million and 2 for %
  select(country_iso, High, Medium) %>%
...
plot(africa_high,
  xlim = c(-20, 60), ylim = c(-40, 40), col = my_colors_high, bg = "#FFFFFF",
  main = "",
  xlab = "",
  ylab = "",
  sub = ""
...
plot(africa_high_medium,
  xlim = c(-20, 60), ylim = c(-40, 40), col = my_colors_high_medium, bg = "#FFFFFF",
  main = "",
  xlab = "",
  ylab = "",
  sub = ""
