...
# Create coordinates of cell centroids Avoid using st_cendroid which fails on
# MARCC for some unkown reason
...
df <- purrr::map_dfr(1:length(stan_data$y), function(i) {
  ...
  # ind_lp <- stan_data$map_obs_loctime_loc[which(stan_data$map_obs_loctime_obs ==
  # stan_data$ind_full[i])] Get the corresponding grid indices
  ...
  y_new <- purrr::map(seq_along(ind_obs), function(x) {
    rep(
      stan_data$y[i] * tfrac[x] / sum(tfrac),
  ...
  return(tibble::tibble(
    obs = i, raw_y = stan_data$y[i], y = y, sx = sx, sy = sy,
  ...
  tibble::as_tibble() %>%
  dplyr::mutate(
    obs_year = factor(obs_year), log_ey = log(ey), log_tfrac = log(tfrac),
...
if (warmup) {
  ...
  if (config$time_effect & timevary_model) {
    frml <- paste(c(frml, colnames(df %>%
      dplyr::select(dplyr::contains("year_")))), collapse = " + ")
  }
  ...
  if (config$censoring) {
    ...
    df <- df %>%
      dplyr::filter(!(y == 0 & right_threshold == 0))
  }
  ...
  predict_df <- tibble::tibble(sx = coord_frame$x[indall], sy = coord_frame$y[indall]) %>%
  ...
  if (stan_data$ncovar >= 1 & covar_warmup) {
    ...
    beta_effect <- as.matrix(dplyr::select(predict_df, dplyr::contains("beta"))) %*%
      matrix(coef(gam_fit)[stringr::str_detect(names(coef(gam_fit)), "beta")],
        ncol = 1
      )
  ...
  if (config$time_effect | config$smoothing_period != 1) {
    ...
    if (config$smoothing_period != 1) {
      ...
      init.list <- lapply(1:nchain, function(i) {
        list(w = rnorm(length(w.init) * config$smoothing_period, rep(
          w.init,
    ...
    if (config$time_effect) {
      ...
      ## etas
      init.list <- lapply(1:nchain, function(i) {
        list(w = rnorm(length(w.init), w.init, 0.1), eta_tilde = as.array(rnorm(
          length(eta),
          eta / stan_data$sigma_eta_scale, 0.05
  ...
  if (stan_data$ncovar >= 1 & covar_warmup) {
    ...
    for (i in 1:length(init.list)) {
      ...
      init.list[[i]] <- append(init.list[[i]], list(betas = rnorm(
        length(betas),
        betas, 0.1
      ) %>%
        array()))
...
initial_values_data <- list(stan_data = stan_data, init.list = init.list, gam_fit_input = df)
if (warmup) {
