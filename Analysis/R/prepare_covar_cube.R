...
prepare_covar_cube <- function(covar_list, dbuser, map_name, cholera_directory, full_grid_name,
                               start_time, end_time, res_space, res_time, username, dbname = "cholera_covariates") {
  ...
  # Define modeling time slices (set of time periods at which the data generating
  # process occurs)
  ...
  # Start and end dates of the model model_date_range <- seq.Date(start_time,
  # end_time + 1, res_time)
  ...
  n_grid_cells <- DBI::dbGetQuery(conn_pg, glue::glue_sql("SELECT COUNT(*) FROM {`{DBI::SQL(cntrd_table)}`};",
    .con = conn_pg
  )) %>%
    .[["count"]] %>%
    as.numeric()
  ...
  covar_cube <- array(rep(0, times = n_time_slices * n_grid_cells * n_covar), c(
    n_grid_cells,
    n_time_slices, n_covar
  ))
  ...
  for (j in seq_along(covar_list)) {
    ...
    covar_date_metadata <- DBI::dbGetQuery(conn_pg, glue::glue_sql("SELECT src_res_time, res_time, first_TL, last_TL
    ...
    if (covar_date_metadata$src_res_time != "static") {
      # Define the left and right bounds of the temporal slices covered by the
      # covariates
      covar_TL_seq <- seq.Date(as.Date(covar_date_metadata$first_tl[1]), as.Date(covar_date_metadata$last_tl[1]),
        by = res_time
      )
      covar_TR_seq <- aggregate_to_end(time_change_func(covar_TL_seq))

      cat("---- Extracting ", covar, "\n")
      covar_bands <- taxdat::get_temporal_bands(
        model_time_slices = time_slices,
        covar_TL_seq = covar_TL_seq, covar_TR_seq = covar_TR_seq
      )

      if (length(covar_bands$ind) != nrow(time_slices)) {
        stop(
          "Failed to match modeling and covariate time slices for covariate: ",
          covar, ". N model time slices: ", nrow(time_slices), "; N covar time slices: ",
          length(covar_bands$ind)
        )
      }
    } else {
    ...
    tmp <- DBI::dbGetQuery(conn_pg, query)
    ...
    if (length(tmp) > 0) {
      ...
      if (!identical(dim(dat)[1:2], dim(covar_cube[, , j, drop = FALSE])[1:2])) {
        stop(
          "Dimensions of extraction for covariate ", covar, " do no match the ones of covar_cube:\n",
          "dim extraction: ", paste(dim(dat), collapse = " x "), "\ndim covar_cube: ",
          paste(dim(covar_cube[, , j]), collapse = " x ")
        )
      }
    ...
    dimnames(covar_cube)[[3]] <- stringr::str_split(covar_list, "\\.") %>%
      purrr::map_chr(~ .[2])
  ...
  non_na_gridcells <- which(apply(covar_cube, 1:2, function(x) {
    (x[1] >= 1) && sum(is.na(x)) == 0
  }))
  # non_na_gridcells <- which(as.numeric(apply(covar_cube, c(1,2), function(x)
  # {(x[1] >= 1) && sum(is.na(x)) == 0})>0))
  grid_changer <- setNames(seq_len(length(non_na_gridcells)), non_na_gridcells)

  # Get the modelling grid
  sf_grid <- sf::st_read(conn_pg, query = glue::glue_sql("SELECT p.* FROM
                   {`{DBI::SQL(paste0(full_grid_name, '_polys'))}`} p
                   INNER JOIN {`{DBI::SQL(cntrd_table)}`} c
                   ON p.rid = c.rid AND p.x = c.x AND p.y = c.y",
    .con = conn_pg
  )) %>%
    dplyr::arrange(rid, x, y) %>%
  ...
  sf_grid <- do.call(rbind, lapply(seq_len(ncol(covar_cube)), function(t) {
    sf_grid$t <- t
    sf_grid
  })) %>%
  ...
  location_periods_dict <- dplyr::inner_join(location_periods_dict, as.data.frame(sf_grid) %>%
    dplyr::select(-geom)) %>%
    # arrange(location_period_id, id, t) %>%
    dplyr::mutate(upd_long_id = grid_changer[as.character(long_id)]) %>%
    dplyr::filter(!is.na(upd_long_id))

  # Create a unique location period id which also accounts for the modeling time
  # slice
  ...
  cat("**** FINISHED EXTRACTING COVARITE CUBE OF DIMENSINONS", paste0(dim(covar_cube),
    collapse = "x"
  ), "[n_pix x n_time x n_covar] \n")
  ...
  # Assemble output
  output <- list(
    covar_cube = covar_cube, sf_grid = sf_grid, non_na_gridcells = non_na_gridcells,
    location_periods_dict = location_periods_dict
  )
