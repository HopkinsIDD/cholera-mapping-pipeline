...
prepare_covar_cube <- function(covar_list, dbuser, map_name, cholera_directory, full_grid_name, 
    ...
    covar_cube <- array(rep(0, times = n_time_slices * n_grid_cells * n_covar), c(n_grid_cells, 
    ...
    cat("**** FINISHED EXTRACTING COVARITE CUBE OF DIMENSINONS", paste0(dim(covar_cube), 
        collapse = "x"), "[n_pix x n_time x n_covar] \n")

    # close database
    DBI::dbDisconnect(conn_pg)

    # Assemble output
    output <- list(covar_cube = covar_cube, sf_grid = sf_grid, non_na_gridcells = non_na_gridcells, 
        location_periods_dict = location_periods_dict)
