...
#========================================================READ ME========================================================#
# This new config writer is updated for the paper run on 7/18/2021
...
# The created config files will be stored in 'Analysis/configs/production_tests/2016_2020_no_covariate_run'  
# All the changes made on top of the configs generated by this writer may be recorded on GitHub 
# Different sets of runs only differ in terms of covariate choices                                             
#========================================================READ ME========================================================#  
working_directory <- "/.../cholera-mapping-pipeline/" #<--------CHANGE THIS AND KEEP BOTH "//" ON BOTH SIDES
setwd(working_directory)
config_path <- "Analysis/configs/production_tests/2016_2020_GAM_Stan_comparison/with_covariates" #<-----dependent on what covariate to test
...
# locs <- locs[locs$country %in% c("KEN", "BEN"), ]
...
params_df <- data.frame(
    ...
    genquant = 'update_yearly_dagar_timevary_speedup_flexible_generate', 
    niter = 2000,
    recompile = TRUE
              )
...
if(scale == "region"){
  ...
  for (j in 1:nrow(locs)){
    ...
    sink(file = config_fname)
    auto_config_Aug2022(par, covar_names, ctry_name, ctry_id)
    sink()
  }
