...
#'
prepare_stan_input <- function(dbuser, cholera_directory, smooth_covariate_number_timesteps, 
    ncore, res_time, time_slices, cases_column, sf_cases, non_na_gridcells, sf_grid, 
    location_periods_dict, covar_cube) {
    ...
    # sf_grid <- filter(sf_grid, as.logical(st_intersects(sf_grid,
    # st_read('/home/perez/temporary/tmp.shp')))) %>% dplyr::mutate(long_id =
    # row_number()) sf_grid <- dplyr::mutate(sf_grid, long_id = row_number())
    ...
    # this is the mapping between the grid ids (from 1 to number of cells that
    # intersect location_periods) and the model ids in space (from 1 to the number of
    # non-NA cells, i.e. cells for which all covariates are non-NA). Note that model
    # ids here do not consider time slices, this is done when computing the adjacency
    # list
    ...
    for (it in model_time_slices) {
        ...
        if (length(isolated_vertices) > 1) {
            cat("Found", length(isolated_vertices), "isolated vertices in time slice", 
                it, ", adding edges until only one remains.\n")

            for (vertex in isolated_vertices[-1]) {
                dist_to_main <- as.vector(sf::st_distance(smooth_grid_it[vertex, 
        ...
        if (ng_cl$no > 1) {
            cat("Found", ng_cl$no, "clusters of sizes {", paste(ng_cl$csize, collapse = ","), 
                "} in time slice", it, ", adding edges from islands to mainland. \n")
            ...
            for (i in cluster_ids[-mainland]) {
                ...
                nearest_main_ids <- apply(dist_to_main, 1, function(x) which(x == 
                  min(x))[1])
                nearest_dist <- unlist(mapply(x = 1:length(island_ids), y = nearest_main_ids, 
                  function(x, y) dist_to_main[x, y]))
            ...
            if (igraph::clusters(igraph::graph_from_adjacency_matrix(nn_mat))$no > 
                1) {
                print(unique(igraph::clusters(igraph::graph_from_adjacency_matrix(nn_mat))$no > 
                  1))
        ...
        # Extract lower trianglur matrix (to make directed graph) This is deprecated with
        # the new re-ordering nn_mat <- Matrix::tril(nn_mat)

        # if (!Matrix::isSymmetric(nn_mat)) { stop('This should not happen') }

        # Reorder nodes in directed graph to have a single source using the breadth-
        # first algorithm. for DAGAR
        smooth_centroids <- sf::st_geometry(sf::st_centroid(smooth_grid_it))

        nn_mat_reordereding <- taxdat::reorder_single_source(A = nn_mat, coords = sf::st_coordinates(smooth_centroids))
        ...
        nonzero_ind <- apply(reorderered_adj_list, 2, function(x) nn_mat_reordereding$reordering[x]) + 
            cnt
        ...
        cell_id_mapping <- rbind(cell_id_mapping, data.frame(s = it, id = ids$id, 
    ...
    # adjacency_list <- matrix(nrow = 0, ncol = 2) cnt <- 0 for (i in
    # seq_along(nearest_neighbor_matrices)) { n_entries <-
    # nrow(nearest_neighbor_matrices[[i]]) nonzero_ind <-
    # as.matrix(Matrix::which(nearest_neighbor_matrices[[i]]>0, arr.ind = T)) + cnt
    # adjacency_list <- rbind(adjacency_list, nonzero_ind) # Update the counter to
    # account for the time slices. Note that n_entries for now # is the same for all
    # time slices, since the non-NA cells are determined across # time slices of
    # covariates cnt <- cnt + n_entries } adjacency_list <-
    # adjacency_list[!is.na(adjacency_list[, 1]), ]

    # flag for time adjacency
    flag_adj <- F
    if (flag_adj) {
        stop("Time Adjacency has not been implemented")
        # Add time adjacency time_adj_inds <- matrix(ncol = 2) for (i in
        # seq_along(grid_changer)) { # Get all overall grid ids ids <-
        # sf_grid$long_id[sf_grid$id == grid_changer[i]] # ! THIS CONNECTS ALL TIMES,
        # should change this for (j in ids) { for (k in ids) { from_id <-
        # cell_id_mapping$long_id[cell_id_mapping$ids == j] to_id <-
        # cell_id_mapping$long_id[cell_id_mapping$ids == k] if (length(from_id) > 0 &
        # length(to_id) > 0) if (from_id != to_id) time_adj_inds <- rbind(time_adj_inds,
        # matrix(c(from_id, to_id), ncol = 2)) } } } time_adj_inds <-
        # time_adj_inds[!is.na(time_adj_inds[, 1]), ] adjacency_list <-
        # rbind(adjacency_list, time_adj_inds)
    }

    adjacency_list <- adjacency_list[order(adjacency_list[, 1]), ]
    # Get number of neighboors (here both in space AND time)
    positive_neighbors <- dplyr::mutate(dplyr::summarize(dplyr::group_by(as.data.frame(adjacency_list), 
        row), n = length(col)))
    ...
    # the first covariate is always the population (n_grid_cells x n_obs_t_units x
    # n_covar)
    stan_data$pop <- as.numeric(covar_cube[, , 1])[non_na_gridcells]

    # Mapping between observations to location periods and between location periods
    # and grid cells
    ind_mapping <- taxdat::get_space_time_ind_speedup(df = sf_cases, lp_dict = location_periods_dict, 
        model_time_slices = time_slices, res_time = res_time, n_cpus = ncore, do_parallel = FALSE)
    ...
    if (config$aggregate) {
        ...
        for (i in seq_len(nrow(sf_cases_resized))) {
            sf_cases_resized$loctime[i] <- paste(ind_mapping$map_obs_loctime_loc[ind_mapping$map_obs_loctime_obs == 
                non_na_obs[[i]]], collapse = ", ")
        }
        sf_cases_resized <- taxdat::aggregate_case_data(sf_cases_resized, "loctime")
    }
    if (config$remove_redundant_observations) {
        print("---- REMOVING OVERLAPPING OBSERVATIONS FROM CHOLERA DATA ----")
        taxdat::remove_overlapping_observations(sf_cases_resized)
    }
    if ((config$remove_redundant_observations) || (config$aggregate)) {
        # Re-compute space-time indices based on aggretated data
        ind_mapping_resized <- taxdat::get_space_time_ind_speedup(df = sf_cases_resized, 
            lp_dict = location_periods_dict, model_time_slices = time_slices, res_time = res_time, 
            n_cpus = ncore, do_parallel = F)
    } else {
    ...
    if (!is.null(config$tfrac_thresh)) {
        ...
        obs_remove_thresh <- unique(ind_mapping_resized$obs[ind_mapping_resized$tfrac < 
            as.numeric(config$tfrac_thresh)])

        if (length(obs_remove_thresh) == 0) {
            cat("---- FOUND none of", nrow(sf_cases_resized), "observations that are under the tfrac threshold of", 
                config$tfrac_thresh, "\n")
        } else {
            cat("---- REMOVING", length(obs_remove_thresh), "of", nrow(sf_cases_resized), 
                "observations that are under the tfrac threshold of", config$tfrac_thresh, 
                "\n")
            ...
            # Re-compute space-time indices based on aggretated data
            ind_mapping_resized <- taxdat::get_space_time_ind_speedup(df = sf_cases_resized, 
                lp_dict = location_periods_dict, model_time_slices = time_slices, 
                res_time = res_time, n_cpus = ncore, do_parallel = F)
        }
    }
    ...
    if (!is.null(set_tfrac) && (set_tfrac)) {
        ...
        ind_mapping_resized$tfrac <- rep(1, length(ind_mapping_resized$tfrac))
    }
    ...
    # We take the average over time so we can sum population over time
    y_tfrac <- tibble::tibble(tfrac = stan_data$tfrac, map_obs_loctime_obs = stan_data$map_obs_loctime_obs) %>%
    ...
    censoring_inds <- purrr::map_chr(1:stan_data$M, function(x) {
    ...
    stan_data$ind_full <- which(censoring_inds == "full") %>%
        array()
    stan_data$M_full <- length(stan_data$ind_full)
    # TODO Left-censoring is not implemented for now
    stan_data$ind_left <- which(censoring_inds == "left-censored") %>%
        array()
    stan_data$M_left <- length(stan_data$ind_left)
    stan_data$ind_right <- which(censoring_inds == "right-censored") %>%
        array()
    stan_data$M_right <- length(stan_data$ind_right)
    stan_data$censoring_inds <- censoring_inds

    bad_data <- as.data.frame(sf_cases)[!(seq_len(nrow(sf_cases)) %in% non_na_obs_resized), 
        c("id", "TL", "TR", cases_column, "valid", "attributes.location_period_id")]
    if (nrow(bad_data) > 0) {
        cat(paste("The following observations were thrown out because they did not cover any gridcells\n ", 
            "id,TL,TR,cases,valid,location_period_id\n ", paste(bad_data$id, bad_data$TL, 
                bad_data$TR, bad_data[[cases_column]], bad_data$valid, bad_data$attributes.location_period_id, 
                sep = ",", collapse = "\n  "), "\n"))
    } else {
    ...
    if (stan_data$ncovar > 0) {
        # Flatten covariate cube to 2d array: [n_pix * n_time_units] * [n_cov] Here the
        # first covariate corresponds to the population raster, so needs to be excluded.
        # Data flattened by pixels first, meaning that stan_data$covar[1:10] =
        # covar_cube[1:10, 1, 2] TODO check if the index removing the first covarcub
        # column which should correspond to population is correct

        stan_data$covar <- matrix(apply(covar_cube, 3, function(x) x[non_na_gridcells])[, 
            -1], nrow = length(non_na_gridcells))

        for (i in rev(seq_len(stan_data$ncovar:1))) {
            # Throw out constant covariates since they mess up the model TODO check whether
            # the indexing in the if is correct (no indexing before)
            if ((max(stan_data$covar[, i]) - min(stan_data$covar[, i])) < 1e-06) {
        ...
        standardize_covar = function(M) cbind(M[, 1], apply(M[, -1, drop = F], 2, 
            standardize))
    ...
    full_grid <- dplyr::left_join(sf::st_drop_geometry(sf_grid), sf::st_drop_geometry(smooth_grid))[, 
        c("upd_id", "smooth_id", "t")]
    ...
    # Compute the mean incidence Note that this is in the model's temporal resolution
    ...
    cat("----\nMean cholera incidence is of", formatC(stan_data$meanrate * 1e+05, 
        format = "e", digits = 2), "cases per 100'000 people per", res_time, "\n----")
    ...
    return(list(stan_data = stan_data, sf_cases_resized = sf_cases_resized, sf_grid = sf_grid, 
