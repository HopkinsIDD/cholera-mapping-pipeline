...
#'
prepare_stan_input <- function(dbuser,
                               cholera_directory,
  ...
  for (it in model_time_slices) {
    ...
    nn_mat_reordereding <- taxdat::reorder_single_source(
      A = nn_mat,
  ...
  if (config$aggregate) {
    ...
    sf_cases_resized <- sf_cases_resized %>%
      ...
      dplyr::group_modify(function(.x, .y) {
        cat("iter", unlist(.y), "\n")
        if (nrow(.x) <= 1) {
          return(.x %>% dplyr::select(TL, TR, !!rlang::sym(cases_column)))
        }
        ...
        .x <- .x %>%
          group_by(set) %>%
          summarize(TL = min(TL), TR = min(TL) + sum(duration) - 1, !!cases_column := sum(!!rlang::sym(cases_column), na.rm = TRUE)) %>%
          ungroup() %>%
          dplyr::select(-set)
