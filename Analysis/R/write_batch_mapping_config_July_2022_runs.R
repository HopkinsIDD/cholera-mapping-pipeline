...
config_path <- "Analysis/configs/production_tests/2016_2020_covariate_selection/2016_2020_dist_to_water" #<-----dependent on what covariate to test
...
# covar_names <- c()
covar_names <- c("dist_to_water") #<-----dependent on what covariate to test
...
if(scale == "region"){
  ...
  for (j in 1:nrow(locs)){
    ...
    sink(file = config_fname)
    taxdat::auto_config_July2022(par, covar_names, ctry_name, ctry_id)
    sink()
  }
