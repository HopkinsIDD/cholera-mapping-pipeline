...
# = T)) { install.packages(pkgs = package) library(package = package,
# character.only = T) } detach(pos = which(grepl(package, search())), force = T)
# }
...
# - - - - Countries over which to run the model For sql, use numeric ids For api,
...
# - - - - Get various functions to convert between time units and dates Function
# to convert from date to temporal grid
...
# Determine the column name that the number of cases is stored in TODO check the
# flag for use_database
