...
all_dfs$observations_df[which(all_dfs$observations_df$qualified_name == "1"), ]$suspected_cases  <- all_dfs$observations_df[which(all_dfs$observations_df$qualified_name == "1"), ]$suspected_cases * 3
...
config <- list(general = list(
  ...
  directory = rprojroot::find_root_file(
    criterion = ".choldir",
    "Analysis", "Stan"
  ), ncores = 4, model = "dagar_seasonal_flexible.stan", niter = as.integer(Sys.getenv("CHOLERA_TEST_ITERATION", 100)),
  recompile = TRUE
), file_names = list(stan_input = rprojroot::find_root_file(
  criterion = ".choldir",
  "Analysis", "output", "test3.stan_input.rdata"
  ...
  Model_covariates = covariates_table$Model_covariates, Observations_with_inconsistent_data = paste0(
    ...
    all_dfs$observations_df[which(all_dfs$observations_df$qualified_name == "1"), ]$cases / sum(all_dfs$observations_df[grep("1::", all_dfs$observations_df$qualified_name), ]$cases), " times of the cases reported at the subnational level."
  ),
