...
test_observations <- observe_polygons(test_polygons = dplyr::mutate(all_dfs$shapes_df,location = qualified_name, geometry = geom), test_covariates = raster_df, underlying_distribution = test_underlying_distribution, 
...
###
   test_covariates2 <- create_multiple_test_covariates(test_raster = test_raster,
                                                      ncovariates = ncovariates,
                                                      nonspatial = !nonspatial,
                                                      nontemporal = !nontemporal,
                                                      spatially_smooth = !spatially_smooth,
   ...
   covariate_raster_funs2 <- taxdat:::convert_simulated_covariates_to_test_covariate_funs(test_covariates2,
    ...
    my_seed <- .GlobalEnv$.Random.seed
###
    test_covariates3 <- create_multiple_test_covariates(test_raster = test_raster,
                                                        ncovariates = ncovariates,
                                                        nonspatial = nonspatial,
                                                        nontemporal = nontemporal,
                                                        spatially_smooth = !spatially_smooth,
...
config <- list(general = list(location_name = all_dfs$location_df$qualified_name[[1]], 
               ...
               data_source = "sql", file_names = list(stan_output = rprojroot::find_root_file(criterion = ".choldir", "Analysis", "output", "test.stan_output.rdata"), 
