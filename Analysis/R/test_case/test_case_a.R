...
#   write.csv(seed,file="test_case_a_seed_0823.csv")
#   return(as.integer(read.csv(file = "test_case_a_seed_0823.csv")[[2]]))
# }
# 
# idx<-1
...
## report
rds_file <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_a_data_simulation_covariates.rds")
...
rds_file <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_a_true_grid_cases.rds")
if (!dir.exists(dirname(rds_file))) {
  dir.create(dirname(rds_file))
}
...
config_filename <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "configs", "config_test_case_a.yml")
...
config <- list(general = list(
  ...
  Cov_data_simulation_filename = file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_a_data_simulation_covariates.rds"),
  test_true_grid_case_filename = file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_a_true_grid_cases.rds")
))
...
rmarkdown::render(
  ...
  output_file = "test_case_a_country_data_report_0823"
)