...
## base_number is the number of locations at the most coarse spatial scale; nlayer
## is the number of spatial levels.  factor: on average, the number of immediate
## child location each location has (if it's not randomized, then it'll be the
## exact number of immediate child locations each location has;if it's randomized,
## then it'll be the average number of immeidate child locaitons)
...
shape_df_names = data.frame(name = colnames(all_dfs$shapes_df), crs = sf::st_crs(all_dfs$shapes_df[, 
    colnames(all_dfs$shapes_df) == "geom"])$proj4string)
...
# ##
# ------------------------------------------------------------------------------------------------------------------------
# ## Change observations the process is like this: a distribution for
# cases/values for each grid cell, based on the underlying distribution, draw
# cases for each grid cell and aggregate to the polygon level, if the draw is
# larger than 1, then, draw multiple times.  the observation process can be
# biased, by specifying polygon_observation/temporal(more likely to be observed
# for termporally close observations)/spatial_bias (more likely to be observed
# for spatially close observations) arguments in the function.
...
# ##
# ------------------------------------------------------------------------------------------------------------------------
# ## Create Database config is a special format to store coviarates for the
# script we're going to run (may need to change: time scales based on the nlayers
# in test_raster and the time ranges of pulled data)
