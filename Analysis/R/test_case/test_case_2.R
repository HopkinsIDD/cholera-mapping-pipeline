...
config <- list(
  ...
  stan = list(
    directory = rprojroot::find_root_file(criterion = ".choldir", "Analysis", "Stan"),
             ...
             file_names = list(
               stan_input = rprojroot::find_root_file(criterion = ".choldir", "Analysis", "output", "test2.stan_input.rdata"),
               stan_output = rprojroot::find_root_file(criterion = ".choldir","Analysis", "output", "test2.stan_output.rdata")
             ),
             test_metadata = list(
               name = "test_2",
               nrows=10,
               ...
               Observations_with_inconsistent_data=paste0(
                 "Nationally reported data is ",
                 all_dfs$observations_df[which(all_dfs$observations_df$qualified_name=="1"),]$suspected_cases /
                   sum(all_dfs$observations_df[grep("1::",all_dfs$observations_df$qualified_name),]$suspected_cases),
                 " times of the cases reported at the subnational level."),
               Loc_with_inconsistent_data="-",
               Cov_data_simulation_filename="/home/app/cmp/Analysis/output/test_case_2_data_simulation_covariates.rdata"
  )
)
