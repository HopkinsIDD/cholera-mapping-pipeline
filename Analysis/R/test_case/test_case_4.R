...
my_seed <- c(10403, 624, 105045778, 1207077739, 2042172336, -219892751, -768060162,
    ...
    as.integer()
cov3_seed <- c(10403, 624, 105045778, 1207077739, 2042172336, -219892751, -768060162,
    -2006256281, -1585201540, -978856627, -1568163926, -1028934365, 1356190728, 1795633769,
    1153151766, 1165788831, 2116870228, 833087301, 829928258, 1681319387, -277008544,
    1376178145, -672930770, 1480724183, 377343276, 861177917, -304533030, -1337000557,
    800359864, -468620199, -254982074, -1990955505, -1337858812, 1651087413, -1015114894,
...
## ------------------------------------------------------------------------------------------------------------------------
## Change covariates
covariates_table <- data.frame(nonspatial = c(FALSE, FALSE, TRUE), nontemporal = c(FALSE,
    FALSE, FALSE), spatially_smooth = c(TRUE, TRUE, TRUE), temporally_smooth = c(FALSE,
    FALSE, FALSE), polygonal = c(TRUE, TRUE, TRUE), radiating = c(FALSE, FALSE, FALSE),
    constant = c(TRUE, FALSE, FALSE), Data_simulation_covariates = c(TRUE, TRUE,
        TRUE), Model_covariates = c(TRUE, TRUE, FALSE))
...
test_raster_observation <- create_test_raster(nrows = 10, ncols = 10, nlayers = 2,
    test_extent = test_extent)

test_covariates_observation <- create_multiple_test_covariates(test_raster = test_raster_observation,
...
test_covariates3_observation <- create_multiple_test_covariates(test_raster = test_raster_observation,
    ncovariates = 2, nonspatial = covariates_table$nonspatial[c(1, 3)], nontemporal = covariates_table$nontemporal[c(1,
        3)], spatially_smooth = covariates_table$spatially_smooth[c(1, 3)], temporally_smooth = covariates_table$temporally_smooth[c(1,
        3)], polygonal = covariates_table$polygonal[c(1, 3)], radiating = covariates_table$radiating[c(1,
        3)], constant = covariates_table$constant[c(1, 3)], seed = cov3_seed)
...
covariate_raster_funs_observation <- taxdat:::convert_simulated_covariates_to_test_covariate_funs(test_covariates_observation,
    min_time_left, max_time_right)
covariate3_raster_funs_observation <- taxdat:::convert_simulated_covariates_to_test_covariate_funs(test_covariates3_observation,
    min_time_left, max_time_right)
...
## report
saveRDS(test_covariates_observation_final, "/home/app/cmp/Analysis/output/test_case_4_data_simulation_covariates.rdata")
...
all_dfs$observations_df[which(all_dfs$observations_df$qualified_name == "1"), ]$suspected_cases = sum(all_dfs$observations_df[grep("1::",
    all_dfs$observations_df$qualified_name), ]$suspected_cases) * 3
# partially covered for certain polygons
all_dfs$observations_df = all_dfs$observations_df %>%
    subset(!qualified_name %in% c("1::2", "1::10", "1::20"))
...
config_filename <- "/home/app/cmp/Analysis/R/config_test_case_4.yml"
...
config <- list(general = list(location_name = all_dfs$location_df$qualified_name[[1]],
    ...
    width_in_km = 1, height_in_km = 1, time_scale = "year", covariates = unique(sapply(covariate_raster_funs,
        function(x) {
            x$name
        }))), stan = list(directory = rprojroot::find_root_file(criterion = ".choldir",
    "Analysis", "Stan"), ncores = 4, model = "dagar_seasonal_flexible.stan", niter = 4000,
    recompile = TRUE), file_names = list(stan_input = rprojroot::find_root_file(criterion = ".choldir",
    "Analysis", "output", "test4.stan_input.rdata"), stan_output = rprojroot::find_root_file(criterion = ".choldir",
    "Analysis", "output", "test4.stan_output.rdata")), test_metadata = list(name = "test_1",
    ...
    Model_covariates = covariates_table$Model_covariates, Observations_with_inconsistent_data = paste0("Nationally reported data is ",
        all_dfs$observations_df[which(all_dfs$observations_df$qualified_name == "1"),
            ]$suspected_cases/sum(all_dfs$observations_df[grep("1::", all_dfs$observations_df$qualified_name),
            ]$suspected_cases), " times of the cases reported at the subnational level."),
    Loc_with_inconsistent_data = "-", Cov_data_simulation_filename = "/home/app/cmp/Analysis/output/test_case_4_data_simulation_covariates.rdata"))
...
rmarkdown::render(rprojroot::find_root_file(criterion = ".choldir", "Analysis", "output",
    "country_data_report_test_case.Rmd"), params = list(config_filename = config_filename,
    ...
    output_file = "Country data report test case 4 updated")
