...
all_dfs$observations_df[which(all_dfs$observations_df$qualified_name=="1"),]$suspected_cases=sum(all_dfs$observations_df[grep("1::",all_dfs$observations_df$qualified_name),]$suspected_cases)*3
...
config <- list(general = list(location_name = all_dfs$location_df$qualified_name[[1]], 
               ...
               file_names = list(stan_output = rprojroot::find_root_file(criterion = ".choldir","Analysis", "output", "test4.stan_output.rdata"), 
                                 stan_input = rprojroot::find_root_file(criterion = ".choldir", "Analysis", "output", "test4.stan_input.rdata")),
               nrows=10,
               ncols=10,
               data_type="Grid data",
               oc_type="-",
               ...
               Cov_data_simulation_filename="/home/app/cmp/Analysis/output/test_case_4_data_simulation_covariates.rdata"
)
