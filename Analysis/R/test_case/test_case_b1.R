...
# 
my_seed <- c(
  10403,3,1235579427,-536490051,-1474526166,-1243979237,936407911,565438754,1449575741,319694383,
...
#   write.csv(seed,file="test_case_b1_seed_0823.csv")
#   return(as.integer(read.csv(file = "test_case_b1_seed_0823.csv")[[2]]))
# }
# 
# idx<-1
...
query_time_left <- c(lubridate::ymd("2000-01-01"), lubridate::ymd("2000-12-31"),lubridate::ymd("2001-12-31"),lubridate::ymd("2002-12-31"))
query_time_right <- c(lubridate::ymd("2000-12-31"), lubridate::ymd("2001-12-31"),lubridate::ymd("2002-12-31"),lubridate::ymd("2003-12-31"))
# query_time_left <- c(lubridate::ymd("2000-01-01"), lubridate::ymd("2001-01-01"),lubridate::ymd("2002-01-01"),lubridate::ymd("2003-01-01"))
# query_time_right <- c(lubridate::ymd("2000-12-31"), lubridate::ymd("2001-12-31"),lubridate::ymd("2002-12-31"),lubridate::ymd("2003-12-31"))
...
## report
rds_file <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_b1_data_simulation_covariates.rds")
...
# }
#all_dfs$observations_df<-all_dfs$observations_df[!all_dfs$observations_df$time_right=="2004-12-31",]
...
rds_file <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_b1_true_grid_cases.rds")
if (!dir.exists(dirname(rds_file))) {
  dir.create(dirname(rds_file))
}
...
config_filename <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "configs", "config_test_case_b1.yml")
...
config <- list(general = list(
  ...
  directory = rprojroot::find_root_file(
    criterion = ".choldir",
    "Analysis", "Stan"
  ), ncores = 4, model = "dagar_seasonal_flexible_narrower_prior.stan", niter = as.integer(Sys.getenv("CHOLERA_TEST_ITERATION", 4000)),
  recompile = TRUE
), file_names = list(stan_input = rprojroot::find_root_file(
  criterion = ".choldir",
  "Analysis", "output", "testb1.stan_input.rdata"
), stan_output = rprojroot::find_root_file(
  criterion = ".choldir",
  "Analysis", "output", "testb1.stan_output.rds"
)), test_metadata = list(
  name = "test_b1",
  ...
  Cov_data_simulation_filename = file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_b1_data_simulation_covariates.rds"),
  test_true_grid_case_filename = file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_b1_true_grid_cases.rds")
))
...
rmarkdown::render(
  ...
  output_file = "test_case_b1_country_data_report_0823"
)