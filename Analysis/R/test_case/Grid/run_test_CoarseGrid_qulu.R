...
test_raster <- create_test_raster(nrows = 5, ncols = 5, nlayers = 2, test_extent)  # fewer grids are observed
...
test_polygons <- create_test_layered_polygons(test_raster = test_raster, base_number = 1, 
    n_layers = 2, factor = 5, snap = FALSE, randomize = FALSE)
...
test_observations <- observe_polygons(test_polygons = dplyr::mutate(all_dfs$shapes_df, 
    ...
    underlying_distribution = test_underlying_distribution, min_time_left = query_time_left, 
    max_time_right = query_time_right, noise = FALSE)
# 
...
## Note 2021-08-01 The grid number can't be 2*2
