...
#   write.csv(seed,file="test_case_c_seed_0823.csv")
#   return(as.integer(read.csv(file = "test_case_c_seed_0823.csv")[[2]]))
# }
# 
# idx<-1
...
## report
rds_file <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_c_data_simulation_covariates.rds")
...
rds_file <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_c_true_grid_cases.rds")
if (!dir.exists(dirname(rds_file))) {
  dir.create(dirname(rds_file))
}
...
config_filename <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "configs", "config_test_case_c.yml")
...
config <- list(general = list(
  ...
  directory = rprojroot::find_root_file(
    criterion = ".choldir",
    "Analysis", "Stan"
  ), ncores = 4, model = "dagar_seasonal_flexible_sum_to_zero_constraint.stan", niter = as.integer(Sys.getenv("CHOLERA_TEST_ITERATION", 4000)),
  recompile = TRUE
  ...
  Cov_data_simulation_filename = file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_c_data_simulation_covariates.rds"),
  test_true_grid_case_filename = file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_c_true_grid_cases.rds")
))
...
rmarkdown::render(
  ...
  output_file = "test_case_c_country_data_report_0823"
)