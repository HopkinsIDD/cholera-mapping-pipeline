...
subnational_query_time_left <- c(lubridate::ymd("2000-01-01"),lubridate::ymd("2000-12-31"))
subnational_query_time_right <- c(lubridate::ymd("2000-12-31"),lubridate::ymd("2001-12-31"))
national_query_time_left<-c(lubridate::ymd("2000-01-01"),lubridate::ymd("2000-12-31"))
national_query_time_right<-c(lubridate::ymd("2000-12-31"),lubridate::ymd("2001-12-31"))
...
test_raster_observation <- create_test_raster(
  nrows = 10, ncols = 10, nlayers = nlayers,
...
for (layer in seq_len(nlayers)) {
  covariate3_raster_funs_observation[[layer + nlayers]]$name <- "covariate3"
  covariate_raster_funs_observation[[layer + 2 * nlayers]] <- covariate3_raster_funs_observation[[layer + nlayers]]
}

## save additional covariates in the data generation process for country data
## report
saveRDS(test_covariates_observation_final, "/home/app/cmp/Analysis/output/test_case_46_data_simulation_covariates.rdata")
...
for (time_index in seq_len(length(national_query_time_left))) {
  test_observations[[time_index]] <- observe_polygons(
    test_polygons = dplyr::mutate(all_dfs$shapes_df[all_dfs$shapes_df$location=="1",],
    ...
    min_time_left = sim_time_left, max_time_right = sim_time_right, observation_time_left = national_query_time_left[[time_index]], observation_time_right = national_query_time_right[[time_index]], seed = my_seed
  )
}

for (time_index in seq_len(length(subnational_query_time_left))) {
  test_observations[[time_index+length(national_query_time_left)]] <- observe_polygons(
    test_polygons = dplyr::mutate(all_dfs$shapes_df[!all_dfs$shapes_df$location=="1",],
                                  location = qualified_name, geometry = geom
    ), test_covariates = raster_df, underlying_distribution = test_underlying_distribution,
    noise = FALSE, number_draws = 1, grid_proportion_observed = 1, polygon_proportion_observed = 1,
    min_time_left = sim_time_left, max_time_right = sim_time_right, observation_time_left = subnational_query_time_left[[time_index]], observation_time_right = subnational_query_time_right[[time_index]], seed = my_seed
  )
}
...
saveRDS(test_true_grid_cases,"/home/app/cmp/Analysis/output/test_case_46_true_grid_cases.rdata")
...
config_filename <- "/home/app/cmp/Analysis/R/config_test_case_46.yml"
...
config <- list(general = list(
  ...
  criterion = ".choldir",
  "Analysis", "output", "test46.stan_input.rdata"
), stan_output = rprojroot::find_root_file(
  criterion = ".choldir",
  "Analysis", "output", "test46.stan_output.rds"
)), test_metadata = list(
  name = "test_46",
  ...
  single_year_run = ifelse(lubridate::year(sim_time_right) - lubridate::year(sim_time_left) ==
  ...
  Model_covariates = covariates_table$Model_covariates, Observations_with_inconsistent_data = paste0(
    "Nationally reported data is 1s times of the cases reported at the subnational level."
  ),
  Loc_with_inconsistent_data = "-", Cov_data_simulation_filename = "/home/app/cmp/Analysis/output/test_case_46_data_simulation_covariates.rdata",test_true_grid_case_filename="/home/app/cmp/Analysis/output/test_case_46_true_grid_cases.rdata"
...
rmarkdown::render(
  ...
  output_file = "test_case_46_country_data_report"
)