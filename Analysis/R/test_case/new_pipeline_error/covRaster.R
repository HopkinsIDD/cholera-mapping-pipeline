...
test_polygons <- sf::st_make_valid(create_test_layered_polygons(
  ...
  base_number = 1, n_layers = 2, factor = 2 * 2, randomize = FALSE,
  seed = my_seed
))
...
## report
rds_file <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_covRaster_data_simulation_covariates.rds")
...
rds_file <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_covRaster_true_grid_cases.rds")
if (!dir.exists(dirname(rds_file))) {
  dir.create(dirname(rds_file))
}
...
config_filename <- file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "configs", "config_test_case_covRaster.yml")
...
config <- list(general = list(
  ...
  criterion = ".choldir",
  "Analysis", "output", "testcovRaster.stan_input.rdata"
), stan_output = rprojroot::find_root_file(
  criterion = ".choldir",
  "Analysis", "output", "testcovRaster.stan_output.rds"
)), test_metadata = list(
  name = "test_covRaster",
  ...
  Cov_data_simulation_filename = file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_covRaster_data_simulation_covariates.rds"),
  test_true_grid_case_filename = file.path(rprojroot::find_root(criterion = ".choldir"), "Analysis", "data", "test_case_covRaster_true_grid_cases.rds")
))
...
rmarkdown::render(
  ...
  output_file = "test_case_covRaster_country_data_report"
)
