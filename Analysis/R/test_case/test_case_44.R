...
subnational_query_time_left <- c(lubridate::ymd("2000-01-01"))
subnational_query_time_right <- c(lubridate::ymd("2000-05-30"))
national_query_time_left <- c(lubridate::ymd("2000-01-01"))
national_query_time_right <- c(lubridate::ymd("2001-12-31"))
...
## report
saveRDS(test_covariates_observation_final, "/home/app/cmp/Analysis/output/test_case_44_data_simulation_covariates.rdata")
...
for (time_index in seq_len(length(subnational_query_time_left))) {
  test_observations[[time_index + 1]] <- observe_polygons(
    test_polygons = dplyr::mutate(all_dfs$shapes_df[!all_dfs$shapes_df$location == "1", ],
      location = qualified_name, geometry = geom
    ), test_covariates = raster_df, underlying_distribution = test_underlying_distribution,
    noise = FALSE, number_draws = 1, grid_proportion_observed = 1, polygon_proportion_observed = 1,
...
saveRDS(test_true_grid_cases, "/home/app/cmp/Analysis/output/test_case_44_true_grid_cases.rdata")
...
config_filename <- "/home/app/cmp/Analysis/R/config_test_case_44.yml"
...
config <- list(general = list(
  ...
  directory = rprojroot::find_root_file(
    criterion = ".choldir",
    "Analysis", "Stan"
  ), ncores = 4, model = "dagar_seasonal_flexible_sum_to_zero_constraint.stan", niter = 4000,
  recompile = TRUE
), file_names = list(stan_input = rprojroot::find_root_file(
  criterion = ".choldir",
  "Analysis", "output", "test44.stan_input.rdata"
), stan_output = rprojroot::find_root_file(
  criterion = ".choldir",
  "Analysis", "output", "test44.stan_output.rds"
)), test_metadata = list(
  name = "test_44",
  ...
  Loc_with_inconsistent_data = "-", Cov_data_simulation_filename = "/home/app/cmp/Analysis/output/test_case_44_data_simulation_covariates.rdata", test_true_grid_case_filename = "/home/app/cmp/Analysis/output/test_case_44_true_grid_cases.rdata"
))
...
rmarkdown::render(
  ...
  output_file = "test_case_44_country_data_report"
)
