...
if (data_source == "api") {
  ...
  cat(
    "cntry:", long_countries, "u:", ifelse(nchar(username) > 0, "****", ""),
...
if (sum(!cases$shapefile.exists) > 0) {
  ...
  print(paste(
    sum(!cases$shapefile.exists), "of", length(cases$shapefile.exists),
  ...
  print(paste(
    sum(problem_cases[[cases_column]]), "/", sum(cases[[cases_column]]),
...
if (any(grepl("GEOMETRYCOLLECTION", sf::st_geometry_type(shapefiles)))) {
  ...
  print(paste("The following location periods are affected:", paste(shapefiles[grepl(
    "GEOMETRYCOLLECTION",
    sf::st_geometry_type(shapefiles)
  ), ][["location_period_id"]], collapse = ", ")))
...
if (any(sf::st_is_empty(sf_cases))) {
  warning(paste(
    "Missing shapefiles for ", sum(sf::st_is_empty(sf_cases)), "observations. They correspond to the following location period IDs:",
