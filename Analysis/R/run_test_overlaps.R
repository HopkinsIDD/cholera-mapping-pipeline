...
option_list <- list(optparse::make_option(c("-d", "--cholera_pipeline_directory"),
  ...
  "-d",
  "--cholera_output_directory"
),
action = "store", default = Sys.getenv(
  "CHOLERA_OUTPUT_DIRECTORY",
  paste0(rprojroot::find_root(".choldir"), "/Analysis/data")
), type = "character",
help = "Output directory"
), optparse::make_option(c("-p", "--postgres_database_name"),
  action = "store", default = Sys.getenv("CHOLERA_POSTGRES_DATABASE", "cholera_covariates"),
  type = "character", help = "Postgres database name"
), optparse::make_option(c("--postgres_database_user"),
...
test_covariates <- create_multiple_test_covariates(
  ...
  nonspatial = c(FALSE, FALSE), nontemporal = c(FALSE, FALSE), spatially_smooth = c(
    TRUE,
    TRUE
  ), temporally_smooth = c(FALSE, TRUE), polygonal = c(TRUE, FALSE), radiating = c(
    FALSE,
    FALSE
  ), seed = my_seed
)
...
observation_time_lefts <- c(observation_time_lefts, observation_time_lefts)
observation_time_rights <- c(observation_time_rights - 100, observation_time_rights)
...
config <- list(
  general = list(
    ...
    directory = rprojroot::find_root_file(
      criterion = ".choldir",
      "Analysis", "Stan"
    ), ncores = 1, model = "dagar_seasonal_priors.stan", niter = 10000,
    recompile = TRUE
  ), name = "test_???", taxonomy = "taxonomy-working/working-entry1",
  smoothing_period = 1, case_definition = "suspected", covariate_choices = raster_df$name,
  data_source = "sql", file_names = list(stan_output = rprojroot::find_root_file(
    criterion = ".choldir",
    "Analysis", "output", "test.stan_output.rdata"
  ), stan_input = rprojroot::find_root_file(
    criterion = ".choldir",
