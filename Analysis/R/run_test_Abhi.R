...
test_polygons <- create_test_layered_polygons(
  test_raster = test_raster, base_number = 1,
...
config <- list(
  general = list(
    ...
    width_in_km = 1, height_in_km = 1, time_scale = "month"
  ), stan = list(directory = rprojroot::find_root_file(
    criterion = ".choldir",
    "Analysis", "Stan"
  ), ncores = 1, model = "dagar_seasonal.stan", niter = 20, recompile = TRUE),
  ...
  file_names = list(stan_output = rprojroot::find_root_file(
    criterion = ".choldir",
