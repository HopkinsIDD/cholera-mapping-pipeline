...
data {
  ...
  int <lower=0, upper=T> map_grid_time[N]; // The time side of the mapping from locations/times to grid
...
transformed parameters {
  ...
  vector[T] eta; // yearly random effects
  ...
  log_lambda =  w[map_smooth_grid] + log_meanrate + covar * betas + eta[map_grid_time];
