...
// 03/03/2021 update for negative-binomial cases (process-level) with poisson/gamm mixture based on Abhi's code
data {
...
parameters {
  ...
  real<lower=0> sqrt_reciprocal_phi; // Overdispersion parameter for the negative-binomial
  vector<lower=0>[N] grid_cases; //cases modeled in each gridcell and time point (becomes a Gamma-distributed parameter in the NegBinom model)
